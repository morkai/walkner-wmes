...
// Recount
// /fix/clip/count-daily-mrp?from=2016-01-03
...
const toUpdate = [{
  oldOrgUnits: {prodLine: 'LM-28'},
  newOrgUnits: {mrpControllers: ['KG9']}
}, {
  oldOrgUnits: {prodLine: 'LM-11'},
  newOrgUnits: {mrpControllers: ['KG9']}
}, {
  oldOrgUnits: {prodLine: 'LM-26'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-23'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-21'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-32'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-9'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-33'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-31'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-29'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-40'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-10'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-8'},
  newOrgUnits: {mrpControllers: ['KF3']}
}, {
  oldOrgUnits: {prodLine: 'LM-7'},
  newOrgUnits: {mrpControllers: ['KF3']}
}];
...
{
  const prodLine = db.prodlines.findOne({_id: update.oldOrgUnits.prodLine});
  ...
  const mrpControllers = db.mrpcontrollers.find({_id: {$in: prodFlow.mrpController}});
  const subdivision = db.subdivisions.findOne({_id: mrpControllers[0].subdivision});
  const division = db.divisions.findOne({_id: subdivision.division});

  update.oldOrgUnits = {
    division: division._id,
    subdivision: subdivision._id,
    mrpControllers: mrpControllers.map(mrp => mrp._id),
    prodFlow: prodFlow._id,
    workCenter: workCenter._id,
    prodLine: prodLine._id
  };

  update.newOrgUnits = Object.assign({}, update.oldOrgUnits, update.newOrgUnits);

  lineToDivision[prodLine._id] = division._id;
});

db.prodlines.find().forEach(pl =>
{
  if (lineToDivision[pl._id])
  {
    return;
  }
