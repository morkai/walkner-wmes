...
exports.DEFAULT_CONFIG = {
  ...
  filterRe: /^T_ZOPP_([0-9]+)\.txt$/,
  uploadUrl: 'http://127.0.0.1/purchaseOrders;import'
};

exports.start = function startPurchaseOrdersExporterModule(app, module)
{
  var filePathCache = {};
  var exportQueue = [];
  ...
  function exportFile(fileInfo)
  {
    ...
    step(
      ...
      function parseStep(err, textPoList)
      {
        if (err)
        {
        ...
        this.poMap = {};
        this.count = parseTextPoList(textPoList, fileInfo.timestamp, this.poMap);

        if (this.count === 0)
        {
          return this.skip(new Error("No purchase orders found."));
        }

        setImmediate(this.next());
      },
      function prepareBodyStep()
      {
