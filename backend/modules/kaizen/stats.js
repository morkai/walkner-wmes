...
module.exports = function setUpKaizenCommands(app, kaizenModule)
{
  ...
  app.broker.subscribe('kaizen.orders.edited', scheduleRecountStats);
  app.broker.subscribe('kaizen.orders.deleted', scheduleRecountStats);

  later.setInterval(recountStats, later.parse.text('on the first day of the month'));

  kaizenModule.getStats = function(userId, done)
  {
  ...
  function recountStats()
  {
    ...
    step(
      ...
      function(err)
      {
        ...
        {
          kaizenModule.debug("[stats] Recounted!");
  ...
  function recountTop10(done)
  {
    ...
    KaizenOrder.aggregate(pipeline, function(err, results)
    {
      if (err)
      {
        return done(err);
  ...
  function countEntries(userId, done)
  {
    ...
    pipeline.push({$group: {
      ...
      monthCount: {$sum: {
        $cond: {
          if: {$gte: ['$eventDate', moment().startOf('month').toDate()]},
          then: 1,
          else: 0
        }
      }}
