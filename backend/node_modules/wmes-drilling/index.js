// Part of <https://miracle.systems/p/walkner-wmes> licensed under <CC BY-NC-SA 4.0>

'use strict';

exports.DEFAULT_CONFIG = {
  mongooseId: 'mongoose',
  expressId: 'express',
  userId: 'user',
  fteId: 'fte',
  settingsId: 'settings',
  updaterId: 'updater',
  generator: false
};

exports.models = [
  require('./models/drillingEvent'),
  require('./models/drillingOrder'),
  require('orders/models/order'),
  require('planning/models/plan'),
  require('planning/models/planSettings')
];

exports.userPrivileges = [
  'DRILLING:VIEW',
  'DRILLING:MANAGE',
  'DRILLING:DRILLER'
];

exports.republishTopics = [
  'drilling.events.saved',
  'drilling.orders.changed.*',
  'drilling.orders.updated.*'
];

exports.optionalModules = {
  'mongoose settings': require('./generator'),
  'mongoose fte user express updater settings': require('./routes')
};
