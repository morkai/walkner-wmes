    ...
    step(
      ...
      function()
      {
        ...
        if (added.length)
        {
          ...
          DrillingOrder.collection.insertMany(added, {ordered: false, writeConcern: {w: 'majority'}}, (err, res) =>
          {
            if (err && err.result)
            {
              res = err.result;
        ...
        if (removed.length)
        {
          DrillingOrder.collection.deleteMany({_id: {$in: removed.map(o => o._id)}}, {writeConcern: {w: 'majority'}}, this.group());
        }

        changed.forEach(change =>
        {
          DrillingOrder.collection.updateOne({_id: change._id}, {$set: change}, {writeConcern: {w: 'majority'}}, this.group());
        });
      },
      function(err)
      {
