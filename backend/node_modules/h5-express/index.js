...
try { messageFormatAmd = require('messageformat-amd'); }
catch (err) {}
...
exports.start = (app, expressModule) =>
{
  ...
  expressModule.sessionStore = mongoose
    ? new MongoStore(mongoose.connection, config.sessionStore)
  ...
  if (config.checkDiskSpace && config.checkDiskSpace.minFreeSpace > 0)
  {
    expressApp.use((req, res, next) =>
    {
      ...
      checkDiskSpace(config.checkDiskSpace.path)
        .then(diskSpace =>
        {
          if (diskSpace.free > config.checkDiskSpace.minFreeSpace)
          {
            return next();
          }

          next(app.createError('Not enough disk space.', 'NO_DISK_SPACE', 500));
        })
        .catch(next);
    });
  }
  ...
  expressApp.use(errorHandlerMiddleware(app, expressModule, errorHandlerOptions));
