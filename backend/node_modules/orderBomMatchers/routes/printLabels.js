...
module.exports = async (app, module, req, res, next) =>
{
  ...
  const {prodLine, secretKey, test, reset} = req.body;
  ...
  try
  {
    ...
    const tplFile = `componentLabel.${componentLabel.template}.ejs`;

    if (reset)
    {
      delete TEMPLATES[tplFile];
    }

    let tpl = TEMPLATES[tplFile];

    if (!tpl)
    {
      tpl = await fs.readFile(`${__dirname}/../templates/${tplFile}`, 'utf8');
      tpl = TEMPLATES[tplFile] = ejs.compile(tpl, {
        cache: false,
        filename: tplFile,
        compileDebug: false,
        rmWhitespace: true
    ...
    tpl = tpl({
      showComponent: componentCode !== '000000000000'
    });
    ...
    for (let no = startNo; no <= endNo; ++no)
    {
      ...
      if (componentCode !== '000000000000')
      {
        code.push(componentCode.padStart(12, '0'));
      }
    ...
    while (remainingZpls.length)
    {
      ...
      if (reset)
      {
        printedZpls = printedZpls.concat(zpls);
      }
      else
      {
    ...
    res.json({
      startNo,
      endNo,
      printedZpls: reset ? printedZpls : undefined
    });
  }
  catch (err)
  {
