...
module.exports = async (app, module, req, res, next) =>
{
  ...
  const {prodLine, secretKey, labelQty, orderNo} = req.body;
  ...
  try
  {
    ...
    const tpl = await fs.readFile(`${__dirname}/../templates/componentLabel.tpl.prn`, 'utf8');

    const componentCode = componentLabel.componentCode;
    ...
    const result = await ComponentLabelCounter.findOneAndUpdate(conditions, update, options).exec();
    ...
    for (let no = startNo; no <= endNo; ++no)
    {
      remainingZpls.push(html2pdf.compileZpl(tpl, {
        ...
        COUNTER: no,
        LABEL_QTY: 1,
        CODE: `PL04.${orderNo.padStart(9, '0')}.${no.toString().padStart(4, '0')}.${componentCode.padStart(12, '0')}`
      }));
    }
