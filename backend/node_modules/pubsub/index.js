...
exports.start = (app, module) =>
{
  ...
  {
    app.broker.subscribe(topic, (message, topic) =>
    {
  ...
  module.on('message', (topic, message, meta) =>
  {
    ...
    // TODO Remove optional binary support after transition period
    if (!meta.json)
    {
      meta.json = true;
    ...
    const binaryMeta = {...meta, binary: true};
    const binaryMessage = meta.binary ? message : Buffer.from(message, 'utf8');
    ...
    */
    ...
    idToMessageMap[meta.messageId] = [topic, message, meta, binaryMessage, binaryMeta];
  ...
  function sendMessages()
  {
    ...
    for (let i = 0, l = socketIds.length; i < l; ++i)
    {
      ...
      // TODO Remove optional binary support after transition period
      const payloadI = socket.handshake.binary ? 3 : 1;
      const metaI = socket.handshake.binary ? 4 : 2;
      ...
      for (let j = 0, m = messageIds.length; j < m; ++j)
      {
        ...
        socket.emit('pubsub.message', message[0], message[payloadI], message[metaI]);
