...
{
  const {gdrive, settings, HelpFile} = module;
  const logger = module.logger.create({submodule: 'gdrive'});
  ...
  function reloadTree()
  {
    ...
    step(
      ...
      function(err, changeCount)
      {
        if (err)
        {
          logger.error(`Failed to reload tree: ${err.message}`);

          setTimeout(reloadTree, 120000);
        }
        else
        {
          logger.info(`Tree reloaded in ${Date.now() - startTime}ms (${changeCount} changes)!`);
  ...
  function monitorChanges()
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          logger.error(`Failed to monitor changes: ${err.message}`);
    ...
    step(
      ...
      function(err)
      {
        if (err)
        {
          ...
          {
            logger.error(`Failed to download file [${fileId}]: ${err.message}`);
          }
        }
        else
        {
  ...
  function removeOldVersions(root)
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          logger.error(`Failed to remove old versions: ${err.message}`);
        }
      }
    );
  }
