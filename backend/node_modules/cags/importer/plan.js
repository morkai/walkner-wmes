...
exports.start = (app, module) =>
{
  ...
  function importNext()
  {
    ...
    importFile(fileInfo, function(err, count)
    {
      ...
      if (err)
      {
        module.error(err, 'Failed to import.');
      ...
      {
        module.info('Imported.', {count, duration: Date.now() - startTime});
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      {
        module.info('Parsing...', {filePath: fileInfo.filePath});
      ...
      function updateDbStep(err, cagPlans)
      {
        ...
        module.info('Parsed!', {count: cagPlans.length, duration: Date.now() - this.t});
  ...
  function updateCagPlansBatch(cagPlans, done)
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to upsert CagPlans.`);
        }
  ...
  {
    const docs = _.map(cags, (name, id) => ({_id: id, name: name}));
    ...
    Cag.collection.insertMany(docs, {ordered: false}, function(err)
    {
      ...
      {
        module.error(err, `Failed to create CAGs.`);
      }
    });
  }
