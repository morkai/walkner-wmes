...
module.exports = function setUpUsersRoutes(app, usersModule)
{
  ...
  express.get('/users/self', selfRoute);
  ...
  function canEdit(req, res, next)
  {
    ...
    {
      if (!user.privileges
        || (!user.privileges.includes('USERS:MANAGE') && !user.privileges.includes('SUPER')))
      {
      ...
      function sendResponseStep(err)
      {
        ...
        if (this.passwordResetRequest)
        {
          ...
          passwordResetRequest.remove(err =>
          {
            if (err)
            {
              usersModule.error(err, 'Failed to remove the password reset request.', {
  ...
  function anonymizeNextModel(userId, modelQueue, modelsToUpdate, done)
  {
    ...
    usersModule.info(`[anonymize] [${userId}] ${modelName}...`);
