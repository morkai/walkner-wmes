...
module.exports = async (app, module, options) =>
{
  ...
  function prepareMatchers(done)
  {
    ...
    step(
      ...
      function(err, divisions)
      {
        ...
        {
          const data = {
            oshWorkplace: division.workplace,
            oshDivision: division._id
          };

          for (const syncPattern of division.syncPatterns.split('\n'))
          {
            const matcher = createMatcher(syncPattern, data);

            if (matcher)
            {
              matchers.divisions.push(matcher);
            }
          }
        }
  ...
  function prepareNewUsers(conn, matchers, done)
  {
    if (app.options.env === 'development')
    {
      try
      {
        const newUsers = JSON.parse(require('fs').readFileSync(`${app.options.rootPath}/../pila-users.json`, 'utf8'));

        newUsers.forEach(newUser =>
        {
          applyMatcher(matchers.divisions, newUser.syncData.workplace, newUser);
        });
    ...
    req.on('row', row =>
    {
      ...
      const newUser = {
        ...
        syncData: {
          ...
          division: str(row[5].value),
          jobTitle: str(row[6].value)
        }
      };

      applyMatcher(matchers.divisions, newUser.syncData.division, newUser);
