...
module.exports = (app, module) =>
{
  ...
  function canEdit(req, res, next)
  {
    ...
    const EDITABLE_PROPERTIES = user.isAllowedTo(sessionUser, 'USERS:MANAGE')
      ? User.EDITABLE_PROPERTIES
      : User.SELF_EDITABLE_PROPERTIES;

    req.body = _.pick(req.body, Object.keys(EDITABLE_PROPERTIES));

    if (!sessionUser || req.params.id !== sessionUser._id)
    {
      return canManage(req, res, next);
    }
  ...
  function checkLogin(req, res, next)
  {
    ...
    User.findOne(conditions).select({_id: 1}).lean().exec((err, userModel) =>
    {
      if (err)
      {
        return next(err);
  ...
  function prepareProdOrgUnitFilter(req, res, next)
  {
    ...
    {
      if (orgUnitIds[term.args[0]] && (term.name === 'eq' || term.name === 'in'))
      {
        if (Array.isArray(term.args[1]))
        {
          term.args[1].forEach(id => orgUnitIds[term.args[0]].add(id));
        }
        else
        {
          orgUnitIds[term.args[0]].add(term.args[1]);
        }
    ...
    orgUnitIds.division.forEach(divisionId =>
    {
      orgUnits.getSubdivisionsFor('division', divisionId).forEach(subdivision =>
      {
        orgUnitIds.subdivision.add(subdivision._id.toString());
      });
    });

    orgUnitIds.subdivision.forEach(subdivisionId =>
    {
      const division = orgUnits.getDivisionFor('subdivision', subdivisionId);

      if (division)
      {
        orgUnitIds.division.add(division._id);
      }
    });

    if (orgUnitIds.division.size || orgUnitIds.subdivision.size)
    {
      req.rql.selector.args.push({
        name: 'in',
        args: [
          'orgUnitId',
          Array.from(orgUnitIds.division).concat(Array.from(orgUnitIds.subdivision))
        ]
      });
    }
