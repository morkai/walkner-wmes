...
module.exports = function syncUsers(app, usersModule, done)
{
  ...
  const logger = usersModule.logger.create({submodule: 'sync'});
  ...
  {
    logger.error(err, 'Tedious error.');
  });
  ...
  function syncNextUser(stats, kdUsers, kdUserIndex)
  {
    ...
    step(
      ...
      function prepareUserModelStep(err, kdIdUser, loginUser)
      {
        if (err)
        {
          logger.error(err, 'Failed to find a user by KD ID.', {kdId: kdUser.kdId});
  ...
  function saveUserModel(userModel, isNew, isRetry, stats, done)
  {
    ...
    userModel.save(function(err)
    {
      if (err)
      {
        logger.error(err, 'Failed to save a user.', {kdId: userModel.kdId});
