...
exports.start = (app, module) =>
{
  ...
  function checkMemoryUsage()
  {
    ...
    const notify = memoryUsage.rss > module.config.notifyThreshold
      && now - module.notifyTime > 3600 * 4 * 1000;
    const dump = memoryUsage.heapTotal > module.config.dumpThreshold
    ...
    {
      module[dump ? 'warn' : notify ? 'info' : 'debug']({
        memoryUsage: {
          now: memoryUsage,
          last: module.last,
          diff: module.diff,
  ...
  function dumpMemoryUsage(done)
  {
    ...
    heapdump.writeSnapshot((err, file) =>
    {
      if (err)
      {
        module.error(err, `Failed to dump the heap.`);
      }
      else
      {
        module.info(`Heap dump written to: ${process.cwd()}/${file}`);
      }
