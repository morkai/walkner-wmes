...
module.exports = function setUpPing(app, watchdogModule)
{
  ...
  function ping()
  {
    step(
      ...
      function handlePingResponseStep(err, res, body)
      {
        if (err)
        {
          watchdogModule.info('[ping] Failed local request: %s', err.message);
  ...
  function notify()
  {
    watchdogModule.info('[ping] Service unavailable: notifying!');
  ...
  function mail(subject, text)
  {
    ...
    mailSender.send(to, subject, text.join('\r\n'), function(err)
    {
      ...
      {
        watchdogModule.info('[ping] [mail] Notified: %s', to);
      }
    });
  }
  ...
  function call(text)
  {
    ...
    twilio.say(sayOptions, function(err)
    {
      ...
      {
        watchdogModule.info('[ping] [call] Notified: %s', sayOptions.to);
      }
    });
  }
};
