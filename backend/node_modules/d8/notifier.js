...
{
  const {mailSender, mongoose, User, D8Entry} = module;
  ...
  app.broker.subscribe(D8Entry.TOPIC_PREFIX + '.added', (message) =>
  {
  ...
  app.broker.subscribe(D8Entry.TOPIC_PREFIX + '.edited', (message) =>
  {
  ...
  function notifyAboutAdd(entry)
  {
    ...
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          logger.error(err, `Failed to notify.`, {rid: entry.rid});
        }
        else if (this.mailOptions)
        {
          logger.info(`Notified.`, {count: this.mailOptions.to.length, rid: entry.rid});
        }
  ...
  function notifyAboutEdit(entry, usersToNotify)
  {
    ...
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          module.error(err, `Failed to notify.`, {rid: entry.rid});
        }
        else if (this.mailOptions)
        {
          module.info(`Notified.`, {count: this.mailOptions.to.length, rid: entry.rid});
        }
