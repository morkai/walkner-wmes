...
module.exports = (app, module) =>
{
  ...
  const {mailSender, User, D8Entry} = module;
  ...
  function remind()
  {
    step(
      function checkBusinessDayStep()
      {
        ...
        {
          logger.info(`Skipping a non-business day.`);
      ...
      function getUserDataStep(err, entries)
      {
        ...
        logger.info(`Found ${entries.length} open entries.`);
        ...
        {
          _.forEach(entry.observers, (observer) =>
          {
      ...
      function sendEmailsStep(err, users)
      {
        ...
        logger.info(`Sending reminders for ${this.entries.length} entries.`);
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to remind.`);
        }
  ...
  function sendReminderEmail(entry, emails)
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to remind.`, {recipients: emails});
        }
      }
    );
  }
