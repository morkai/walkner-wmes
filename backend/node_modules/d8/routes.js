...
{
  const {express, user, mongoose, D8Entry} = module;

  const tmpAttachments = {};

  const canViewDictionaries = user.auth('D8:DICTIONARIES:VIEW');
  const canManageDictionaries = user.auth('D8:DICTIONARIES:VIEW');
  const canView = user.auth('D8:VIEW');
  const canDownload = user.auth('LOCAL', 'D8:VIEW');
  const canManage = user.auth('D8:MANAGE');
  ...
  {
    D8Entry.findById(req.params.id, (err, entry) =>
    {
      if (err)
      {
        return next(err);
      ...
      if (user.isAllowedTo(userData, 'D8:MANAGE')
        || (entry.creator.id === userData._id && moment(entry.createdAt).diff(Date.now()) >= -10))
      {
        req.model = entry;
  ...
  function findByRidRoute(req, res, next)
  {
    ...
    D8Entry.findOne({rid: rid}, {_id: 1}).lean().exec((err, model) =>
    {
      if (err)
      {
        return next(err);
  ...
  {
    D8Entry.findOne({rid: parseInt(req.params.rid, 10)}, {_id: 1}).lean().exec((err, d8Entry) =>
    {
      if (err)
      {
        return next(err);
      ...
      if (d8Entry)
      {
        return res.redirect('/#d8/entries/' + d8Entry._id);
      }
  ...
  {
    const userData = req.session.user;
    const updater = user.createUserInfo(userData, req);
    let body = req.body;
    ...
    step(
      ...
      function applyChangesStep(err, entry)
      {
        ...
        if (!user.isAllowedTo(userData, 'D8:MANAGE'))
        {
          body = _.pick(body, getPropsForEdit(entry, entry.getUserRoles(userData)));
        }
  ...
  {
    D8Entry.findById(req.params.entry, {attachment: 1, changes: 1}).lean().exec((err, entry) =>
    {
      if (err)
      {
        return next(err);
