...
module.exports = function createParser(app, importerModule, callback)
{
  ...
  function importFile(fileInfo)
  {
    ...
    importerModule.info(`Handling ${fileInfo.type}...`, {key: timeKey});
  ...
  function enqueueAndParse(timeKey, delayed)
  {
    ...
    if (delayed)
    {
      importerModule.info('Queued (delayed)...', {key: timeKey});
    }
    else
    {
      importerModule.info('Queued...', {key: timeKey});
    }
  ...
  function parseData()
  {
    ...
    importerModule.info('Parsing...', {key: timeKey});
    ...
    parseOrdersAndOperations(timeKeyFileInfo.orders, timeKeyFileInfo.operations, function(err, orders, missingOrders)
    {
      ...
      if (!err)
      {
        importerModule.info('Parsed!', {key: timeKey, duration: Date.now() - startTime});
      }
  ...
  function parseOrdersAndOperations(ordersFileInfo, operationsFileInfo, done)
  {
    ...
    step(
      ...
      function parseOrdersStep(err, ordersFileContents)
      {
        if (err)
        {
          importerModule.error(err, 'Failed to read the orders input file.');
        }
      ...
      function parseOperationsStep(err, operationsFileContents)
      {
        if (err)
        {
          importerModule.error(err, 'Failed to read the operations input file.');
        }
  ...
  function moveFileInfoStepFile(oldFilePath)
  {
    ...
    fs.move(oldFilePath, newFilePath, {overwrite: true}, function(err)
    {
      if (err)
      {
        importerModule.error(err, 'Failed to rename file.', {oldFilePath, newFilePath});
      }
    });
  }
  ...
  function deleteFileInfoStepFile(filePath)
  {
    fs.unlink(filePath, function(err)
    {
      if (err)
      {
        importerModule.error(err, 'Failed to delete file.', {filePath});
      }
    });
  }
