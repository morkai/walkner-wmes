...
module.exports = function createParser(app, importerModule, callback)
{
  ...
  function importFile(fileInfo)
  {
    ...
    importerModule.info('Handling %s for %s...', fileInfo.type, timeKey);
  ...
  function enqueueAndParse(timeKey, delayed)
  {
    ...
    if (delayed)
    {
      importerModule.info('Queued %s (delayed)...', timeKey);
    }
    else
    {
      importerModule.info('Queued %s...', timeKey);
    }
  ...
  function parseData()
  {
    ...
    importerModule.info('Parsing %s...', timeKey);
    ...
    parseOrdersAndOperations(timeKeyFileInfo.orders, timeKeyFileInfo.operations, function(err, orders, missingOrders)
    {
      ...
      if (!err)
      {
        importerModule.info('Parsed %s in %d ms!', timeKey, Date.now() - startTime);
      }
