...
{
  const {Order, Component, Setting, XiconfOrder} = module;
  ...
  function parseXiconfPatterns(setting)
  {
    ...
    setting.value.split('\n').forEach(line =>
    {
      ...
      try
      {
        ...
        patterns[type].push(new RegExp(pattern, 'i'));
      }
  ...
  function matchXiconfComponent(order, component, orderNoToComponentMap, patterns)
  {
    ...
    if (patterns.exclude.some(pattern => pattern.test(component.name)))
    {
      return;
    }

    if (!patterns.include.some(pattern => pattern.test(component.name)))
    {
      return;
    }
