...
exports.start = (app, module) =>
{
  ...
  function compareNext()
  {
    ...
    module.info(`Comparing ${orderIds.length} orders and ${missingOrderIds.length} missing orders...`);
    ...
    step(
      ...
      function findOrdersStep(err)
      {
        if (err)
        {
          module.error(`Failed to update ETO: ${err.message}`);
        }

        Order
          .find({_id: {$in: allOrderIds}})
          .select({documents: 0, bom: 0, changes: 0})
          .lean()
          .exec(this.next());
      },
...
console.log('new');
console.log(newEtoContinuations);
