...
exports.start = (app, module) =>
{
  ...
  )
  {
    ...
    {
      if (Order.IGNORED_IMPORT_PROPS[key] === 1)
      {
        return;
      }

      if (Order.IGNORED_IMPORT_PROPS[key] === 0 && _.isEmpty(newValue))
      {
        return;
      }
  ...
  async function updateCompletedWhOrders(completedOrders)
  {
    ...
    try
    {
      const cancelQueue = await WhOrder
        .aggregate([
          {$match: {order: {$in: completedOrders}}},
          {$group: {_id: '$order'}}
        ])
        .exec();
