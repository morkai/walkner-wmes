...
exports.start = (app, module) =>
{
  ...
  const COMPLETED_ORDER_STATUSES = ['CNF', 'DLV', 'TECO', 'DLT', 'DLFL'];
  ...
  )
  {
    ...
    if (different)
    {
      ...
      {
        const oldStatus = _.intersection(changes.oldValues.statuses, COMPLETED_ORDER_STATUSES);
        const newStatus = _.intersection(changes.newValues.statuses, COMPLETED_ORDER_STATUSES);
  ...
  async function updateCompletedWhOrders(completedOrders)
  {
    ...
    module.debug('Cancelling completed WH orders...', {orderCount: completedOrders.length});

    try
    {
      const whOrders = await WhOrder
        .find({
          status: {$in: ['pending', 'problem']},
          order: {$in: completedOrders}
        })
        .select({_id: 1, order: 1, date: 1, set: 1})
        .lean()
        .hint({order: 1})
        .exec();
      ...
      whOrders.forEach(whOrder =>
      {
        ...
        whOrder.status = 'cancelled';
      });

      const [eventsResult] = Promise.all([
        WhEvent.record({
        ...
        WhOrder.collection.updateMany({_id: {$in: whOrderIds}}, {$set: {status: 'cancelled'}})
      ]);

      app.broker.publish(`${WhOrder.TOPIC_PREFIX}.updated`, {
        updated: whOrders
      });

      eventsResult.messages.forEach(({topic, message}) => app.broker.publish(topic, message));

      module.info('Cancelled completed WH orders.', {orders: Array.from(sapOrderIds)});
    }
    catch (err)
    {
      module.error(err, 'Failed to cancel completed WH orders.');
    }
  }
};
