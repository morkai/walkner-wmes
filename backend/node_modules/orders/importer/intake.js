...
exports.recordTopics = {
  info: ['orders.intake.synced'],
  error: ['orders.intake.syncFailed']
};

exports.start = (app, module) =>
{
  const {Order, OrderIntake} = module;
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      function parseFileStep(err, fileContents)
      {
        ...
        module.info('Parsed!', {fileInfo, count: this.orderIntakes.length, duration: Date.now() - t});

        setImmediate(this.next());
      },
      function upsertOrderIntakesStep()
      {
        module.info('Upserting order intakes...', {fileInfo});
        ...
        function upsertNextOrderIntakesBatch(orderIntakes, batchNo, batchSize, done)
        {
          ...
          step(
            ...
            function(err)
            {
              if (err)
              {
                module.error(err, 'Failed to upsert order intakes.');
              }

              if (endIndex === orderIntakes.length)
              {
                module.info('Upserted!', {fileInfo, count: orderIntakes.length, duration: Date.now() - t});
      ...
      {
        module.info('Updating orders...', {fileInfo});
  ...
  function upsertNextOrdersBatch(fileInfo, t, orderIntakes, batchNo, batchSize, done)
  {
    ...
    step(
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, 'Failed to update orders.', {fileInfo});
        }

        if (endIndex === orderIntakes.length)
        {
          module.info('Updated orders!', {fileInfo, duration: Date.now() - t.getTime()});
