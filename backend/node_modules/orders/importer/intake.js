...
exports.start = function startOrderIntakeImporterModule(app, module)
{
  ...
  function queueFile(fileInfo)
  {
    ...
    module.info('[%s] Queued...', fileInfo.timeKey);
  ...
  function importNext()
  {
    ...
    module.info('[%s] Importing...', fileInfo.timeKey);
    ...
    importFile(fileInfo, function(err, count)
    {
      ...
      {
        module.info('[%s] Imported %d in %d ms', fileInfo.timeKey, count, Date.now() - startTime);
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      function parseFileStep(err, fileContents)
      {
        ...
        module.info('[%s] Parsing ~%d bytes...', fileInfo.timeKey, fileContents.length);

        const t = Date.now();

        this.orderIntakes = parseOrderIntakeTable(fileContents);

        module.info('[%s] Parsed %d items in %d ms!', fileInfo.timeKey, this.orderIntakes.length, Date.now() - t);

        setImmediate(this.next());
      },
      function upsertOrderIntakesStep()
      {
        module.info('[%s] Upserting order intakes...', fileInfo.timeKey);
      ...
      {
        module.info('[%s] Updating orders...', fileInfo.timeKey);
  ...
  function upsertNextOrdersBatch(fileInfo, t, orderIntakes, batchNo, batchSize, done)
  {
    ...
    step(
      ...
      function(err)
      {
        ...
        if (endIndex === orderIntakes.length)
        {
          module.info('[%s] Updated orders in %d ms!', fileInfo.timeKey, Date.now() - t.getTime());
