...
module.exports = (app, module) =>
{
    ...
    step(
      ...
      function(err, drillingOrder)
      {
        ...
        Order
          ...
          .select({_id: 1, leadingOrder: 1, drillStatus: 1})
          .lean()
          .exec(this.next());
      },
      ...
      function(err, drillingOrders)
      {
        ...
        if (!drillingOrders.length || !this.leadingSapOrder)
        {
          return;
        }
        ...
        else if (statuses.aside === drillingOrders.length)
        {
          newStatus = 'aside';
        }
        else if (statuses.finished)
        {
          newStatus = (statuses.finished + statuses.cancelled) === drillingOrders.length ? 'finished' : 'partial';
        }
        else if (statuses.aside)
        {
          newStatus = 'aside';
        }
        else if (statuses.cancelled === drillingOrders.length)
        {
          newStatus = 'cancelled';
        }
  ...
  {
    const changedDrillingOrders = [];

    changes.added.forEach(o => changedDrillingOrders.push(o._id));
    changes.changed.forEach(o => changedDrillingOrders.push(o._id));
    ...
    updateNextDrillStatus(drillingEvent, changes.changed.concat(changes.added));
  }
