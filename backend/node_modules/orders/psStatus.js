...
module.exports = (app, module) =>
{
  if (module.config.syncPsStatus.updater)
  {
    ...
    // app.broker.subscribe('orders.leadingOrdersChanged', onLeadingOrdersChanged);
  }
  ...
  function updatePsStatus(paintShopEvent, done)
  {
    ...
    step(
      ...
      function(err, paintShopOrder)
      {
        ...
        if (!paintShopOrder)
        {
          if (!paintShopEvent.data.sapOrder)
          {
            return this.skip(app.createError(`Paint-shop order not found.`, {
              psOrder: paintShopEvent.order,
              sapOrder: paintShopEvent.data.sapOrder
            }));
          }

          paintShopOrder = {
            _id: paintShopEvent.order,
            status: 'unknown',
            order: paintShopEvent.data.sapOrder,
            qty: 0,
            qtyDone: 0,
            qtyDlv: 0
          };
  ...
  function onPaintShopChanged({changes})
  {
    ...
    const psOrderQueue = [].concat(
      changes.removed,
      changes.added,
      changes.changed.filter(change => Object.keys(change).filter(k => !reorderProps[k]).length > 0)
    );

    updateNextPsStatus(psEvent, psOrderQueue);
  }

  // TODO Remove if there're no PS status errors
  function onLeadingOrdersChanged({leadingOrders}) // eslint-disable-line no-unused-vars
  {
