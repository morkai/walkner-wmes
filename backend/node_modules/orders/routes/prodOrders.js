...
module.exports = (app, module) =>
{
  ...
  express.get('/orders;incomplete', canView, incompleteRoute);
  express.post('/orders;missing', canView, missingRoute);
    ...
    prepareResult: (model, done) =>
    {
      step(
        function()
        {
          ...
          Order.assignChangeNumbers(model, this.parallel());
          Order.assignNotes(model, null, this.parallel());
          Order.assignCompRelEntries(model, this.parallel());
        },
        function(err)
        {
          done(err, model);
  ...
  function exportOrder(doc, req)
  {
    ...
    return {
      ...
      salesOrderDate: doc.salesOrderDate,
  ...
  async function incompleteRoute(req, res)
  {
    const fromMoment = moment().startOf('day');

    for (let days = 0; days < 2;)
    {
      if (businessDays.countInDay(fromMoment.toDate()))
      {
        days += 1;
      }
    ...
    const psos = await ProdShiftOrder
      .aggregate([
        {$match: {
          finishedAt: {$gte: fromMoment.toDate()},
          mechOrder: false
        }},
        {$group: {_id: '$orderId'}}
      ])
    ...
    const orders = await Order
      .aggregate([
        {$match: {
          _id: {$in: psos.map(pso => pso._id)},
          $and: [
            {statuses: {$nin: ['TECO', 'DLT', 'DLFL']}},
            {statuses: {$not: {$all: ['CNF', 'DLV']}}}
          ]
        }},
        {$project: {_id: '$_id'}}
      ])
    ...
    res.json({orders: orders.map(o => o._id)});
  }

  async function missingRoute(req, res, next)
  {
    const {orders} = req.body;

    if (!Array.isArray(orders) || !orders.length)
    {
      return next(app.createError('Invalid orders.', 'INPUT', 400));
    }
    ...
    const validLists = ['operations', 'bom', 'documents'];

    if (!validLists.includes(list))
    {
      return next(app.createError('Invalid list.', 'INPUT', 400));
    }
    ...
    const result = await Order.aggregate([
      {$match: {
        _id: {$in: orders},
        [`${list}.0`]: {$exists: false},
        statuses: {$nin: Order.COMPLETED_STATUSES}
      }},
      {$group: {
        _id: null,
        orders: {$addToSet: '$_id'}
