...
module.exports = (app, ordersModule, req, res, next) =>
{
  ...
  if (!orderNos.length)
  {
    return next(app.createError('INVALID_ORDER_NO'));
  }
  ...
  function prepareOrderTemplateData(extraRows, order, done)
  {
    ...
    step(
      ...
      function assignBarcodesStep(err, qrCode)
      {
        if (err)
        {
          ordersModule.error(err, 'Failed to generate barcodes.');
        }
...
{
  const cmd = `"${zintExe}" --barcode=58 --vers=5 --scale=1 --notext --directpng --data="${data}"`;
