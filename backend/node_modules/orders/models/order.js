...
exports.setUp = (app, mongoose) =>
{
  ...
  schema.statics.TOPIC_PREFIX = 'orders';
  schema.statics.BROWSE_LIMIT = 1000;
  schema.statics.COMPLETED_STATUSES = ['CNF', 'DLV', 'TECO', 'DLT', 'DLFL', 'DEL'];
  schema.statics.DELETED_STATUSES = ['TECO', 'DLT', 'DLFL', 'DEL'];

  schema.index({scheduledStartDate: -1});
  schema.index({scheduledFinishDate: -1});
  schema.index({startDate: -1});
  schema.index({finishDate: -1});
  schema.index({nc12: 1, scheduledStartDate: -1});
  schema.index({mrp: 1, scheduledStartDate: -1});
  schema.index({delayReason: 1, scheduledStartDate: -1});
  schema.index({salesOrder: 1, salesOrderItem: 1});
  schema.index({leadingOrder: 1});
  schema.index({'bom.nc12': 1});
  schema.index({'documents.nc15': 1});
  schema.index({'operations.workCenter': 1, scheduledStartDate: -1});

  schema.statics.IGNORED_IMPORT_PROPS = {
    createdAt: 1,
    updatedAt: 1,
    tzOffsetMs: 1,
    statusesSetAt: 1,
  ...
  schema.statics.createForInsert = function(obj, importTs)
  {
  ...
  schema.statics.createMissingForInsert = function(missingOrder, createdAt)
  {
    return {
  ...
  schema.statics.prepareForInsert = function(order, createdAt)
  {
    order.createdAt = createdAt;
    order.tzOffsetMs = (order.startDate ? order.startDate.getTimezoneOffset() : 0) * 60 * 1000 * -1;
    order.changes = [];

    schema.statics.resetStatusesSetAt(order);

    return order;
  };

  schema.statics.prepareForUpdate = function(orderModel, newOrderData, updatedAt, change, $set)
  {
    ...
    if ($set.statuses)
    {
      $set.statusesSetAt = schema.statics.prepareStatusesSetAt(
  ...
  schema.statics.copyOrderIntake = function(order, orderIntake)
  {
    order.description = orderIntake.description;
    order.soldToParty = orderIntake.soldToParty;
    order.salesOrderDate = orderIntake.salesOrderDate;
  };

  schema.statics.resetStatusesSetAt = function(order)
  {
  ...
  schema.statics.prepareStatusesSetAt = function(oldStatusesSetAt, newStatusSetAt, newStatuses)
  {
  ...
  schema.statics.recountQtyDone = function(orderNo, done)
  {
    const qtyDone = {
    ...
    step(
      ...
      function(err, psos)
      {
        ...
        mongoose.model('Order').collection.findOneAndUpdate(filter, update, options, this.next());
      },
      function(err, result)
      {
        if (result && result.value)
        {
          app.broker.publish(`orders.quantityDone.${orderNo}`, result.value);
        }
  ...
  schema.statics.assignPkhdStrategies = function(order, done)
  {
  ...
  schema.statics.assignChangeNumbers = function(order, done)
  {
    if (!Array.isArray(order.bom) || !order.bom.length)
    {
      return done(null, order);
    ...
    order.bom.forEach(component =>
    {
      ...
      if (!cnToComp.has(component.cn))
      {
        cnToComp.set(component.cn, []);
      }

      cnToComp.get(component.cn).push(component);
    });
    ...
    step(
      function()
      {
        mongoose.model('ChangeNumber')
          .find({
            _id: {$in: Array.from(cnToComp.keys())}
          })
          .select({
            validFrom: 1
          })
      ...
      function(err, changeNumbers)
      {
        ...
        changeNumbers.forEach(cn =>
        {
          cnToComp.get(cn._id).forEach(component =>
          {
            component.validFrom = cn.validFrom;
          });
        });
      },
      function(err)
  ...
  schema.statics.assignNotes = function(order, target, done)
  {
  ...
  schema.statics.assignCompRelEntries = function(order, done)
  {
  ...
  schema.statics.assignStationComponents = function(components, line, station, done)
  {
    ...
    {
      if (!component.supplyArea)
      {
        return;
      }

      supplyAreas.add(component.supplyArea);
      bom.set(component.nc12, component);
    });
    ...
    step(
      function()
      {
        KanbanEntry
          .find({
            nc12: {$in: Array.from(bom.keys())},
            supplyArea: {$in: Array.from(supplyAreas)},
            [`workstations.${station - 1}`]: {$gt: 0}
          })
          .select({
            nc12: 1
          })
      ...
      function(err, kanbanEntries)
      {
        ...
        kanbanEntries.forEach(({nc12}) =>
        {
          bom.get(nc12).usedOnStation = true;
        });
