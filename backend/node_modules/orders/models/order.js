...
exports.setUp = (app, mongoose) =>
{
  ...
  orderSchema.statics.assignNotes = function(order, done)
  {
    ...
    step(
      function()
      {
        ...
        mongoose.model('ProductNote')
          .find({nc12: order.nc12})
          .select({text: 1})
          .lean()
          .exec(this.next());
      },
      function(err, productNotes)
      {
        ...
        if (Array.isArray(order.bom))
        {
          order.bom.forEach(component =>
          {
            if (!component.nc12)
            {
              order.notes.push(component.name);
            }
          });
        }
