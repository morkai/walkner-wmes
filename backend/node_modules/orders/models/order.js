...
exports.setUp = (app, mongoose) =>
{
  ...
  orderSchema.statics.assignMessages = function(order, done)
  {
    ...
    order.messages = order.messages.map(text =>
    {
      return {
        _id: null,
        source: 'order',
    ...
    mongoose.model('ProductMessage').find({nc12: order.nc12}).select({text: 1}).lean().exec((err, productMessages) =>
    {
      if (err)
      {
        return done(err);
      ...
      productMessages.forEach((_id, text) =>
      {
