...
exports.start = (app, module) =>
{
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      function findSplitEntriesStep()
      {
        KanbanEntry
          .find({split: {$gt: 1}})
          .select({_id: 1, split: 1})
          .lean()
          .exec(this.next());
      },
      function splitEntriesStep(err, kanbanEntries)
      {
        if (err)
        {
          return this.skip(err);
        ...
        const splitKanbanEntries = new Map();
        const rows = [];

        kanbanEntries.forEach(kanbanEntry =>
        {
          splitKanbanEntries.set(kanbanEntry._id, kanbanEntry.split);
        });

        const idI = this.input.kanbans.columns.indexOf('_id');
        const kanbanIdI = this.input.kanbans.columns.indexOf('kanbanId');

        this.input.kanbans.rows.forEach(mainRow =>
        {
          rows.push(mainRow);

          const splitCount = splitKanbanEntries.get(mainRow[idI]);
          ...
          const remainingKanbanIds = mainRow[kanbanIdI];
          const perEntryCount = Math.ceil(remainingKanbanIds.length / splitCount);
          const mainKanbanIds = remainingKanbanIds.splice(0, perEntryCount);

          mainRow[kanbanIdI] = null;

          for (let i = 1; i < splitCount; ++i)
          {
            const dupRow = Array.prototype.slice.call(mainRow);

            dupRow[idI] += (i + 9).toString(36);
            dupRow[kanbanIdI] = remainingKanbanIds.splice(0, perEntryCount);

            rows.push(dupRow);
          }

          mainRow[kanbanIdI] = mainKanbanIds;
        });

        this.input.kanbans.rows = rows;
      },
  ...
  function importDocs(Model, input, updatedAt, done)
  {
    ...
    step(
      function()
      {
        ...
        Model
          .find({_id: {$in: Array.from(this.newDocs.keys())}})
          .select({changes: 0})
          .lean()
          .exec(this.next());
      },
  ...
  function updateNextComponentUsage(remaining, componentToUsage, done)
  {
    ...
    KanbanComponent.collection.updateOne({_id: nc12}, {$set: {usage}}, err =>
    {
      if (err)
      {
        module.error(err, 'Failed to update component usage.', {
