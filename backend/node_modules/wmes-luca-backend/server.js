...
module.exports = (app, module) =>
{
  ...
  setInterval(() => lineOrderCache.clear(), 24 * 3600 * 1000);
  ...
  {
    lineOrderCache.clear();
  }

  function handleSetLineOrder({lineId, orderNo}, done)
  {
    if (lineOrderCache.has(lineId))
    {
      lineOrderCache.get(lineId).delete(orderNo);
    }
  ...
  function getLineOrder(lineId, orderNo, done)
  {
    if (!lineOrderCache.has(lineId))
    {
      lineOrderCache.set(lineId, new Map());
    }

    const orderCache = lineOrderCache.get(lineId);

    if (orderCache.has(orderNo))
    {
      return setImmediate(done, null, orderCache.get(orderNo));
    }
    ...
    step(
      ...
      function(err, pso)
      {
        ...
        this.cycleTime = pso.sapTaktTime > 0
          ? pso.sapTaktTime
          : Math.ceil((pso.laborTime / 100 * 3600) / pso.workerCount);
      ...
      function(err, sapOrder)
      {
        ...
        if (!this.cycleTime)
        {
          const operation = resolveBestOperation(sapOrder.operations);

          if (operation)
          {
            const workerCount = operation.laborTime / operation.machineTime;

            this.cycleTime = Math.ceil((operation.laborTime / 100 * 3600) / workerCount);
          }
        }
        ...
        const result = {
          ...
          order: {
            ...
            nc12: sapOrder.nc12,
            name: sapOrder.name,
            description: sapOrder.description,
        ...
        sapOrder.bom.forEach(component =>
        {
          ...
          components.set(component.nc12, component);
        });

        setImmediate(this.parallel(), null, result);
        resolveKanbanComponents(result, components, this.parallel());
      },
      function(err, wmesResult)
      {
        ...
        if (!wmesResult)
        {
          return done(err, orderCache.get(orderNo));
        }

        const lucaResult = prepareLucaResult(wmesResult);

        orderCache.set(wmesResult.order.no, lucaResult);
  ...
  function prepareLucaResult(wmesResult)
  {
    const lucaResult = {
      ...
      description: wmesResult.order.description,
      name: wmesResult.order.name,
      orderNo: wmesResult.order.no,
      type: /[A-Z]/.test(wmesResult.order.nc12) ? 'CONFIG' : 'PRODUCT',
      cycleTime: wmesResult.order.cycleTime,
      components: []
    };
