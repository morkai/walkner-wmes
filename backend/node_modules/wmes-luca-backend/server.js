...
module.exports = (app, module) =>
{
  ...
  function onAppStarted()
  {
    settings.findById('luca.lines', (err, setting) =>
    {
      ...
      if (!Array.isArray(setting) || !setting.value.length)
      {
        return;
      }

      setting.value.forEach(lineId => setLineOrder(lineId, null, _.noop));
    });
  }

  function onLineOrderChanged({lineId})
  {
    setLineOrder(lineId, null, _.noop);
  }
  ...
  function getLineOrder(lineId, orderNo, done)
  {
    ...
    step(
      ...
      function(err, result)
      {
        if (!err && result)
        {
          orderCache.set(`${result.line}:${result.order.no}`, result);
        }
