...
module.exports = async (app, module, job, output, updatedBy) =>
{
  ...
  if (ordersOps.length)
  {
    module.debug('dpBom found dummy codes.', {jobId: job._id, count: ordersOps.length});
  ...
  {
    module.debug('dpBom did not find any dummy codes.', {jobId: job._id});
  }
  ...
  if (!result.length)
  {
    module.debug('dpBom did not find any DP orders?!', {jobId: job._id});
  ...
  module.debug('dpBom starting dpSo.', {jobId: job._id, count: leadingOrders.length});
