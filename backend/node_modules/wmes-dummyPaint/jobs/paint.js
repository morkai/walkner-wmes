...
module.exports = async (app, module, job, updatedBy) =>
{
  ...
  module.debug('Resolving paint for DP orders.', {jobId: job._id, count: orders.length});
  ...
  if (!items.length)
  {
    module.debug('No items to update after paints were resolved.', {jobId: job._id});
  ...
  module.debug('Starting dpUpdate after paints were resolved.', {jobId: job._id, count: items.length});
