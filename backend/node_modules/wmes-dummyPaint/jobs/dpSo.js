...
module.exports = async (app, module, job, output, updatedBy) =>
{
  ...
  {
    module.debug('dpSo found no sales order data.', {jobId: job._id});
  ...
  const leadingNos = Object.keys(leadingOrders);

  module.debug('dpSo found sales orders for leading orders.', {jobId: job._id, count: leadingNos.length});

  const orders = await DpOrder
    .find({
      leadingNo: {$in: leadingNos},
      job: job._id
    })
    .select({
      dummyNc12: 1,
  ...
  {
    module.debug('dpSo found no DP orders with sales orders.', {jobId: job._id});

    return await module.jobs.finishJob(job._id, updatedBy);
  }

  module.debug('dpSo starting dpCh.', {jobId: job._id, count: salesOrders.length});
