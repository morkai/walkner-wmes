...
module.exports = async (app, module, job, output, updatedBy) =>
{
  ...
  {
    module.debug('dpCh found no data.', {jobId: job._id});
  ...
  const salesNos = Object.keys(salesOrders);

  module.debug('dpCh found characteristics for sales orders.', {jobId: job._id, count: salesNos.length});
  ...
  {
    const {error: noError, items} = salesOrders[order.salesNo];
    ...
    const {error: itemError, config, override} = items[order.salesItem] || {};
  ...
  if (result.length === 1 && result[0].nc12s.length)
  {
    const {nc12s} = result[0];

    module.debug('dpCh starting dpClr because of DP orders with no paint code.', {jobId: job._id, count: nc12s.length});
  ...
  module.debug('dpCh going straight to paint.', {jobId: job._id});
