...
{
  const logger = module.logger.create({submodule: 'omr'});
  ...
  function queueInputDir(fileInfo)
  {
    ...
    step(
      ...
      function(err, stats)
      {
        if (err)
        {
          return logger.error(err, 'Failed to stat input dir.', {fileInfo});
        }
        ...
        if (stats.isDirectory())
        {
          ...
          logger.info('Queued a new input dir.', {fileInfo});
  ...
  function processNextInputDir()
  {
    ...
    step(
      ...
      {
        logger.info('Waiting for input dir to calm down.', {fileInfo: currentInputDir});
      ...
      function readDirStep(err)
      {
        ...
        logger.info('Reading input dir.', {fileInfo: currentInputDir});
      ...
      {
        logger.info('Reading files remaining in input dir.', {fileInfo: currentInputDir});
      ...
      function handleRemainingFilesStep(err, files)
      {
        ...
        if (err || files.length)
        {
          logger.info('Moving files remaining in input dir.', {fileInfo: currentInputDir});
        ...
        {
          logger.info('Removing input dir.', {fileInfo: currentInputDir});
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          logger.error(err, 'Failed to process input dir.', {fileInfo: currentInputDir});
        }
        else
        {
          logger.info('Finished processing input dir.', {
            fileInfo: currentInputDir,
  ...
  function moveDir(from, to, done)
  {
    ...
    {
      cmd = `robocopy "${from}" "${path.join(to, path.dirname(from))}" /MOVE`;
    }
    else
    {
      cmd = `mv -f "${from}" "${to}"`;
  ...
  {
    exec(process.platform === 'win32' ? `rmdir /S /Q "${dir}"` : `rm -rf "${dir}"`, done || function() {});
  }
  ...
  function processInputFile(inputFileName, scanIndex, responseId, config, done)
  {
    ...
    logger.info('Processing input file.', {responseId, inputFileName});
    ...
    step(
      ...
      {
        logger.info('Decoding the QR code.', {responseId, inputFileName});
      ...
      function handleDecodeQrCodeResultStep(err, qrCodes)
      {
        ...
        if ((!Array.isArray(qrCodes) || !qrCodes.length) && currentSurveyId && currentPageNumbers[scanIndex])
        {
          ...
          logger.info(
            'Failed to decode QR code. Using the previous survey and page number.', {
              responseId,
              inputFileName,
              surveyId: currentSurveyId,
              pageNumber: currentPageNumbers[scanIndex]
            }
          );
      ...
      function parseSurveyAndPageNumberStep()
      {
        ...
        if (matches === null)
        {
          logger.info('Invalid QR code. Using the previous survey and page number.', {
            responseId,
            inputFileName,
      ...
      {
        logger.info('Deskewing the input image.', {responseId, inputFileName});
      ...
      {
        logger.info('Resizing the input image.', {responseId, inputFileName});
  ...
  function handleResults(results, done)
  {
    ...
    step(
      ...
      function handleInsertOmrResultsResultStep(err)
      {
        if (err)
        {
          logger.error(err, 'Failed to insert OMR results.');
        }
      },
      ...
      function handleFindSurveyResultStep(err, survey)
      {
        if (err)
        {
          return this.skip(new Error('Failed to find survey: ' + err.message));
        }
      ...
      function handleCreateResponseResultStep(err, response)
      {
        if (err)
        {
          logger.error(err, 'Failed to insert a new response created from the OMR results.', {
            responseId: results[0].response
          });
        }
        else if (response)
        {
  ...
  function cleanUpProcessingFiles(results)
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          logger.error(err, 'Failed to clean up the processing files.', {responseId: results[0].response});
        }

        _.forEach(this.processingDirPaths, (dirPath) =>
        {
          removeDir(dirPath);
        });
      }
