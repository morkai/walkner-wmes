...
module.exports = function setUpAlertsServer(app, module)
{
  ...
  function onAppStarted()
  {
    loadAlerts(function(err)
    {
      if (err)
      {
        return module.error(err, 'Failed to reload all alerts.');
      }
      ...
      loadActiveDowntimes(function(err)
      {
        if (err)
        {
          return module.error(err, 'Failed to load active downtimes.');
        }

        _.forEach(onLoad, (cb) => { cb(); });
  ...
  function reloadAlert(prodDowntimeAlertId)
  {
    ProdDowntimeAlert.findById(prodDowntimeAlertId).lean().exec(function(err, prodDowntimeAlert)
    {
      if (err)
      {
        return module.error(err, `Failed to reload alert [${prodDowntimeAlertId}].`);
      }
  ...
  function handleDowntimeStart(prodDowntimeId)
  {
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          module.error(err, `Failed to handle downtime start [${prodDowntimeId}].`);
        }
        else
        {
  ...
  function finishDowntimeAlert(downtime, alert)
  {
    ...
    createProdLogEntry('finishDowntimeAlert', downtime, data).save(function(err)
    {
      if (err)
      {
        module.error(err, 'Failed to save the [finishDowntimeAlert] operation.');
      }
      else
      {
  ...
  function executeAction(downtime, alert, actionIndex)
  {
    ...
    step(
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to execute action.`, {
            actionIndex,
            alertId: alert._id,
            downtimeId: downtime._id
          });
  ...
  function sendEmail(recipients, messageValues)
  {
    ...
    app[module.config.mailSenderId].send(mailOptions, function(err)
    {
      if (err)
      {
        module.error(err, 'Failed to send e-mail notification.');
      }
      else
      {
        module.info(`Sent e-mail.`, {
          downtime: messageValues.downtime.rid,
          recipients: emails
        });
      }
    });
  ...
  function sendSms(recipients, messageValues)
  {
    ...
    app[module.config.smsSenderId].send(smsOptions, function(err)
    {
      if (err)
      {
        module.error(err, 'Failed to send SMS notification.');
      }
      else
      {
        module.info(`Sent text.`, {
          downtime: messageValues.downtime.rid,
          recipients: smsOptions.to
        });
      }
    });
  }
