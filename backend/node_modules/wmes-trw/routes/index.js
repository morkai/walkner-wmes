...
const prepareImageRoute = require('./prepareImage');
...
module.exports = (app, module) =>
{
  ...
  const canManage = user.auth('LOCAL', 'TRW:MANAGE');
  const canProgram = user.auth('TRW:PROGRAM', 'TRW:MANAGE');
  ...
  express.post('/trw/setIo', canManage, setIoRoute);
  express.post('/trw/getIo', canManage, getIoRoute);
  express.post('/trw/screen', canManage, screenRoute);

  express.post(
    '/trw/bases;prepare-image',
    canManage,
    multer({
      dest: os.tmpdir(),
      limits: {
    ...
    prepareImageRoute.bind(null, app, module)
  );
  ...
  function setUpDictionaryRoutes(modelName, dictionaryName)
  {
    ...
    const canManageDict = modelName === 'TrwProgram' ? canProgram : canManage;

    express.get(urlPrefix, canViewLocal, express.crud.browseRoute.bind(null, app, Model));
    express.post(urlPrefix, canManageDict, express.crud.addRoute.bind(null, app, Model));
    express.get(`${urlPrefix}/:id`, canViewLocal, express.crud.readRoute.bind(null, app, Model));
    express.put(`${urlPrefix}/:id`, canManageDict, express.crud.editRoute.bind(null, app, Model));
    express.delete(`${urlPrefix}/:id`, canManageDict, express.crud.deleteRoute.bind(null, app, Model));
  }
  ...
  function saveTestRoute(req, res, next)
  {
    step(
      ...
      function(err)
      {
        ...
        const program = this.test.program;
        ...
        app.broker.publish(`${TrwTest.TOPIC_PREFIX}.saved`, {
          test: {
            ...
            program: {
              _id: program._id,
              base: {
                _id: program.base._id,
                tester: {
                  _id: program.base.tester._id
                }
              }
            }
          }
        ...
        app.broker.publish(`${TrwCounter.TOPIC_PREFIX}.updated.${this.test.order}.${program._id}`, {
          count: this.test.pce
        });
      }
    );
  ...
  function exportTest(doc)
  {
    return {
      ...
      tester: doc.program.base.tester.name,
      base: doc.program.base.name,
      program: doc.program.name
    };
  }

  function setIoRoute(req, res)
  {
    req.body.outputs.forEach(output =>
    {
      io.set(`${output.device}:${output.channel}`, output.value);
    });

    res.json({});
  }

  function getIoRoute(req, res)
  {
    const result = {};

    req.body.inputs.forEach(input =>
    {
      if (!result[input.device])
      {
        result[input.device] = {};
      }

      result[input.device][input.channel] = io.get(`${input.device}:${input.channel}`) || 0;
    });

    res.json(result);
  }

  function screenRoute(req, res)
  {
    module.changeScreen(req.body);
