...
exports.report = (app, {Suggestion, User}, options, done) =>
{
  ...
  function handleSuggestion(s)
  {
    ...
    if (KAIZEN_STATUS[s.status])
    {
      inc('type', 'kaizen');
    }

    const kaizenEvent = s.categories.includes('KI');

    if (kaizenEvent)
    {
      inc('type', 'kaizenEvent');
    }
    ...
    if (s.kaizenEvent)
    {
      const key = transliterate(s.kaizenEvent, {unknown: '?'}).toUpperCase().replace(/[^A-Z0-9]+/g, '');

      results.kaizenEvents[key] = s.kaizenEvent;

      inc('productFamily', key);
    }

    ['confirmer', 'superiors'].forEach(prop =>
    {
      const user = s[prop];
      ...
      (Array.isArray(user) ? user : [user]).forEach(user =>
      {
    ...
    if (kaizenEvent)
    {
      incOwners('kaizenEvent', s.suggestionOwners);
      incOwners('kaizenEvent', s.kaizenOwners);
    }
