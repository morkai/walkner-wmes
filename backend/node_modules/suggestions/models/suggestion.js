...
exports.setUp = (app, mongoose) =>
{
  ...
  const observerSchema = new mongoose.Schema({
    ...
    role: {
      type: String,
      required: true,
      enum: ['creator', 'owner', 'confirmer', 'coordinator', 'subscriber']
    },
    lastSeenAt: Date,
    notify: Boolean,
    changes: {}
  ...
  suggestionSchema.methods.updateObservers = function(changedPropertyList, changes, newSubscribers)
  {
    ...
    // Owners
    const owners = [].concat(
      this.suggestionOwners,
      this.status !== 'new' && this.status !== 'cancelled' ? this.kaizenOwners : []
    );
  ...
  suggestionSchema.methods.compareProperty = function(property, input, changes)
  {
    ...
    else if (property === 'coordSections' && Array.isArray(newValue))
    {
      newValue.forEach(coordSection =>
      {
        coordSection.time = coordSection.time ? new Date(coordSection.time) : null;
      });
    }
    ...
    if (jsonDeepEqual(newValue, oldValue, {strict: true}))
    {
      return false;
    }
