...
{
  const {ATTACHMENT_KINDS} = module.Suggestion;
  ...
  function addPendingUploads(files, user)
  {
    ...
    _.forEach(files, file =>
    {
      ...
      const attachment = {
        ...
        kind: ATTACHMENT_KINDS.includes(file.fieldname) ? file.fieldname : 'other',
        meta: {}
      };
  ...
  function removeAttachments(filesToRemove)
  {
    ...
    {
      fs.unlink(path.join(module.config.attachmentsDest, fileName), err =>
      {
  ...
  {
    const releaseLock = await locks.createAsync(`suggestions/attachments/${attachment._id}`);

    const originalFilePath = path.join(module.config.attachmentsDest, attachment._id);
  ...
  function prepareInput(input, entry)
  {
    ...
    if (Array.isArray(edited))
    {
      edited.forEach(update =>
      {
        ...
        if (attachment && ATTACHMENT_KINDS.includes(update.kind))
        {
