...
exports.report = (app, {mongoose}, options, done) =>
{
  ...
  function countSuggestions(options, done)
  {
    ...
    if (options.superior)
    {
      conditions['superiors.id'] = options.superior;
    }
  ...
  function handleSuggestion(doc)
  {
    ...
    doc.suggestionOwners.forEach((owner, i) =>
    {
      incOwner(owner, doc.suggestionSuperiors[i]);
    });

    doc.kaizenOwners.forEach((owner, i) =>
    {
      incOwner(owner, doc.kaizenSuperiors[i]);
    });

    function incOwner(owner, superior)
    {
      if (options.superior && superior.id !== options.superior)
      {
        return;
      }
