...
{
  const {express, mongoose, user, settings, ToolCalTool, tmpAttachments} = module;
  ...
  express.post('/toolcal/tools', canManage, express.crud.addRoute.bind(null, app, {
    model: ToolCalTool,
    afterSave: keepAttachment
  }));
  ...
  function keepAttachment(tool)
  {
    if (tool.certificateFile && tmpAttachments.has(tool.certificateFile.hash))
    {
      clearTimeout(tmpAttachments.get(tool.certificateFile.hash).timer);
      tmpAttachments.delete(tool.certificateFile.hash);
    }
  }
};
