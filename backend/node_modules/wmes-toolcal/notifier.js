...
module.exports = (app, module) =>
{
  ...
  function check()
  {
    step(
      ...
      function(err, settings)
      {
        ...
        setImmediate(this.parallel(), null, settings['notifier.globalUsers'] || []);
      },
      function(err, tools, typeList, globalUsers)
      {
        if (err)
        {
        ...
        notifyNext(tools, typeMap, globalUsers.map(u => new ObjectId(u.id)), this.next());
      },
      function(err)
      {
        if (err)
  ...
  function notifyNext(tools, types, globalUsers, done)
  {
    if (tools.length === 0)
    {
      return done();
    ...
    step(
      function()
      {
        const userIds = globalUsers.concat(tool.users.map(u => new ObjectId(u.id)));
      ...
      function(err, recipients)
      {
        ...
        const mailOptions = {
          ...
          subject: `[WMES] [TOOLCAL] Nadchodząca kalibracja: ${tool.name}`,
          html: renderEmail({
      ...
      function(err)
      {
        ...
        setImmediate(notifyNext, tools, types, globalUsers, done);
      }
    );
  }
};
