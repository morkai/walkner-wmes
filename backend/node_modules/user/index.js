...
exports.start = (app, module) =>
{
    ...
    return function authMiddleware(req, res, next)
    {
      step(
        function()
        {
          if (!req.session)
          {
            return this.skip(app.createError('No request session.', 'NO_SESSION', 500));
          }
        ...
        function(err, sessionUser)
        {
          ...
          {
            module.warn(`Unauthorized access.`, {
              url: req.url,
  ...
  function resolveApiUser(sessionUser, apiKey, req, done)
  {
    ...
    step(
      function()
      {
        ...
        {
          User.findById(req.headers['x-api-user']).exec(this.parallel());
        }
      },
      function(err, apiUser, loggedUser)
      {
        ...
        if (!apiUser)
        {
          return done(app.createError('Unknown user API key.', 'INPUT', 400));
        }

        if (req.headers['x-api-user'] && !loggedUser)
        {
  ...
  function authenticate(credentials, done)
  {
    step(
      ...
      function checkUserDataStep(err, users)
      {
        ...
        const hasUnsafePassword = userData.login === credentials.password
  ...
  function removeApiSessions()
  {
    ...
    step(
      function()
      {
        app[module.config.mongooseId].model('User')
          .find({apiKey: {$ne: ''}})
          .select({_id: 1})
          .lean()
          .exec(this.next());
