...
exports.republishTopics = ['mor.**'];

exports.userPrivileges = ['MOR:MANAGE', 'MOR:MANAGE:USERS'];
...
exports.start = (app, module, done) =>
{
  ...
  module.USER_FIELDS = USER_FIELDS;
  ...
  app.broker.subscribe('users.updated.*', handleUserUpdated);
  ...
  function updateState(action, params, instanceId, done)
  {
    ...
    if (!done)
    {
      done = (err) =>
      {
        if (err)
        {
          module.error(err, `Failed to update state.`, {action, params});
        }
      };
    }
  ...
  function doSaveState()
  {
    ...
    fs.writeFile(module.config.statePath, stateJson, (err) =>
    {
      if (err)
      {
        module.error(err, `Failed to save the state file.`);
      }
  ...
  function reloadState(publish, done)
  {
    step(
      ...
      function(err, stateData, settings)
      {
        if (err)
        {
          module.warn(err, `Failed to reload the state.`);
        }
        ...
        if (stateData)
        {
          try
          {
            Object.assign(module.state, JSON.parse(stateData));
          }
          catch (err)
          {
            module.warn(err, `Failed to parse the state file.`);
          }
        }
  ...
  {
    const morUser = module.state.users.find(u => u._id.equals(updatedUser._id));

    if (morUser)
    {
      Object.assign(morUser, _.pick(updatedUser, _.keys(USER_FIELDS)));
    }

    if (updatedUser.prodFunction === 'manager')
    {
      reloadUsers(err =>
      {
        if (err)
        {
          module.error(err, `Failed to reload users after user update.`);
        }
      });
    }
  }
  ...
  function handleUserEdited(message)
  {
    ...
    if (morUser)
    {
      Object.assign(morUser, _.pick(editedUser, _.keys(USER_FIELDS)));
    }
    ...
    if (editedUser.prodFunction === 'manager')
    {
      reloadUsers(err =>
      {
        if (err)
        {
          module.error(err, `Failed to reload users after manager edit.`);
        }
      });
    }
  }
  ...
  {
    const morUser = module.state.users.find(u => u._id.equals(message.model._id));
    ...
    module.state.sections.forEach(section =>
    {
      if (section.watch.some(w => w.user === deletedUserId))
      {
        updateState('removeWatch', {
          section: section._id,
          user: deletedUserId
        });
      }
  ...
  {
    _.forEach(changes, (presence, userId) =>
    {
      if (userMap[userId])
      {
  ...
  {
    const key = message._id.replace('mor.', '');

    module.state.settings[key] = message.value;

    if (key === 'globalProdFunctions')
    {
      reloadState(true, _.noop);
    }
  }
  ...
  function addSection(params, done)
  {
    ...
    section = _.defaults(_.pick(params, props), defaultSection);
  ...
  function editSection(params, done)
  {
    ...
    Object.assign(section, _.pick(params, [
      'name', 'watchEnabled', 'mrpsEnabled', 'subdivisions', 'prodFunctions'
    ]));
  ...
  function editWatch(params, done)
  {
    ...
    Object.assign(watch, _.pick(params, ['days', 'from', 'to']));
  ...
  function addMrp(params, done)
  {
    ...
    loadUsers(params.iptCheckRecipients, (err, iptCheckRecipients) =>
    {
      if (err)
      {
        return done(err);
      ...
      done(null, Object.assign(params, {iptCheckRecipients}));
    });
  }
  ...
  function editProdFunction(params, done)
  {
    ...
    loadUsers(params.users, (err, users) =>
    {
      if (err)
      {
        return done(err);
      ...
      done(null, Object.assign(params, {users}));
    });
  }

  function fixGlobalProdFunctions()
  {
    const globalProdFunctions = module.state.settings.globalProdFunctions || [];
    const prevCount = module.state.globalProdFunctions.length;

    module.state.globalProdFunctions = module.state.globalProdFunctions.filter(
      p => globalProdFunctions.includes(p._id)
    );

    if (module.state.globalProdFunctions.length !== prevCount)
    {
      saveState();
    }
  }
