...
module.exports = class MoxaRestLamps
{
    ...
    lamps.request(null, (err, res) =>
    {
      ...
      else
      {
        ...
        lamps.line.stations.forEach((station, i) =>
        {
          ...
          lamps.lc.stations[i + 1].lamp = redLampIo && redLampIo.doStatus === 1
            ? 'red'
  ...
  writeLamps(stations, done)
  {
    ...
    lamps.request(lamps.state, (err, res) =>
    {
      if (err)
      {
        lamps.logger.error(err, 'Failed to write lamps.', {state: lamps.state});
      }
      ...
      {
        lamps.logger.debug('Wrote lamps.', {state: lamps.state});
      }
