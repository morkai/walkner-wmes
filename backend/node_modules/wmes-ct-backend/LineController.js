...
module.exports = class LineController
{
    ...
    step(
      ...
      function(err)
      {
        ...
        if (lc.updatedStations
          && !lc.timers.readLamps
          && Date.now() - lc.readLampsAt > 300000)
        {
          lc.readLamps(false, this.next());
        }
      },
      function()
      {
  ...
  startNextPce(startedAt, stationNo, cart, done)
  {
    ...
    {
      this.trace('Ignored next PCE: no order at previous station.', {stationNo});
    }
  ...
  readLamps(repeat, done)
  {
    const lc = this;

    lc.trace('Reading lamps...');

    lc.readLampsAt = Date.now();

    clearTimeout(lc.timers.readLamps);
    ...
    function readLamp(lampIp, done)
    {
      lc.request(lampIp, '', (err, res) =>
      {
        ...
        {
          lc.logger.warn('Failed to read lamp: invalid response.', {lampIp, response: res});
        }
  ...
  toggleLamps(done)
  {
    ...
    const candidates = _.map(lc.stations, station =>
    {
      ...
      const actualTaktTime = station.qtyDone ? (station.durations.work / station.qtyDone / 1000) : 0;
    ...
    lc.trace('Candidates:', {
      ...
      {
        return {station: c.station._id, rank: c.rank};
      })
    });
    ...
    candidates.forEach(({rank, station}) =>
    {
      if (rank <= 0)
      {
        if (station.lamp || forceRequests)
        {
    ...
    function toggleLamp(station, done)
    {
      ...
      lc.request(lampIp, `?out=${station.lamp ? 1 : 0}`, (err, res) =>
      {
        const validResponse = res.includes('<out>');

        if (err)
        {
          lc.logger.error(err, 'Failed to toggle lamp.', {lampIp});
        }
        else if (!validResponse)
        {
          lc.logger.warn('Failed to toggle lamp: invalid response.', {lampIp, response: res});
        }
  ...
  request(lampIp, query, done)
  {
    ...
    timers[timerId] = setTimeout(() => socket.destroy(), 5000);
