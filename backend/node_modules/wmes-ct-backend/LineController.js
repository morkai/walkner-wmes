...
const DummyLamps = require('./DummyLamps');
...
module.exports = class LineController
{
  ...
  {
    const Lamps = line.common && line.common.lampControllerIp
      ? MoxaRestLamps
      : line.type === 'inveo'
        ? InveoLamps
        : DummyLamps;
    ...
    this.lamps = new Lamps(app, module, this);
    ...
    Object.keys(this.stations).forEach(stationNo =>
    {
      const stationI = parseInt(stationNo, 10) - 1;

      if (!line.stations[stationI])
      {
        delete this.stations[stationNo];
      }
    });
  ...
  scheduleCheckTodos(delay)
  {
    ...
    this.hasMoreTodos = false;
    this.timers.checkTodos = setTimeout(this.checkTodos.bind(this), delay);
  }
  ...
  {
    this.logger.debug('Handling todo...', {todo});
  ...
  handleInputAction(todo, done)
  {
    if (!todo.data.state)
    {
      this.logger.debug('Ignored the elevator going down.', {todo});
    ...
    if (todo.station !== 2)
    {
      this.logger.debug('Ignored elevator going up on an invalid station.', {todo});
  ...
  handleOrderAction(todo, done)
  {
    ...
    {
      this.logger.debug('Ignored order action: luma2.', {todo});
        ...
        _.forEach(lc.stations, (station, n) =>
        {
          ...
          else if (todo.action === 'correctOrder')
          {
            if (station.order.pso === newOrder.pso)
            {
              ...
              lc.logger.debug('Order corrected.', {station});
  ...
  handleSnAction(todo, done)
  {
    ...
    {
      this.logger.debug('Ignored SN action: luma2.');
    ...
    const pipeline = [
      {$match: {
        prodShiftOrder: station.order.pso,
    ...
    this.module.ProdSerialNumber.aggregate(pipeline, (err, result) =>
    {
      if (err)
      {
        return done(err);
      ...
      const qtyScanned = result && result.length ? result[0].qtyScanned : 0;

      if (station.qtyDone < qtyScanned)
      {
        this.finishPce(station, todo.time, null, done);
      }
      else
      {
        this.logger.warn('Ignored SN action: not enough scanned.', {
          station,
  ...
  handlePceStartedAction(todo, done)
  {
    ...
    lc.logger.debug('Starting next PCE...', {
    ...
    step(
      ...
      function()
      {
        ...
        lc.logger.debug('PCE started.', {station});
      },
      done
    );
  }
  ...
  finishPce(station, finishedAt, qtyDone, done)
  {
    ...
    step(
      ...
      function()
      {
        ...
        lc.logger.debug('PCE finished.', {station});
  ...
  startFirstPce(startedAt, cart, done)
  {
    ...
    if (!station)
    {
      lc.logger.debug('Ignored first PCE: no stations defined.');

      return done();
    }

    lc.logger.debug('Starting first PCE...', {startedAt, cart});
    ...
    step(
      function()
      {
        if (station.pceStartedAt && !station.pceFinishedAt)
        {
          lc.finishPce(station, startedAt, null, this.next());
        }
      },
      function()
      {
      ...
      function(err, newOrder)
      {
        ...
        lc.logger.debug('First PCE started.', {station});
  ...
  changeOrder(station, newOrder)
  {
    ...
    this.logger.debug('Order changed.', {
  ...
  startNextPce(startedAt, stationNo, cart, done)
  {
    ...
    lc.logger.debug('Starting next PCE...', {startedAt, stationNo, cart});

    const startedStation = lc.stations[stationNo];

    if (!startedStation)
    {
      lc.logger.debug('Ignored next PCE: invalid station.', {stationNo});
    ...
    {
      lc.logger.debug('Ignored next PCE: no order at previous station.', {stationNo});
    ...
    step(
      function()
      {
        if (!finishedStation.pceFinishedAt && (!finishedStation.cart || finishedStation.cart === cart))
        {
          lc.finishPce(finishedStation, startedAt, null, this.parallel());
        }

        if (!startedStation.pceFinishedAt)
        {
          lc.finishPce(startedStation, startedAt, null, this.parallel());
        }
      },
      function()
      {
        ...
        lc.module.CtPce
          .findOne({
            ...
            station: stationNo - 1,
            cart,
            startedAt: {$gte: new Date(Date.now() - 3 * 3600 * 1000)}
          })
          .select({order: 1})
          .sort({line: 1, station: 1, cart: 1, startedAt: -1})
          .lean()
          .exec(this.next());
      },
      function(err, prevPce)
      {
        if (err)
        {
          lc.logger.error(err, 'Failed to find prev PCE.', {stationNo});
        }

        const oldOrder = startedStation.order;
        const newOrder = prevPce ? prevPce.order : {...finishedStation.order};
        ...
        lc.logger.debug('Started next PCE.', {station: startedStation});
    ...
    this.module.ProdShiftOrder
      ...
      .exec((err, pso) =>
      {
        ...
        if (!pso)
        {
          ...
          _id: pso.orderId,
          pso: pso._id,
          mrp: pso.orderData && pso.orderData.mrp || '',
  ...
  toggleLamps(done)
  {
    ...
    if (lc.destroyed || lc.lamps instanceof DummyLamps)
    {
      return done();
    }

    lc.logger.debug('Toggling lamps...');
    ...
    {
      lc.logger.debug('No lamps to change.');

      return done();
    }

    lc.logger.debug('Writing lamps...', {
