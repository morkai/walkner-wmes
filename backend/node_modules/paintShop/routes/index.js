...
const editLoadRoute = require('./editLoad');
const exportLoad = require('./exportLoad');
...
module.exports = (app, module) =>
{
  ...
  const canView = user.auth('LOCAL', 'PAINT_SHOP:VIEW', 'PLANNING:VIEW', 'PROD_DATA:VIEW', 'PRESS_WORKSHEETS:VIEW');
  ...
  // Ignored components
  express.get(
    '/paintShop/ignoredComponents',
    canView,
    express.crud.browseRoute.bind(null, app, PaintShopIgnoredComponent)
  );
  express.post(
    '/paintShop/ignoredComponents',
    canManage,
    express.crud.addRoute.bind(null, app, PaintShopIgnoredComponent)
  );
  express.get(
    '/paintShop/ignoredComponents/:id',
    canView,
    express.crud.readRoute.bind(null, app, PaintShopIgnoredComponent)
  );
  express.put(
    '/paintShop/ignoredComponents/:id',
    canManage,
    express.crud.editRoute.bind(null, app, PaintShopIgnoredComponent)
  );
  express.delete(
    '/paintShop/ignoredComponents/:id',
    canManage,
    express.crud.deleteRoute.bind(null, app, PaintShopIgnoredComponent)
  );

  // Load
  express.get('/paintShop/load', canView, express.crud.browseRoute.bind(null, app, PaintShopLoad));
  express.patch('/paintShop/load', canUpdate, editLoadRoute.bind(null, app, module));
  express.get('/paintShop/load;export.:format?', canView, exportLoad.bind(null, app, module));
  express.get('/paintShop/load/reasons', canView, express.crud.browseRoute.bind(null, app, PaintShopLoadReason));
  ...
  {
    PaintShopLoad.getStats(+req.query.counter, (err, stats) =>
    {
      if (err)
      {
        return next(err);
  ...
  async function getRecentLoadRoute(req, res)
  {
    const limit = Math.min(3840, Math.max(1, +req.query.limit || 100));
    const conditions = {};
    const sort = {};

    if (req.query.counter > 0)
    {
      conditions['_id.c'] = +req.query.counter;
      sort['_id.c'] = 1;
    }

    sort['_id.ts'] = -1;

    const docs = await PaintShopLoad
      .find(conditions)
      .select({_id: 0, d: 1})
      .sort(sort)
      .limit(limit)
    ...
    const collection = new Array(docs.length);

    for (let i = docs.length - 1; i >= 0; --i)
    {
      collection[i] = Math.round(docs[i].d / 1000);
    }
  ...
  function loadReportRoute(req, res, next)
  {
    ...
    const options = {
      ...
      bucketSize: Math.max(parseInt(query.bucketSize, 10) || 0, 5)
    };
