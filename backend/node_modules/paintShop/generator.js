...
module.exports = function(app, module)
{
  ...
  function createPaintShopOrder(date, o)
  {
    return {
      _id: o._id,
      status: 'new',
  ...
  function generate(date)
  {
    ...
    const oldUnpainted = Date.parse(date) < Date.parse('2019-05-13T00:00:00Z');
    ...
    step(
      function()
      {
        if (key === '2019-05-13' || key === '2019-05-14')
        {
      ...
      function(err, settings, planSettings, orders, startTimes)
      {
        ...
        orders.forEach(o =>
        {
          ...
          this.newOrders.set(o._id, createPaintShopOrder(date, o));
        });
      ...
      function(err, childOrders, unplannedOrders, splitChildOrders)
      {
        ...
        if (oldUnpainted)
        {
          childOrders = childOrders.filter(childOrder =>
          {
            if (!this.unpaintedMrps.includes(childOrder.mrp))
            {
              return true;
            }

            const paint = childOrder.bom.find(component => component.unit === 'G' || component.unit === 'KG');
            ...
            childOrder.leadingOrder = null;

            unplannedOrders.push(childOrder);
        ...
        unplannedOrders.concat(splitChildOrders).forEach(unplannedOrder =>
        {
          ...
          {
            this.newOrders.set(unplannedOrder._id, createPaintShopOrder(date, unplannedOrder));
          }
        });
      ...
      function(err, childOrders, leadingOrders)
      {
        ...
        {
          this.newOrders.set(leadingOrder._id, createPaintShopOrder(date, leadingOrder));
        });
        ...
        childOrders.forEach(childOrder =>
        {
          ...
          if (!paint)
          {
            ...
            else if (!this.unpaintedMrps.includes(leadingOrder.mrp)
      ...
      function()
      {
        ...
        mergedNewOrders.forEach((newOrder, i) =>
        {
          ...
          if (!newOrder.qtyPaint && newOrder.paint.nc12 === '000000000000')
          {
  ...
  function saveChangeLog(key, changeLog)
  {
    ...
    fs.writeFile(logFilePath, JSON.stringify(changeLog, null, 2), err =>
    {
      if (err)
      {
        logger.error(err, `Failed to save log data.`);
      }
    });
  }
