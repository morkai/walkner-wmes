...
module.exports = (app, module) =>
{
  ...
  function createNotes(o, productNotes)
  {
    ...
    (productNotes.get(o._id) || []).forEach(note => notes.add(note));
    ...
    step(
      ...
      function(err, psSettings, planSettings, orders, startTimes, productNotes)
      {
        ...
        productNotes.forEach(productNote =>
        {
          ...
          productNote.codes.forEach(code =>
          {
            if (!this.productNotes.has(code))
            {
              this.productNotes.set(code, []);
            }

            this.productNotes.get(code).push(note);
          });
        });
      ...
      function(err, childOrders, leadingOrders)
      {
        ...
        childOrders.forEach(childOrder =>
        {
          ...
          (this.productNotes.get(childOrder._id) || []).forEach(note => notes.add(note));
      ...
      function()
      {
        ...
        if (added.length)
        {
          ...
          PaintShopOrder.collection.insertMany(added, {ordered: false, writeConcern: {w: 'majority'}}, (err, res) =>
          {
            if (err && err.result)
            {
              res = err.result;
        ...
        if (removed.length)
        {
          PaintShopOrder.collection.deleteMany(
            {_id: {$in: removed.map(o => o._id)}},
        ...
        {
          PaintShopOrder.collection.updateOne(
            {_id: change._id},
            {$set: change},
            {writeConcern: {w: 'majority'}},
            this.group()
      ...
      function(err)
      {
        ...
        this.notAdded.forEach(duplicateOrder =>
        {
          ...
          const options = {returnOriginal: false, writeConcern: {w: 'majority'}};
