...
exports.models = [
  ...
  require('./models/paintShopIgnoredComponent'),
...
exports.optionalModules = {
  ...
  'mongoose user express updater reports settings': require('paintShop/routes')
};
...
exports.start = (app, module) =>
{
  ...
  app.broker.subscribe('paintShop.load.changed', onLoadChanged);
  ...
  function onLoadChanged({counter, timestamp, duration})
  {
    const items = [{
      _id: {
      ...
      d: duration > module.config.maxLoadDuration ? 0 : duration,
      r: null
    }];
console.inspect(items);
    PaintShopLoad.updateLoad(counter, items, err =>
    {
      if (err)
      {
        module.error(err, 'Failed to update load.');
      }
    });
  }
};
