...
exports.setUp = (app, mongoose) =>
{
  ...
  const orderSchema = new mongoose.Schema({
    ...
    status: {
      type: String,
      enum: ['new', 'aside', 'started', 'partial', 'finished', 'cancelled'],
      default: 'new'
    },
    startedAt: Date,
    finishedAt: Date,
  ...
  orderSchema.statics.TOPIC_PREFIX = 'paintShop.orders';
  orderSchema.statics.BROWSE_LIMIT = 1000;

  orderSchema.index({date: -1});
  orderSchema.index({status: 1, date: -1});
  orderSchema.index({order: 1});
  orderSchema.index({'childOrders.order': 1});

  orderSchema.methods.act = function(input, done)
  {
