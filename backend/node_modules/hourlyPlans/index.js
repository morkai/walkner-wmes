...
exports.optionalModules = {
  ...
  'mongoose user sio divisions fte': require('./commands')
};
...
exports.start = (app, module) =>
{
  ...
  function recountPlannedQuantities(shiftId, prodFlowId, activeProdLineIds)
  {
    ...
    if (!division)
    {
      return module.warn(`Couldn't find a division for prod flow [${prodFlowId}].`);
    }
    ...
    HourlyPlan.recountPlannedQuantities(division._id, shiftId, prodFlowId, activeProdLineIds, (err, prodShifts) =>
    {
      if (err)
      {
        return module.error(err, `Failed to recount planned quantities for prod flow [${prodFlowId}]`);
      }
  ...
  function findAndRecountPlannedQuantities(hourlyPlanId)
  {
    ...
    HourlyPlan.findById(hourlyPlanId).exec((err, hourlyPlan) =>
    {
      if (err)
      {
        return module.error(err, `Failed to find hourly plan [${hourlyPlanId}] to recount planned quantities.`);
      }
  ...
  function updateHourlyPlansFromDailyPlan(planDate, changedLines, done)
  {
    ...
    step(
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to update from plan [${dailyMoment.format('YYYY-MM-DD')}].`);
        }
