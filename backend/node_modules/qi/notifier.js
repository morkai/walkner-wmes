  ...
  function notifyNokOwner(result)
  {
    step(
      ...
      function findManagerStep(err, nokOwner, order)
      {
        ...
        {
          if (!order || !section.mrps.some(mrp => mrp._id === order.mrp))
          {
            return;
          }
      ...
      function prepareTemplateDataStep(err, managers)
      {
        ...
        this.mailOptions = {
          ...
          subject: `[WMES] [QI] Przypisanie do wyniku inspekcji: ${result.rid}`,
          html: ''
        };
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          module.error(err, `Failed to notify the NOK owner about a result [${result.rid}].`);
        }
        else if (this.mailOptions)
        {
          module.info(`Notified the NOK owner [${this.mailOptions.to}] about a result: ${result.rid}`);
        }
