...
module.exports = function editResultRoute(app, qiModule, req, res, next)
{
  ...
  else
  {
    ...
    const inspector = userModule.isAllowedTo(user, 'QI:INSPECTOR') || leader;
    const specialist = userModule.isAllowedTo(user, 'QI:SPECIALIST');
    const master = userModule.isAllowedTo(user, 'FN:master');
    const qualityEngineer = userModule.isAllowedTo(user, 'FN:quality_engineer');
  ...
  step(
    ...
    function applyChangesStep(err, qiResult)
    {
      ...
      const nokOwner = qiResult.nokOwner && qiResult.nokOwner.id === user._id;
      const leader = qiResult.leader && qiResult.leader.id === user._id;
