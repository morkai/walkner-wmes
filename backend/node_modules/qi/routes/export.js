...
exports.fetchDictionaries = (app, module, req, res, next) =>
{
  const {
    mongoose,
    QiResult
  ...
  step(
    function findStep()
    {
      ...
      QiResult.aggregate(pipeline, this.group());
...
exports.serializeRow = (app, module, doc, req) =>
{
  ...
  const row = {
    ...
    inspector: doc.inspector ? doc.inspector.label : '',
    nokOwner: doc.nokOwner ? doc.nokOwner.label : '',
    leader: doc.leader ? doc.leader.label : '',
    coach: doc.coach ? doc.coach.label : '',
    ...
    rootCause: (doc.rootCause || [])
      .map((rootCause) => rootCause.map((why, i) => `${i + 1}. ${why}`).join('\r\n'))
      .join('\r\n\r\n'),
    notes: doc.notes
  };
