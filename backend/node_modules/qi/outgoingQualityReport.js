...
exports.report = (app, {QiOqlWeek, QiResult, Setting}, options, done) =>
{
  ...
  options.topCount = options.topCount || 3;
  ...
  step(
    ...
    function findRemainingOqlWeeksStep(err, firstOqlWeek, settings)
    {
      if (err)
      {
      ...
      {
        options.settings[setting._id] = setting.value;
      });
    ...
    function findResultsStep()
    {
      ...
      const oqlKinds = options.settings['qi.oqlKinds'];

      if (Array.isArray(oqlKinds) && oqlKinds.length)
      {
        conditions.kind = {$in: oqlKinds.map(id => new ObjectId(id))};
      }
  ...
  function sortGroup(unsorted, total)
  {
    ...
    Object.keys(unsorted).forEach(key =>
    {
      sorted.push([
        key,
        unsorted[key].value,
        Math.round(unsorted[key].value / total * 100),
        unsorted[key].errorCategories
      ]);
    });
  ...
  function handleQiResult(qiResult)
  {
    ...
    inc(totals, 'what', qiResult.faultCode, qiResult.qtyNok, qiResult.errorCategory);
    inc(group, 'what', qiResult.faultCode, qiResult.qtyNok, qiResult.errorCategory);
  ...
  function inc(group, prop, key, value, errorCategory)
  {
    if (!group[prop][key])
    {
      group[prop][key] = {
    ...
    group[prop][key].value += value;

    if (errorCategory)
    {
      if (!group[prop][key].errorCategories[errorCategory])
      {
        group[prop][key].errorCategories[errorCategory] = 0;
      }

      group[prop][key].errorCategories[errorCategory] += value;
    }
  }
};
