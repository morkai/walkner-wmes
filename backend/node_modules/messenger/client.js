...
exports.start = (app, module, done) =>
{
  ...
  _.forEach(module.config.broadcastTopics, (broadcastTopic) =>
  {
    app.broker.subscribe(broadcastTopic, (message, topic, meta) =>
    {
  ...
  createPushSocket(function(err, socket)
  {
    ...
    if (pushSocket === null)
    {
      module.info('push socket not used.');
    }
    else
    {
      module.info(`push socket listening on port ${module.config.pushPort}.`);
    }
  ...
  function createSubSocket()
  {
    ...
    {
      module.error(err, 'sub socket error');
    });
    ...
    subSocket.on('connect', function()
    {
      ...
      module.info(`sub socket connected on port ${module.config.pubPort}.`);
    ...
    subSocket.on('reconnect attempt', function()
    {
      if (connected)
      {
        module.info('sub socket disconnected. Reconnecting...');
  ...
  function createReqSocket()
  {
    ...
    {
      module.error(err, 'req socket error');
    });
    ...
    reqSocket.on('connect', function()
    {
      ...
      module.info(`req socket connected on port ${module.config.repPort}.`);
    ...
    reqSocket.on('reconnect attempt', function()
    {
      if (connected)
      {
        module.info('req socket disconnected. Reconnecting...');
  ...
  function sendMessage(socket, type, data, responseHandler)
  {
    ...
    const responseTimeout = data && data.responseTimeout || module.config.responseTimeout;
