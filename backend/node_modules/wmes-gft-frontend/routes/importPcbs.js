...
module.exports = (app, module, req, res, next) =>
{
  ...
  step(
    ...
    function(err, sheets)
    {
      ...
      setImmediate(this.next(), null, Array.from(pcbs.values()));
    },
    function(err, pcbs)
    {
      if (err)
  ...
  function parseSheet(sheet, pcbs)
  {
    ...
    for (let c = range.s.c; c <= range.e.c; ++c)
    {
      ...
      const cell1 = sheet[col + row1] || empty;
      const cell2 = sheet[col + row2] || empty;
    ...
    for (let r = range.s.r + 2; r <= range.e.r + 1; ++r)
    {
      ...
      if (!pcb.productFamily.length)
      {
        pcb.productFamily.push('ANY');
      }
      ...
      const key = `${pcb.code}:${pcb.quantity}`;

      if (pcbs.has(key))
      {
        const mainPcb = pcbs.get(key);

        mainPcb.productFamily = Array.from(new Set(mainPcb.productFamily.concat(pcb.productFamily)));
        mainPcb.lampColor = Array.from(new Set(mainPcb.lampColor.concat(pcb.lampColor)));
        mainPcb.ledCount = Array.from(new Set(mainPcb.ledCount.concat(pcb.ledCount)));
      }
      else
      {
        pcbs.set(key, pcb);
