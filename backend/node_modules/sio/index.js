...
exports.republishTopics = ['sockets.connected', 'sockets.disconnected'];
...
exports.start = (app, module, done) =>
{
  ...
  function killZombies()
  {
    ...
    {
      const oldSocketIds = Object.keys(module.sockets.connected);

      oldSocketIds.forEach(socketId =>
      {
        const ioSocket = module.sockets.connected[socketId];
      ...
      const newSocketIds = Object.keys(module.sockets.connected);

      if (newSocketIds.length < oldSocketIds.length)
      {
        module.info(`Killed zombies!`, {
          zombieCount: oldSocketIds.length - newSocketIds.length
        });
      }
    });
  }
  ...
  function netstat(done)
  {
    ...
    const cmd = module.config.netstatCmd || (win32 ? 'netstat -p tcp -n' : 'netstat -ant | grep ESTABLISHED');
