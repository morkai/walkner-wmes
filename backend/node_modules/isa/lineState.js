...
module.exports = (app, module) =>
{
  ...
  module.getAllActiveRequests = getAllActiveRequests;
  module.getLineActiveRequests = getLineActiveRequests;
  module.updateActiveRequest = updateActiveRequest;
  ...
  function loadActiveRequests()
  {
    ...
    step(
      ...
      function(err, requests)
      {
        if (err)
        {
          module.error(err, `Failed to load active requests.`);
        }
        ...
        module.info(`Loaded in ${Date.now() - t} ms.`);
      }
    );
  }
  ...
  function recordEvent(eventData)
  {
    ...
    event.save((err) =>
    {
      if (err)
      {
        module.error(err, `Failed to save event [${event.type}].`);
      }
    });
  }
  ...
  function updateActiveRequest(prodLineId, action, parameters, userData, done)
  {
    ...
    handleAction(prodLineId, parameters, userData, (err, result) =>
    {
      done(err, result);
  ...
  function createRequest(prodLineId, type, requester, data, done)
  {
    const request = new IsaRequest({
      ...
      orgUnits: orgUnits.getAllForProdLineAsList(prodLineId),
      type: type,
      data: data,
      requester: requester,
  ...
  actionHandlers.requestPickup = (prodLineId, parameters, userData, done) =>
  {
    ...
    createRequest(prodLine._id, 'pickup', userData.info, {}, (err, request) =>
    {
      if (!err)
      {
        recordEvent({
  ...
  actionHandlers.requestDelivery = (prodLineId, parameters, userData, done) =>
  {
    const prodLine = orgUnits.getByTypeAndId('prodLine', prodLineId);
  ...
  actionHandlers.cancelRequest = (prodLineId, parameters, userData, done) =>
  {
  ...
  actionHandlers.acceptRequest = (prodLineId, parameters, userData, done) =>
  {
    // TODO: remove local access?
    if (!userData.isLocal && !userData.isWhman && !userData.canManage)
    {
    ...
    step(
      ...
      function(err, responder)
      {
        ...
        if (!_.includes(responder.privileges, 'ISA:WHMAN'))
        {
          return this.skip(app.createError('INVALID_RESPONDER', 400));
        }

        parameters.responder = user.createUserInfo(responder, null);
  ...
  actionHandlers.finishRequest = (prodLineId, parameters, userData, done) =>
  {
    const prodLine = orgUnits.getByTypeAndId('prodLine', prodLineId);
