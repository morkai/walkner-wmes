...
{
  const {mailSender, User, Subscription, OrderDocumentFolder} = module;
  ...
  function onFilePurged({file})
  {
    Subscription.deleteMany({type: 'orderDocumentTree', target: file._id.toString()}, err =>
    {
      if (err)
      {
        module.error(err, `Failed to delete subscriptions for file [${file._id}].`);
      }
    });
  }
    ...
    step(
      ...
      function(err, users, folders)
      {
        ...
        const mailOptions = {
          to: emails,
          replyTo: emails,
          subject: `[WMES] [DOCS] Zmiana dokumentu: ${file.name} (${file._id})`,
      ...
      function(err)
      {
        if (err)
        {
          module.error(err, `Failed to notify users about file edit [${file._id}].`);
        }
      }
    );
  }
