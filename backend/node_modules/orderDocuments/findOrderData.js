...
module.exports = (app, module, {orderNo, line}, done) =>
{
  ...
  step(
    ...
    function(err, sapOrder)
    {
      ...
      const components = sapOrder.bom.filter(c => !!c.nc12).map(c => c.nc12);

      OrderDocumentFile
        .find({'components.nc12': {$in: components}})
        .select({_id: 1, name: 1})
        .lean()
        .exec(this.parallel());
    },
  ...
  function resolveConfirmations(orderData, componentFiles, done)
  {
    ...
    componentFiles.forEach(file =>
    {
      ...
      orderData.confirmations[file._id] = [null, null, null, null, null, null, null, null];
    });
    ...
    OrderDocumentConfirmation.find(conditions).select({nc15: 1, station: 1}).lean().exec((err, confirmations) =>
    {
      if (err)
      {
        return done(err);
      ...
      _.forEach(orderData.confirmations, stations =>
      {
        stations.forEach((v, i) => stations[i] = false);
      });

      confirmations.forEach(confirmation =>
      {
        orderData.confirmations[confirmation.nc15][confirmation.station] = true;
      });
