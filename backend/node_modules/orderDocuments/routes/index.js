...
module.exports = (app, module) =>
{
  ...
  express.post(
    ...
    multer({
      storage: multer.diskStorage({
        destination: path.join(module.config.uploadsDest, '.tmp')
      }),
      fileFilter: (req, file, done) =>
      {
        done(null, allowedTypes[file.mimetype] === file.originalname.split('.').pop().toLowerCase());
      }
    }).single('file'),
    (req, res, next) =>
    {
  ...
  express.get('/orderDocuments/folders', canViewLocal, express.crud.browseRoute.bind(null, app, OrderDocumentFolder));
  express.get('/orderDocuments/folders/:id', canViewUser, express.crud.readRoute.bind(null, app, OrderDocumentFolder));
  express.get('/orderDocuments/files', canViewLocal, express.crud.browseRoute.bind(null, app, OrderDocumentFile));
  ...
  // Tools
  //
  express.get('/orderDocuments;fix-missing-docs', user.auth('ORDERS:MANAGE'), fixMissingDocsRoute);

  function fixMissingDocsRoute(req, res, next)
  {
    if (/^[0-9]{9}$/.test(req.query.order))
    {
      module.scheduleMissingDocsFix(req.query.order);

      res.sendStatus(204);
    }
    else
    {
      next(app.createError('Invalid order number.', 'INPUT', 400));
    }
  }
};
