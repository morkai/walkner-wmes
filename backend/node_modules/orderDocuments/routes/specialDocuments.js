...
function handleBomDocument(app, {Order}, req, res, next)
{
  step(
    ...
    function(err, order)
    {
      ...
      Order.assignChangeNumbers(order, this.parallel());
    ...
    function(err, order)
    {
      ...
      (order.bom || []).forEach(component =>
      {
        ...
        {
          const validFrom = moment.utc(component.validFrom);

          if (Math.abs(validFrom.diff(Date.now(), 'days')) <= 30)
          {
            component.rowClassName += ' recent';
          }
        }
  ...
  step(
    ...
    function(err, order)
    {
      ...
      order.compRels.forEach(compRel =>
      {
        ...
        if (!compRel.oldComponents.length)
        {
          compRel.newComponents.forEach(newComponent =>
          {
            compRels.push({
...
function handleEtoDocument(app, {Order, OrderEto}, req, res, next)
{
  step(
    function()
    {
      Order.findById(req.query.order).select({leadingOrder: 1, nc12: 1}).lean().exec(this.next());
    },
    function(err, order)
    {
      if (err)
      ...
      OrderEto.findById(order.nc12).select({html: 1}).lean().exec(this.parallel());

      if (order.leadingOrder && order.leadingOrder !== order._id)
      {
        Order.findById(order.leadingOrder).select({nc12: 1}).lean().exec(this.parallel());
      }
    },
    function(err, orderEto, leadingOrder)
    {
      if (err)
      {
        return this.skip(err);
      ...
      if (orderEto)
      {
        return setImmediate(this.next(), null, orderEto);
      }

      if (leadingOrder)
      {
        this.order = leadingOrder;

        OrderEto.findById(leadingOrder.nc12).select({html: 1}).lean().exec(this.next());
      }
    },
