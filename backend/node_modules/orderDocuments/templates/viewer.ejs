...
<div>
  <ul id="images">
    <% var pageCount = meta.pageCount || (meta.pages ? meta.pages.length : 1); %>
    <% for (var p = 1; p <= pageCount; ++p) { %>
    <li><img id="image" src="/orderDocuments/<%- nc15 %>/<%- p %>?hash=<%- hash %>" alt="<%- p %>"></li>
    <% } %>
  </ul>
</div>
...
<script src="/app/orderDocuments/viewer.js"></script>
</body>
</html>
