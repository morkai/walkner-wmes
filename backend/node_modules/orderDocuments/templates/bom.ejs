...
<head>
  <meta charset="utf-8">
  <title><%- order %> - Lista komponentów - BOM</title>
  <style>
    * {
...
<body>
<h1 id="hd">Lista komponentów</h1>
<% if (!components.length) { %>
<p>Zlecenie <%- order %> nie ma żadnych komponentów.</p>
<% } else { %>
<% if (notes.length || compRels.length) { %>
<div id="notes">
  <% compRels.forEach(function(compRel) { %>
  <p class="danger">
    ⇄ <% if (compRel.oldCode) { %><%- compRel.oldCode %> ➡ <% } %><%- compRel.newCode %>
  </p>
  <% }) %>
...
<div id="columns">
  <div class="column">
    <table>
      ...
      <tr class="<%- component.rowClassName || '' %> <%- component.usedOnStation ? 'used' : '' %>">
        <% if (component.releasedAt) { %>
        <td class="item min center">⇄
        <% } else { %>
...
<script>
  ...
  function resize()
  {
    ...
    var notesHeight = notesEl ? notesEl.getBoundingClientRect().height : 0;
    var newColumnCount = Math.floor((window.innerWidth - PADDING) / (MIN_COLUMN_WIDTH + PADDING));
    var availableHeight = window.innerHeight - thHeight - (PADDING * 3) - HEADER_HEIGHT - notesHeight;
