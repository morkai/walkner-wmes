  ...
  function convertNext()
  {
    ...
    step(
      ...
      function ensureDirStep(err, todo)
      {
        ...
        this.todo = todo;
        this.tmpDirPath = path.join(module.config.uploadsDest, '.tmp');
        this.sourceFilePath = path.join(this.tmpDirPath, todo._id);
        this.targetDirPath = path.join(module.config.uploadsDest, todo.nc15, todo._id);
        this.targetFilePath = path.join(this.targetDirPath, `${this.todo.nc15}.tmp`);

        OrderDocumentFile.findById(todo.nc15).lean().exec(this.next());
      },
      function(err, orderDocumentFile)
      {
        if (err)
        {
          return this.skip(err);
        ...
        const file = orderDocumentFile.files.find(f => f.hash === this.todo._id);
        ...
        if (!file)
        {
          return this.skip(app.createError(
            ...
            'NOT_FOUND',
            500,
            {nc15: this.todo.nc15, hash: this.todo._id}
          ));
        }
        ...
        if (!this.ext)
        {
          return this.skip(app.createError(
            ...
            'INVALID_TYPE',
            500,
            {nc15: this.todo.nc15, hash: this.todo._id, type: file.type}
          ));
        }

        this.targetFilePath = path.join(this.targetDirPath, `${this.todo.nc15}.${this.ext}`);

        if (this.todo.count === 0)
        {
          fs.copy(this.targetFilePath, this.sourceFilePath, this.next());
        }
      },
      function(err)
      {
        ...
        fs.pathExists(this.sourceFilePath, this.next());
      },
      function ensureDirStep(err, sourceExists, targetExists)
      {
        if (!sourceExists && targetExists)
        {
          sourceExists = true;
          this.sourceFilePath = this.targetFilePath;
        }
      ...
      function moveStep(err)
      {
        ...
        fs.move(this.sourceFilePath, this.targetFilePath, this.next());
      },
      ...
      function(err, tryNext)
      {
        if (err)
        {
          fs.emptyDir(
            this.targetDirPath,
            () => fs.copy(this.sourceFilePath, this.targetFilePath, () => {})
          );
