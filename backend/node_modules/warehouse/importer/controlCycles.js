...
exports.models = [
  ...
  require('../models/whControlCycleArchive')
];
...
exports.start = function startControlCyclesImporterModule(app, module)
{
  ...
  function queueFile(fileInfo)
  {
    ...
    module.info('Queued %s...', fileInfo.timeKey);
  }
  ...
  function importNext(fileInfo)
  {
    ...
    module.info('Importing %s...', fileInfo.timeKey);
    ...
    importFile(fileInfo, function(err, controlCycles)
    {
      ...
      {
        module.info('Imported %d of %s in %d ms!', controlCycles.length, fileInfo.timeKey, Date.now() - startTime);
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      function parseFileStep(err, fileContents)
      {
        ...
        module.info('Parsing ~%d bytes of %s...', fileContents.length, fileInfo.timeKey);

        const t = Date.now();

        this.controlCycles = parseControlCycles(fileContents, new Date(fileInfo.timestamp));

        module.info('Parsed %s in %d ms!', fileInfo.timeKey, Date.now() - t);
