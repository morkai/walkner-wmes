...
exports.report = (app, {Setting, ProdShift, ProdShiftOrder, CtMrpConfig}, options, done) =>
{
  const results = {
    options,
    lines: {},
  ...
  step(
    ...
    function()
    {
      ...
      ProdShift.aggregate([
        {$match: {
          date: {
            $gte: moment(options.fromTime).subtract(days, 'days').hours(6).toDate(),
    ...
    function(err, settings, configs, lineMrps)
    {
      if (err)
      {
      ...
      if (lineMrps.length)
      {
        lineMrps.forEach(({_id, mrp}) =>
        {
          this.lineMrps.set(_id, mrp);
        });
      }
      ...
      if (options.includedMrps.length)
      {
        conditions['orderData.mrp'] = {$in: options.includedMrps};
      }
      else if (options.ignoredMrps.length)
      {
        conditions['orderData.mrp'] = {$nin: options.ignoredMrps};
      }
  ...
  function handleShiftOrder(shiftOrder)
  {
    ...
    const mrpConfig = mrpConfigs.get(mrp._id);
  ...
  function isUpphOrder(shiftOrder)
  {
    if (options.minUpphWorkDuration && shiftOrder.workDuration <= (options.minUpphWorkDuration / 60))
    {
      return false;
    }

    const ops = Object.values(shiftOrder.orderData && shiftOrder.orderData.operations);

    ops.sort((a, b) => b.laborTime - a.laborTime);

    return ops[0].no === shiftOrder.operationNo;
  }
  ...
  function countAllLines(lineMrps, usedLines, includedMrps, excludedMrps)
  {
    ...
    if (includedMrps.length)
    {
      allLineCount += countIncludedMrps(lineMrps, includedMrps);
    }
    else if (excludedMrps.length)
    {
  ...
  function countIncludedMrps(lineMrps, includedMrps)
  {
    let count = 0;

    lineMrps.forEach((mrps) =>
    {
      count += includedMrps.some(mrp => mrps.includes(mrp)) ? 1 : 0;
    });
  ...
  function countExcludedMrps(lineMrps, excludedMrps)
  {
    ...
    lineMrps.forEach((mrps) =>
    {
      count += mrps.some(mrp => !excludedMrps.includes(mrp)) ? 1 : 0;
    });
