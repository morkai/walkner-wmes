...
exports.report = (app, module, options, done) =>
{
  ...
  step(
    ...
    function(err)
    {
      ...
      _.forEach(results.upph, (data, mrp) =>
      {
        const upphList = [];

        _.forEach(data, (upph, key) =>
        {
          ...
          if (std || norm)
          {
            upphList.push({_id: +key, std, norm});
          }
        });

        upphList.sort((a, b) => a._id - b._id);
  ...
  function calcUpph()
  {
    ...
    {
      let ordersWithData = shiftOrders.filter(o =>
      {
        return !!o.orderData
          && !!o.orderData.operations
          && !!o.orderData.scheduledStartDate
          && !!o.orderData.qty;
      });

      if (!ordersWithData.length)
      {
        return;
      }

      const scheduledStartDate = new Date(ordersWithData[0].orderData.scheduledStartDate);
      const scheduledStartTime = scheduledStartDate.getTime() + 6 * 3600 * 1000;
      const assemblyOp = Object
        .values(ordersWithData[0].orderData.operations)
        .sort((a, b) => b.laborTime - a.laborTime)[0];
      const diffDays = scheduledStartDate.getDay() === 1 ? 5 : 7;
      const maxTimeDiff = diffDays * 24 * 3600 * 1000 + 8 * 3600 * 1000;

      ordersWithData = ordersWithData.filter(o => Math.abs(o.date - scheduledStartTime) <= maxTimeDiff);

      const assemblyOrders = ordersWithData
        .filter(o => o.operationNo === assemblyOp.no)
        .sort((a, b) => b.quantityDone - a.quantityDone);
      ...
      const assemblyOrder = assemblyOrders[0];
      const {mrp, qty} = assemblyOrder.orderData;
      const opDurations = {};
      let workerCount = 0;
      ...
      ordersWithData.forEach(o =>
      {
        ...
        if (!opDurations[o.operationNo])
        {
          opDurations[o.operationNo] = 0;
        }

        opDurations[o.operationNo] += o.workDuration;
      });

      const workDuration = opDurations[Object.keys(opDurations).sort((a, b) => opDurations[b] - opDurations[a])[0]];

      if (!workDuration
        || (options.minUpphWorkDuration && workDuration <= (options.minUpphWorkDuration / 60)))
      {
        return;
      }

      const date = moment(assemblyOrder.date);
      const key = util.createGroupKey(options.interval, date, false);

      if (!results.upph.total[key])
      {
        results.upph.total[key] = [0, 0, 0, 0];
      }
      ...
      if (!results.upph[mrp][key])
      {
        results.upph[mrp][key] = [0, 0, 0, 0];
      }
      ...
      results.upph.total[key][0] += num;
      results.upph.total[key][1] += 1;
      results.upph.total[key][2] += num * coeff;
      results.upph.total[key][3] += 1;
      results.upph[mrp][key][0] += num;
      results.upph[mrp][key][1] += 1;
      results.upph[mrp][key][2] += num * coeff;
      results.upph[mrp][key][3] += 1;
    });
  }
