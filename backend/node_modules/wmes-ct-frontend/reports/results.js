...
exports.report = (app, module, options, done) =>
{
  ...
  step(
    ...
    function(err)
    {
      ...
      results.allLineCount = countAllLines(this.lineMrps, results.lines, options.ignoredMrps);
      ...
      _.forEach(results.upphNorm, (upph, week) =>
      {
        upph = _.round(upph[0] / upph[1], 2);

        if (upph)
        {
          upphNormList.push({_id: week, upph});
        }
      });

      upphList.sort(sortById);
      upphNormList.sort(sortById);

      results.upph = upphList;
      results.upphNorm = upphNormList;
  ...
  function handleShiftOrder(shiftOrder)
  {
    ...
    if (isUpphOrder(shiftOrder))
    {
      ...
      if (!results.upph[week])
      {
        ...
        results.upphNorm[week] = [0, 0];
      }

      const quarter = date.utc().startOf('quarter').valueOf();
      const coeff = upphCoeffMap.get(`${quarter}:${mrp._id}`) || upphCoeffMap.get(`${quarter}:*`) || 1;

      results.upph[week][0] += shiftOrder.quantityDone / shiftOrder.workerCount / shiftOrder.workDuration;
      results.upph[week][1] += 1;
      results.upphNorm[week][0] += shiftOrder.quantityDone * coeff / shiftOrder.workerCount / shiftOrder.workDuration;
      results.upphNorm[week][1] += 1;
    }
