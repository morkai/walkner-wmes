...
module.exports = (app, module) =>
{
  ...
  const dictionaries = {
    ...
    lines: {
      model: CtLine,
      prepareReadResult: assignCarts
    },
    orderGroups: {
      model: CtOrderGroup,
      canManage: user.auth('CT:MANAGE:ORDER_GROUPS'),
      prepareReadResult: assignComponentNames
    },
    mrpConfigs: {
      model: CtMrpConfig,
  ...
  express.get('/ct-balancing*', serveBalancingRoute);
  ...
  function assignCarts(ctLine, formatResult, req)
  {
    if (!req.query.carts)
    {
  ...
  function assignComponentNames(orderGroup, formatResult)
  {
    const codes = new Set();

    orderGroup.bomInclude.concat(orderGroup.bomExclude).forEach(patterns =>
    {
    ...
    if (codes.size === 0)
    {
      return formatResult(null, orderGroup);
    }

    Component.find({_id: {$in: Array.from(codes)}}).select({name: 1}).lean().exec((err, components) =>
    {
      if (err)
      {
        return formatResult(err);
      ...
      orderGroup.componentNames = {};

      components.forEach(component =>
      {
        orderGroup.componentNames[component._id] = component.name;
      });

      formatResult(null, orderGroup);
    });
  }

  function serveBalancingRoute(req, res, next)
  {
    if (!/\.(js|html|css|json|png|ico|map)$/.test(req.url))
    {
      req.url = '/ct-balancing/index.html';
    }
