...
const logEntryHandlers = require('production/logEntryHandlers');

exports.name = 'ProdLogEntry';
...
exports.setUp = (app, mongoose) =>
{
  ...
  prodLogEntrySchema.statics.editShift = function(prodShift, creator, changes)
  {
    ...
    {
      const props = ['actual', 'planned'];

      data.quantitiesDone = changes.quantitiesDone.map(quantityDone => _.pick(quantityDone, props));
    ...
    if (Array.isArray(changes.comments))
    {
      const props = ['date', 'user', 'text'];
      ...
      for (let i = 0; i < 8; ++i)
      {
        const comment = {
          ...
          user: null,
          text: '',
          ..._.pick(changes.comments[i], props)
        };
        ...
        data.comments.push(comment.user && comment.date ? comment : null);
      }

      compareProperty(data, modelData, 'comments');
    }
    else if (changes.comment && changes.comment.i >= 0 && changes.comment.i < 8 && _.isString(changes.comment.text))
    {
      const oldComments = modelData.comments || [];
      const newComments = [];

      for (let i = 0; i < 8; ++i)
      {
        if (i === changes.comment.i)
        {
          newComments.push({
            date: new Date(),
            user: creator,
            text: changes.comment.text
          });
        }
        else
        {
          newComments.push(oldComments[i] || null);
        }
      }
  ...
  mongoose.model(exports.name, prodLogEntrySchema);
};
