...
exports.setUp = (app, mongoose) =>
{
  ...
  schema.statics.TOPIC_PREFIX = 'prodShifts';

  schema.statics.HOUR_TO_INDEX = [
    2, 3, 4, 5, 6, 7, 0, 1,
    2, 3, 4, 5, 6, 7, 0, 1,
    2, 3, 4, 5, 6, 7, 0, 1
  ];

  schema.statics.NO_TO_START_HOUR = {
    1: 6,
    2: 14,
    3: 22
  };
  ...
  schema.index({subdivision: 1, date: -1});
  schema.index({mrpControllers: 1, date: -1});
  schema.index({prodFlow: 1, date: -1});
  schema.index({workCenter: 1, date: -1});
  schema.index({prodLine: 1, date: -1});
  schema.index({shutdown: 1, prodLine: 1});
  schema.index({shutdown: 1, date: -1});
  schema.index({orderMrp: 1, date: -1});
  ...
  schema.statics.setPlannedQuantities = function(prodLineIds, date, plannedQuantities, done)
  {
    ...
    ProdShift
      ...
      .exec((err, prodShifts) =>
      {
        ...
        const dividedQuantities = prodLineIds.map(() => [0, 0, 0, 0, 0, 0, 0, 0]);

        _.forEach(plannedQuantities, (plannedQuantity, hour) =>
        {
          const quantityForProdLine = Math.floor(plannedQuantity / prodLineCount);

          _.forEach(dividedQuantities, quantitiesForProdLine =>
          {
  ...
  schema.methods.recalcOrderData = function(orders)
  {
    let orderMrp = {};
    let effNum = 0;
    let effDen = 0;
    ...
    {
      if (order.quantityDone && order.orderData && order.orderData.mrp)
      {
      ...
      {
        effNum += order.laborTime * order.getTaktTimeCoeff() / 100 * order.totalQuantity;
  ...
  schema.methods.recalcTimes = function(done)
  {
    const shift = this;
  ...
  schema.methods.getNextOrders = function()
  {
