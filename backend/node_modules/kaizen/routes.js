...
module.exports = (app, module) =>
{
  ...
  const canViewDictionaries = user.auth('KAIZEN:DICTIONARIES:VIEW');
  const canManageDictionaries = user.auth('KAIZEN:DICTIONARIES:MANAGE');
  ...
  express.get(
    '/kaizen/settings',
    canView,
    settings.browseRoute('kaizen')
  );
  express.put('/kaizen/settings/:id', canManageDictionaries, settings.updateRoute);

  express.get('/kaizen/recalcDurations', user.auth('SUPER'), recalcDurationsRoute);
  ...
  express.post('/kaizen/orders', canView, prepareForAdd, express.crud.addRoute.bind(null, app, {
    model: KaizenOrder,
    prepareMessage: prepareAddedMessage
  }));
  ...
  express.get('/kaizen/orders;export.:format?', canView, fetchDictionaries, express.crud.exportRoute.bind(null, app, {
    ...
    columns: {
      ...
      causeId: 10,
      riskId: 10,
      correctiveMeasures: 40,
      preventiveMeasures: 40,
      stdReturn: 'boolean',
      relatedSuggestion: {
        type: 'integer',
        width: 5
  ...
  express.get(
    '/kaizen/report',
    canView,
    reports.helpers.sendCachedReport.bind(null, 'kaizen/count'),
    countReportRoute
  );
  express.get(
    '/kaizen/reports/summary',
    canView,
    reports.helpers.sendCachedReport.bind(null, 'kaizen/summary'),
    summaryReportRoute
  );
  express.get(
    '/kaizen/reports/metrics',
    canView,
    reports.helpers.sendCachedReport.bind(null, 'kaizen/metrics'),
    metricsReportRoute
  );
  ...
  function dictionariesRoute(req, res, next)
  {
    step(
      function findStep()
      {
        ...
        settings.find({_id: /^kaizen/}, this.group());
      },
      function sendResultStep(err, dictionaries)
      {
        ...
        const result = {
          ...
          nmStatuses: KaizenOrder.STATUSES,
          kzStatuses: Suggestion.STATUSES,
  ...
  function prepareAddedMessage(message, {body})
  {
    const {fm24} = body;

    if (!_.isPlainObject(fm24)
      || fm24.confirmation !== '1'
      || !_.isString(fm24.email)
      || !fm24.email.includes('@'))
    {
      return;
    }
  ...
  function findByRidRoute(req, res, next)
  {
    ...
    KaizenOrder.findOne({rid: rid}, {_id: 1}).lean().exec((err, kaizenOrder) =>
    {
      if (err)
      {
        return next(err);
  ...
  {
    KaizenOrder.findOne({rid: parseInt(req.params.rid, 10)}, {_id: 1}).lean().exec((err, kaizenOrder) =>
    {
      if (err)
      {
        return next(err);
  ...
  function editKaizenOrderRoute(req, res, next)
  {
    ...
    _.forEach(newAttachmentList, (attachment) =>
    {
  ...
  {
    KaizenOrder.findById(req.params.order, {attachments: 1, changes: 1}).lean().exec((err, kaizenOrder) =>
    {
      if (err)
      {
        return next(err);
      ...
      if (!isNaN(changeIndex))
      {
        ...
        {
          attachment = _.find(change.data.attachments[changeIndex < 0 ? 0 : 1], (attachment) =>
          {
  ...
  function mergeAttachments(kaizenOrder, newAttachmentMap)
  {
    ...
    _.forEach(kaizenOrder.attachments, (oldAttachment) =>
    {
    ...
    _.forEach(newAttachmentMap, (newAttachment) =>
    {
      attachments.push(newAttachment);
    });
  ...
  function fetchDictionaries(req, res, next)
  {
    ...
    step(
      ...
      function sendResultStep(err, dictionaries)
      {
        ...
        _.forEach(Object.keys(module.DICTIONARIES), (dictionaryName, i) =>
        {
          req.kaizenDictionaries[dictionaryName] = {};

          _.forEach(dictionaries[i], (dictionaryModel) =>
          {
  ...
  {
    const multiType = module.config.multiType;
    ...
    result.stdReturn = doc.stdReturn;
    result.relatedSuggestion = doc.relatedSuggestion;
  ...
  function metricsReportRoute(req, res, next)
  {
    ...
    const options = {
      fromTime: reports.helpers.getTime(query.from) || null,
      toTime: reports.helpers.getTime(query.to) || null,
      interval: reports.helpers.getInterval(query.interval, 'none'),
      sections: _.isEmpty(query.sections) ? [] : query.sections.split(',')
    };
