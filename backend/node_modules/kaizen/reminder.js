...
module.exports = (app, module) =>
{
  ...
  const {mongoose, mailSender, User, KaizenOrder} = module;
  ...
  {
    const delay = moment().hours(12).startOf('hour').add(1, 'days').diff(Date.now());
  ...
  function remind()
  {
    step(
      ...
      function getUserDataStep(err, remindedAtResults, updatedAtResults)
      {
        ...
        _.forEach(updatedAtResults, (result) =>
        {
        ...
        _.forEach(remindedAtResults, (result) =>
        {
      ...
      function sendEmailsStep(err, users)
      {
        ...
        logger.info(`Reminded [${users.length}] users about incomplete orders.`);
      },
  ...
  function sendReminderEmail(user, stats)
  {
    const mailOptions = {
      to: user.email,
      replyTo: user.email,
      subject: `[WMES] [ZPW] Niezakończone zgłoszenia`,
      html: renderEmail({
    ...
    mailSender.send(mailOptions, (err) =>
    {
      if (err)
      {
        module.error(err, `Failed to remind about incomplete near misses.`, {recipients: mailOptions.to});
      }
    });
  }
