...
exports.report = (app, {User, KaizenOrder}, options, done) =>
{
  ...
  step(
    function findKaizenOrdersStep()
    {
      ...
      {
        conditions.area = options.areas[0];
      }
      else if (options.areas.length)
      {
        conditions.area = {$in: options.areas};
      }
  ...
  function handleKaizenOrder(ko)
  {
    ...
    _.forEach(ko.types, (type) => inc('type', type));
    ...
    {
      ko[prop].forEach(owner =>
      {
        companies.add(owner.company);
      });
    });

    companies.forEach(company => inc('company', company));
  ...
  function sortOwnerTotals()
  {
    ...
    _.forEach(results.totals.owner, (values, key) =>
    {
      totals.push([key, values.total, values.nearMiss || 0, values.suggestion || 0, values.kaizen || 0]);
    });

    results.totals.owner = totals.sort((a, b) => b[1] - a[1]);
  }
};
