...
module.exports = (app, module) =>
{
  ...
  function recountStats()
  {
    ...
    step(
      ...
      function(err)
      {
        if (err)
        {
          logger.error(err, 'Failed to recount.');
        }
        else
        {
          logger.info('Recounted.', {duration: Date.now() - t});
  ...
  function countEntries(userId, done)
  {
    ...
    pipeline.push({$group: {
      ...
      openCount: {$sum: {
        $cond: {
          if: {$in: ['$status', ['new', 'accepted', 'todo', 'inProgress', 'verification', 'paused']]},
          then: 1,
          else: 0
        }
      }},
