...
module.exports = function(app, productionModule, done)
{
  ...
  step(
    ...
    function countPressWorksheetsStep()
    {
      ...
      mongoose.model('PressWorksheet').countDocuments({}).exec(this.next());
    },
    function recreatePressWorksheetDataStep(err, allPressWorksheetCount)
    {
      ...
      stream.on('data', function(pressWorksheet)
      {
        pressWorksheet.createOrdersAndDowntimes(function(err)
        {
          if (err)
          {
            productionModule.error(err, 'Failed to recreate the press worksheet.', {
              pressWorksheetId: pressWorksheet._id
            });
          }
    ...
    function handleRecreatePressWorksheetDataResultStep(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to recreate the press worksheets.');
    ...
    function recreateProdLogEntryDataStep()
    {
      ...
      mongoose.model('ProdLogEntry').aggregate(
        ...
        function(err, results)
        {
          ...
          productionModule.info('Recreating the prod log entries...', {
            from: formatDate(fromMoment.toDate()),
            to: formatDate(maxTime)
          });
    ...
    function finishRecreatingStep(err)
    {
      ...
      if (err)
      {
        productionModule.error(err, 'Failed to recreate the production data.');
      }
      else
      {
        productionModule.info('Finished recreating the production data!', {
  ...
  function createHandleWeekOfLogEntriesStep(from, to)
  {
    ...
    {
      const week = formatDate(from);

      productionModule.info(`Recreating week ${week}...`);
      ...
      mongoose.model('ProdLogEntry').updateMany(conditions, update, err =>
      {
        if (err)
        {
          productionModule.error(err, `Failed to reset todo flag of log entries for week ${week}.`);
        ...
        productionModule.handleLogEntries(function(err)
        {
          ...
          if (err)
          {
            productionModule.error(err, `Failed to handle log entries for week ${week}.`);
          }
