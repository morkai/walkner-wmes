...
exports.mapTaktTimeCoeffs = mapTaktTimeCoeffs;
...
function prepareSettings(module, orderList, done)
{
  ...
  step(
    ...
    function(err, dateToPlanSettings, productionSettings)
    {
      ...
      const taktTimeMrpCoeffs = mapTaktTimeCoeffs(productionSettings['taktTime.coeffs']);
...
// Check frontend/app/production/ProductionSettingCollection.js#mapTaktTimeCoeffs
function mapTaktTimeCoeffs(input)
{
  const mrpCoeffs = {};
  ...
  {
    const re = /([A-Z0-9]+[A-Z0-9_\- ]*|\*)(\/[0-9]{4})?\s*=\s*([0-9]+(?:[.,][0-9]+)?)/ig;
    const wcCoeffs = {};
    let remaining = line;
    let matchCount = 0;
    let match = null;
