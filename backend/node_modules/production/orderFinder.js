...
function findOrdersByNo(Order, no, done)
{
  ...
  query.select({changes: 0, importTs: 0, __v: 0}).lean().exec((err, sapOrders) =>
  {
    if (err)
    {
      return done(err);
    ...
    else if (!Array.isArray(sapOrders))
    {
      sapOrders = [sapOrders];
    }
...
function prepareSettings({settingsCache, PlanSettings}, orderList, done)
{
  ...
  step(
    ...
    function(err, dateToPlanSettings, productionSettings)
    {
      ...
      const taktTimeMrpCoeffs = parseTaktTimeCoefffs(productionSettings['taktTime.coeffs']);
      ...
      dateToOrdersMap.forEach((orders, date) =>
      {
        ...
        orders.forEach(order =>
        {
          ...
          order.taktTimeCoeff = taktTimeMrpCoeffs[order.mrp] || taktTimeMrpCoeffs['*'] || {'*': 1};
        });
      });
...
function parseTaktTimeCoefffs(input)
{
  const mrpCoeffs = {};
  ...
  {
    const re = /([A-Z0-9]+[A-Z0-9_\- ]*|\*)\s*=\s*([0-9]+(?:[.,][0-9]+)?)/ig;
    const wcCoeffs = {};
    let remaining = line;
    let matchCount = 0;
    let match = null;
    ...
    const mrp = remaining.indexOf('*') === -1 ? remaining.split(/[^A-Za-z0-9]/)[0].toUpperCase() : '*';
