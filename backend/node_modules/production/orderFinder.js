  ...
  return {
    findOrderByNo: (orderNo, done) =>
    {
      findOrdersByNo(Order, orderNo, (err, orders) =>
      {
        ...
        prepareSettings(module, [order], err =>
        {
          if (err)
          {
            return done(err);
...
{
  if (Array.isArray(nc12) && nc12.length)
  {
    return findOrdersByNc12s(Order, MechOrder, nc12, done);
  }
...
function findOrderByNc12(Order, MechOrder, nc12, done)
{
  const query = Order
    .find({
      nc12,
      scheduledStartDate: {$gte: getTwoWeeksAgo()},
      statuses: {
        $in: ['REL'],
        $nin: Order.DELETED_STATUSES
      }
    })
...
function findOrdersByNc12s(Order, MechOrder, nc12, done)
{
  const pipeline = [
    {$match: {
      nc12: {$in: nc12},
  ...
  Object.keys(MECH_ORDER_FIELDS).forEach(prop =>
  {
    pipeline[2].$group[prop] = {$first: `$${prop}`};
  });

  Order.aggregate(pipeline).exec((err, orders) =>
  {
    if (err)
    {
      return done(err);
...
function prepareSettings(module, orderList, done)
{
  ...
  const {settingsCache, PlanSettings, Plan, PlanOrderGroup} = module;
  ...
  step(
    function()
    {
      const localDates = Array.from(dateToOrdersMap.keys());
      const planIds = localDates.map(
        localDate => moment.utc(moment(localDate).format('YYYY-MM-DD'), 'YYYY-MM-DD').toDate()
      );

      findOrderGroups({Plan, PlanOrderGroup}, planIds, Array.from(noToOrderMap.keys()), this.parallel());
      findPlanSettings(PlanSettings, planIds, this.parallel());

      setImmediate(this.parallel(), null, {
        'taktTime.coeffs': settingsCache['taktTime.coeffs']
      });
    },
    function(err, orderToGroups, dateToPlanSettings, productionSettings)
    {
      if (err)
      {
        return done(err);
      ...
      dateToOrdersMap.forEach((orders, date) =>
      {
        ...
        orders.forEach(order =>
        {
          ...
          order.taktTimeCoeff = getTaktTimeCoeffsForOrder(taktTimeCoeffs, order);
          order.planOrderGroups = orderToGroups.get(order._id) || {};
        });
      });
...
async function findOrderGroups({Plan, PlanOrderGroup}, planIds, orders, done)
{
  try
  {
    const result = await Plan.aggregate([
      ...
      {$unwind: '$lines.orders'},
      {$match: {
        'lines.orders.orderNo': {$in: orders}
      }},
      {$project: {
        line: '$lines._id',
        order: '$lines.orders.orderNo',
        group: '$lines.orders.group'
      }},
      {$match: {
        group: {$nin: [null, new ObjectId('000000000000000000000000')]}
      }}
    ]);
    ...
    result.forEach(r =>
    {
      orderGroups.set(r.group.toString(), r.group);
    });

    const orderGroupNames = await PlanOrderGroup
      .find({_id: Array.from(orderGroups.values())})
      .select({name: 1})
      .lean()
      .exec();

    orderGroupNames.forEach(g =>
    {
      orderGroups.set(g._id.toString(), g.name);
    });

    const orderToGroups = new Map();

    result.forEach(r =>
    {
      if (!orderToGroups.has(r.order))
      {
        orderToGroups.set(r.order, {});
      }

      const groupId = r.group.toString();

      orderToGroups.get(r.order)[r.line] = {
        _id: groupId,
        name: orderGroups.get(groupId) || groupId
      };
    });

    done(null, orderToGroups);
  }
  catch (err)
  {
    done(err);
...
function findPlanSettings(PlanSettings, planIds, done)
{
  const conditions = {
    _id: {$in: planIds}
  };
