...
module.exports = function unlockCommand(app, productionModule, socket, req, reply)
{
  ...
  step(
    ...
    function updateSecretKeyStep(err, secretBytes)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to generate a secret key.', {prodLineId: this.prodLine._id});
    ...
    function fetchProdShiftStep(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to save a secret key.', {prodLineId: this.prodLine._id});
    ...
    function fetchDictionaries(err, prodShift, prodShiftOrder, prodDowntimes)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to fetch prod data after unlock.', {prodLineId: this.prodLine._id});
