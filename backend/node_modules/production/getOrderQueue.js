  ...
  step(
    ...
    function(err, prodDowntimes, prodShiftOrders)
    {
      ...
      prodShiftOrders.forEach(pso =>
      {
        ...
        execution.doneOrders.push({
          ...
          productCode: pso.orderData.nc12,
      ...
      PlanSettings.aggregate([
        ...
        {$project: {_id: 0, mrps: '$lines.mrpPriority', workerCount: '$lines.workerCount'}}
      ], this.parallel());
    ...
    function(err, settings, lineOrders)
    {
      ...
      settings.forEach(settings =>
      {
        ...
        {
          this.workerCountMap[mrp] = settings.workerCount;
        });
      });
      ...
      Plan.aggregate([
        ...
        {$project: {
          _id: '$orders._id',
          operationNo: '$orders.operation.no',
          nc12: '$orders.nc12',
          name: '$orders.name'
        }}
      ], this.parallel());
      ...
      if (options.prev)
      {
        ...
        FapEntry
          .find({orderNo: {$in: orderIds}})
          .select(arrayToObject(module.FAP_PROPS, 1))
          .sort({createdAt: 1})
          .lean()
          .exec(this.parallel());
      }
    ...
    function(err, planOrders, sapOrders, prodShiftOrders, fapList)
    {
      if (err)
      {
      ...
      const fapMap = new Map();

      fapList.forEach(fapEntry =>
      {
        if (!fapMap.has(fapEntry.orderNo))
        {
          fapMap.set(fapEntry.orderNo, []);
        }

        fapMap.get(fapEntry.orderNo).push(fapEntry);

        fapEntry.orderNo = undefined;
      });
      ...
      planOrders.forEach(planOrder =>
      {
        this.lineOrdersMap[planOrder._id].forEach(lineOrder =>
        {
          ...
          lineOrder.nc12 = planOrder.nc12;
          lineOrder.name = planOrder.name;
        });
      });
      ...
      this.lineOrdersList.forEach(o =>
      {
        execution.todoOrders.push({
          shift: shifts.getShiftNo(o.startAt, true),
          orderId: o._id,
          productName: o.name,
          productCode: o.nc12,
        ...
        {
          this.startedOrdersMap[o._id] = true;
