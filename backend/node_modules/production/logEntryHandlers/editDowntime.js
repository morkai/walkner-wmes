...
module.exports = function(app, productionModule, prodLine, logEntry, done)
{
  ...
  function changeOrderIfNecessary(prodDowntime, done)
  {
    step(
      ...
      function checkTimesStep(err, oldProdShiftOrder)
      {
        if (err)
        {
          productionModule.error(err, 'Failed to find old order while editing downtime.', {
            prodShiftOrderId: changes.prodShiftOrder,
            prodDowntimeId,
        ...
        {
          productionModule.warn('Old order not found while editing downtime.', {
            prodShiftOrderId: changes.prodShiftOrder,
            prodDowntimeId,
            prodLogEntryId: logEntry._id
          });
      ...
      function findNewOrderStep(err, allProdShiftOrders)
      {
        if (err)
        {
          productionModule.error(err, 'Failed to find all orders while editing downtime.', {
            prodDowntimeId,
            prodLogEntryId: logEntry._id
          });
  ...
  step(
    ...
    function getProdShiftOrderModelStep(err, prodDowntime)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to find downtime to edit.', {
          prodDowntimeId,
          prodLogEntryId: logEntry._id
        });
    ...
    function handleRecalcDurationsResultStep(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to recalc order durations after editing downtime.', {
          oldProdShiftOrderId: this.oldProdShiftOrder ? this.oldProdShiftOrder._id : null,
          newProdShiftOrderId: this.newProdShiftOrder ? this.newProdShiftOrder._id : null,
          prodDowntimeId,
          prodLogEntryId: logEntry._id
        });
      }
