...
module.exports = function(personnelProperty)
{
  return function(app, productionModule, prodLine, logEntry, done)
  {
    ...
    {
      const personnelId = logEntry.data.label;

      if (typeof personnelId !== 'string' || personnelId.length === 0)
      {
        logEntry.data = null;
      ...
      User
        .findOne({personnelId}, {_id: 1, firstName: 1, lastName: 1})
        .lean()
        .exec(function(err, user)
        {
          ...
          if (!user)
          {
            productionModule.warn(`Couldn't find a ${personnelProperty} by personnel ID.`, {
              personnelId,
              prodShiftId: logEntry.prodShift,
              prodLogEntryId: logEntry._id
            });
