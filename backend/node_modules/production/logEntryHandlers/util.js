...
exports.mapOperations = function(operations)
{
  return prepareOperations({operations}).operations;
};
...
{
  productionModule.orderFinder.findOrdersByNc12(logEntry.data.orderId, function(err, mechOrders)
  {
    if (err)
    {
...
{
  productionModule.orderFinder.findOrdersByNo(logEntry.data.orderId, function(err, orders)
  {
    if (err)
    {
...
function prepareOperations(orderData)
{
  if (Array.isArray(orderData.operations))
  {
    ...
    orderData.operations.forEach(operation =>
    {
      if (operation.workCenter === '' || operation.laborTime === -1 || operations[operation.no])
      {
        return;
      }
