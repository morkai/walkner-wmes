...
module.exports = function(app, productionModule, prodLine, logEntry, done)
{
  ...
  step(
    ...
    function(err, prodShiftOrder)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to get prod shift order to change the quantity done.', {
      ...
      if (!prodShiftOrder)
      {
        productionModule.warn(`Couldn't find prod shift order to change the quantity done.`, {
          prodShiftOrderId: logEntry.prodShiftOrder,
          prodLogEntryId: logEntry._id
        });
    ...
    function(err, prodShiftOrder)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to save prod shift order after changing the quantity done.', {
          prodShiftOrderId: logEntry.prodShiftOrder,
          prodLogEntryId: logEntry._id
        });
    ...
    function(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to recount the total quantity done after changing the quantity done.', {
          prodShiftOrderId: logEntry.prodShiftOrder,
          prodLogEntryId: logEntry._id
        });
