...
module.exports = function(app, productionModule, prodLine, logEntry, done)
{
  ...
  step(
    ...
    function updateDowntimesStep(err, prodShiftOrder, prodDowntimes)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to save a new order.', {
    ...
    function recalcOrderDurationsStep(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to update downtimes after adding order.', {
          prodShiftOrderId: logEntry.data._id,
          prodLogEntryId: logEntry._id
        });
    ...
    function handleRecalcDurationsResultStep(err)
    {
      if (err)
      {
        productionModule.error(err, 'Failed to recalc durations after adding order.', {
