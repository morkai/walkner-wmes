...
{
  if (currentShift.no === 1 && this.qi.count > 0)
  {
    this.qi.ok = null;
    this.qi.count = 0;

    this.publishOrMergeChanges({qi: this.qi});
  }
};
...
ProdLineState.prototype.update = function(newData, options)
{
  ...
  step(
    ...
    function(err)
    {
      ...
      if (newData.online)
      {
        prodLineState.updateQi(this.parallel());
      }
    },
    function(err)
    {
...
ProdLineState.prototype.checkExtendedDowntime = function()
{
  ...
  const delay = (this.stateChangedAt + this.productionModule.settings.extendedDowntimeDelay * 60 * 1000) - Date.now();
 ...
 * @returns {undefined}
 */
ProdLineState.prototype.updateQi = function(done)
{
  if (!done)
  {
    done = () => {};
  ...
  const pls = this;
  const {QiResult, settings} = pls.productionModule;
  ...
  step(
    function()
    {
      const $match = {
        ...
        inspectedAt: {$gte: new Date(now - 32 * 3600 * 1000)},
        createdAt: {$gte: new Date(shifts.getFirstShiftStartTime(now))}
      };

      if (Array.isArray(settings.qiKinds) && settings.qiKinds.length)
      {
        $match.kind = {$in: settings.qiKinds.map(id => new ObjectId(id))};
      }
    ...
    function(err, result)
    {
      ...
      const qi = result.length ? result[0] : {
        ok: null,
        count: 0
      };

      if (!jsonDeepEqual(qi, pls.qi))
      {
        pls.qi = qi;

        pls.publishOrMergeChanges({qi});
      }
