...
module.exports = function setUpActiveProdLines(app, productionModule)
{
  ...
  {
    productionModule.info('Removing active prod line data...');
  ...
  function aggregateActiveProdLines(prodFlowId, shiftDate, done)
  {
    ...
    mongoose.model('ProdShiftOrder').aggregate(
      ...
      function(err, results)
      {
        if (err)
        {
          productionModule.error(err, 'Failed to aggregate active prod lines.');
  ...
  function activateProdLine(shiftDate, prodLineId, prodFlowId, reactivation)
  {
    if (!shiftDate)
    {
      return productionModule.info(`Tried to activate prod line [${prodLineId}] on an unknown shift.`);
    }
    ...
    if (shiftsDiff < 0)
    {
      return productionModule.info(`Tried to activate prod line [${prodLineId}] on shift [${shiftDate}].`);
    }
    ...
    productionModule.info(`${reactivation ? 'Early reactivated' : 'Activated'} prod line.`, {
      prodLineId,
      shiftDate,
      allProdLinesCount: allActiveProdLines.length,
