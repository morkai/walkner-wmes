...
module.exports = function setUpProdState(app, productionModule)
{
  ...
  productionModule.getProdLineStates = function(done)
  {
    ...
    if (!done)
    {
      ...
      {
        productionModule.getProdLineState(() => resolve());
      });
    }
  ...
  app.broker.subscribe('production.synced.**', function(changes)
  {
    ...
    if (!prodLineState)
    {
      return productionModule.info('Data synced but no state for prod line...', {prodLineId: changes.prodLine});
    }
  ...
  {
    updateLineInspection(model.line);
  });
  ...
  app.broker.subscribe('qi.results.edited', ({model}) =>
  {
    ...
    if (change.data.line)
    {
      updateLineInspection(change.data.line[0]);
      updateLineInspection(change.data.line[1]);
    }
    else if (change.data.ok)
    {
      updateLineInspection(model.line);
    }
  });
  ...
  {
    console.log('deleted', model.rid, model.line);
    updateLineInspection(model.line);
  });
  ...
  function updateLineInspection(line)
  {
    const pls = productionModule.getProdLineState(line);
    ...
    pls.updateInspection((err) =>
    {
      if (err)
      {
        productionModule.error(err, 'Failed to update line inspection.', {line});
      }
    });
  }
};
