...
module.exports = (app, module, logEntry, components, done) =>
{
  ...
  step(
    ...
    function(err, result)
    {
      ...
      if (logEntry.data.skipMaxCheck)
      {
        setImmediate(this.parallel(), null, null);
      }
      else
      {
        Order.findById(logEntry.data.orderNo).select({qty: 1, qtyDone: 1}).exec(this.parallel());
      }
    },
    function(err, sapOrder)
    {
      ...
      if (sapOrder)
      {
        const qtyTodoTotal = sapOrder.qty;
        const qtyDoneTotal = sapOrder.qtyDone && sapOrder.qtyDone.total || 0;
