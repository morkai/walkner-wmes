...
module.exports = (app, module, logEntry, components, done) =>
{
  ...
  step(
    function()
    {
      ...
      this.locks.push(lockLine(logEntry.prodLine, this.parallel()));
    },
    function(err, result)
    {
      if (err || result)
...
LIMIT 1`;
  ...
  function lockLine(id, cb)
  {
    if (LINE_LOCKS[id])
    {
      LINE_LOCKS[id].push(cb);
    }
    else
    {
      LINE_LOCKS[id] = [];
    ...
    return releaseLine.bind(null, id);
  }

  function releaseLine(id)
  {
    const queue = LINE_LOCKS[id];
