...
module.exports = async (app, module, orderNo) =>
{
  const {
    Order,
    ProdShiftOrder,
  ...
  if (orderNo && !/^[0-9]{9}$/.test(orderNo))
  {
    ...
    {
      result.error = `No active order on line: ${orderNo}`;
  ...
  if (!sapOrder)
  {
    result.error = `Production order not found: ${orderNo}`;
  ...
  {
    const matches = op.name.match(/^LED\s+Tester\s+(.*?)$/i);

    if (matches)
    {
      result.tester = matches[1].trim().toUpperCase();
    }
  });

  const orderRes = await axios.get(`http://prd1.ipt.intra.lighting.com/api/order/${result.orderNo}`, {
    validateStatus: null,
    proxy: false
  });
    ...
    orderRes.data.order_overrides.forEach(override =>
    {
      ...
      if (product)
      {
        ...
        if (product.driverType)
        {
          ...
          result.numberOfDrivers = !result.driverType.includes('_') ? 0 : result.driverType
            .split('_')
  ...
  if (Array.isArray(orderRes.data.operations))
  {
    orderRes.data.operations.forEach(op =>
    {
      ...
      if (op.parameters.data)
      {
        op.parameters = op.parameters.data;
      }
      ...
      else if (op.operation === 'PrintLabel' || op.operation === 'PrintStandardLabel')
      {
  ...
  if (result.ltuneCode)
  {
    ...
    if (itisRes.data.data.DRIVERS_CODE_KEY)
    {
      ...
      result.numberOfDrivers = !result.driverType.includes(':') ? 0 : result.driverType
        .split(':')
  ...
  const pcb = await GftPcb
    .findOne({
      productFamily: {$in: ['ANY', result.productFamily]},
