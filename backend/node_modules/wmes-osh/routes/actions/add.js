...
module.exports = async (app, module, req, res, next) =>
{
  ...
  try
  {
    ...
    else if (action.status === 'verification')
    {
      action.startedAt = action.createdAt;
    ...
    if (Array.isArray(action.implementers))
    {
      exceptUsers.push(action.creator);
      action.implementers.forEach(user => exceptUsers.push(user));
    }

    const [coordinators] = await Promise.all([
      OshAction.resolveCoordinators(action, {exceptUsers}),
