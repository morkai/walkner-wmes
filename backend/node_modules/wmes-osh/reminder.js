...
module.exports = (app, module) =>
{
  ...
  later.setInterval(() => runIfNotDuplicate(handleDailyReminder), later.parse.cron('10 0 * * *'));
  later.setInterval(() => runIfNotDuplicate(handleWeeklyReminder), later.parse.cron('20 0 * * 1'));

  function runIfNotDuplicate(handler)
  {
    const now = Date.now();

    if (handler.lastCalledAt && now - handler.lastCalledAt < 10 * 60 * 1000)
    {
      return;
    }

    handler.lastCalledAt = now;
  ...
  async function handleDailyReminder(retryCounter)
  {
    ...
    try
    {
      ...
      const day = moment(conditions.createdAt.$gte).format('L');
