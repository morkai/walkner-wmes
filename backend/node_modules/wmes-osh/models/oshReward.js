...
exports.setUp = (app, mongoose) =>
{
  ...
  })
  {
    ...
    const coordinator = !!user && user.privileges.includes('OSH:OBSERVATIONS:MANAGE');

    const settings = await Setting.findValues('osh.rewards.observations.');
    const min = coordinator ? 0 : observer ? settings.observerMin : settings.userMin;
    const amount = coordinator || observer ? settings.observer : settings.user;
    ...
    for (const {from, to, rid} of todo)
    {
      ...
      const date = moment.utc(from).format('MMMM YYYY');
      const subject = coordinator
        ? `Nagroda za koordynację obserwacji (${date})`
        : `Nagroda za obserwacje (${date})`;
