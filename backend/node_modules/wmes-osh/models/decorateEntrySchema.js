...
module.exports = (app, mongoose, modelName, schema) =>
{
  ...
  schema.statics.prepareCoordinatorsInput = async function(entry, input, props, options = {})
  {
    const coordinatorsInput = {};

    props.forEach(prop =>
    {
      coordinatorsInput[prop] = input[prop] === undefined ? entry[prop] : input[prop];
    });

    if (props.every(prop => jsonDeepEqual(coordinatorsInput[prop], entry[prop])))
    {
      return;
    }

    input.coordinators = await this.resolveCoordinators(coordinatorsInput, options);
  };

  schema.statics.resolveCoordinators = async function({kind, department, location}, {session, exceptUsers})
  {
    const Entry = this;
    const User = mongoose.model('User');
    const OshKind = mongoose.model('OshKind');
    const OshDepartment = mongoose.model('OshDepartment');
    const OshLocation = mongoose.model('OshLocation');

    const coordinatorSources = await Promise.all([
      kind && kind.length
        ? OshKind.find({_id: {$in: kind}}).select({coordinators: 1}).lean().session(session).exec()
        : null,
      department
        ? OshDepartment.find({_id: department}).select({coordinators: 1}).lean().session(session).exec()
        : null,
      location
        ? OshLocation.find({_id: location}).select({coordinators: 1}).lean().session(session).exec()
        : null
    ]);

    const userIds = new Set();

    coordinatorSources.forEach(sources =>
    {
      if (!sources)
      {
        return;
      ...
      sources.forEach(source =>
      {
        ...
        source.coordinators.forEach(coordinators =>
        {
          if (coordinators.types.length && !coordinators.types.includes(Entry.RELATION_TYPE))
          {
            return;
          }
          ...
          if (kind
            && kind.length
            && coordinators.kinds.length
            && !_.intersection(kind, coordinators.kinds).length)
          {
            return;
          }

          coordinators.users.forEach(user =>
          {
            userIds.add(user.id);
          });
        });
    ...
    if (Array.isArray(exceptUsers))
    {
      exceptUsers.forEach(user => userIds.delete(user.id));
    }
    ...
    return users
      .map(userData => (app['wmes-osh'] || app.user).createUserInfo(userData))
  ...
  schema.methods.applyChanges = function(input, updater, updatedAt)
  {
    ...
    this.updateStatusUpdaters(changes);
  ...
  schema.methods.updateStatusUpdaters = function(changes)
  {
    if (!changes.status)
    {
      return;
    ...
    if (oldStatus === 'new' && newStatus === 'inProgress')
    {
      this.acceptor = this.updater;
    }
    else if (oldStatus === 'inProgress' && newStatus === 'verification')
    {
      this.solver = this.updater;
    }
