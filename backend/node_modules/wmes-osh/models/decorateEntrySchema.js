...
module.exports = (app, mongoose, modelName, schema) =>
{
  ...
  schema.statics.resolveCoordinators = async function({kind, department, location}, {exceptUsers})
  {
    const Entry = this;
    ...
    const coordinatorSources = await Promise.all([
      ...
      department ? OshDepartment.findById(department).select({coordinators: 1}).lean().exec() : null,
      location ? OshLocation.findById(location).select({coordinators: 1}).lean().exec() : null
    ]);
