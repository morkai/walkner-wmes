...
module.exports = (app, mongoose, modelName, schema) =>
{
  ...
  schema.statics.prepareCoordinatorsInput = async function(entry, input, props, options = {})
  {
    ...
    if (props.every(prop => coordinatorsInput[prop] === entry[prop]))
    {
      return;
    }
  ...
  schema.statics.resolveCoordinators = async function({kind, department, location}, {session, exceptUsers})
  {
    ...
    const coordinatorSources = await Promise.all([
      kind ? OshKind.findById(kind).select({coordinators: 1}).lean().session(session).exec() : null,
      department ? OshDepartment.findById(department).select({coordinators: 1}).lean().session(session).exec() : null,
      location ? OshLocation.findById(location).select({coordinators: 1}).lean().session(session).exec() : null
    ]);
    ...
    coordinatorSources.forEach(source =>
    {
      ...
      source.coordinators.forEach(coordinators =>
      {
        ...
        if (kind && coordinators.kinds.length && !coordinators.kinds.includes(kind))
        {
          return;
        }
