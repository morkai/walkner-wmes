...
exports.report = (app, module, options, done) =>
{
  ...
  function finalizeGroup(group)
  {
    ...
    const iprDen = group.fte.total;
    const ipsNum = group.observationCount;
    const ipsDen = iprNum;
    const ipcNum = group.userCount;
    const ipcDen = group.fte.avg;

    group.ipr = util.round(iprNum / iprDen);
    group.ips = util.round(ipsNum / ipsDen * 100);
    group.ipc = Math.min(100, util.round(ipcNum / ipcDen * 100));
  }
  ...
  function handleKaizen(entry)
  {
    ...
    const orgUnits = remapUserOrgUnits(entry.implementers);
  ...
  function handleAction(entry)
  {
    ...
    const orgUnits = remapUserOrgUnits(users);
