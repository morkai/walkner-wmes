...
exports.report = (app, module, options, done) =>
{
  ...
  const TYPE_TO_MODEL = {
    ...
    [OshAccident.RELATION_TYPE]: OshAccident
  };
  ...
  Object.keys(GROUP_PROPERTIES).forEach(prop =>
  {
  ...
  utcGroup = GROUP_PROPERTIES[groupProperty];
  ...
  if (!(selector[groupProperty].$gte instanceof Date))
  {
    selector[groupProperty].$gte = new Date(selector[groupProperty].$gte);
  }

  if (selector[groupProperty].$lt && !(selector[groupProperty].$lt instanceof Date))
  {
    selector[groupProperty].$lt = new Date(selector[groupProperty].$lt);
  }

  delete options.conditions;

  options.fromTime = selector[groupProperty].$gte.getTime();
  options.toTime = selector[groupProperty].$lt ? selector[groupProperty].$lt.getTime() : 0;

  if (GROUP_PROPERTIES[groupProperty])
  {
    selector[groupProperty].$gte = moment(selector[groupProperty].$gte).utc(true).toDate();

    if (selector[groupProperty].$lt)
    {
      selector[groupProperty].$lt = moment(selector[groupProperty].$lt).utc(true).toDate();
    }
  }
  ...
  {
    let date = entry[groupProperty];

    if (GROUP_PROPERTIES[groupProperty])
    {
      date = moment.utc(selector[groupProperty].$gte).local(true).toDate();
    }

    const totals = results.totals;
    const groupKey = util.createGroupKey(options.interval, date, false);
