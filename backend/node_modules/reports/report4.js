...
Report4.prototype.finalizeEffAndProd = function()
{
  ...
  while (fromGroupKey <= toGroupKey)
  {
    ...
    if (groupData)
    {
      ...
      const shifts = Object.keys(groupData.shifts).length > 0 ? 1 : 0;

      effAndProd.eff = groupData.effDen
        ? (groupData.effNum / groupData.effDen)
        : 0;

      effAndProd.prod = prodDen && shifts
        ? (groupData.effNum / (this.options.prodNumConstant * shifts) / prodDen)
        : 0;
...
Report4.prototype.handleEffAndProd = function(prodShiftOrder, workerCount)
{
  ...
  const workDuration = prodShiftOrder.opWorkDuration || prodShiftOrder.workDuration;

  if (workerCount.match)
  {
    groupData.effNum
      += prodShiftOrder.laborTime / 100 * (totalQuantity * workerCount.ratio);
    groupData.effDen += workDuration * workerCount.match;
  ...
  byDivision.effDen = workDuration * prodShiftOrder.operators.length;
};

Report4.prototype.handleOrderWorkTimes = function(prodShiftOrder, ratio)
{
  this.results.workTimes.total += prodShiftOrder.opWorkDuration || prodShiftOrder.workDuration;
