...
exports.report = (app, module, options, done) =>
{
  ...
  function findProdShiftOrdersStep(err)
  {
    ...
    cursor.on('data', pso =>
    {
      ...
      if (pso.orderData
        && pso.orderData.taktTimeCoeff
        && pso.orderData.operations)
      {
        const op = pso.orderData.operations[pso.operationNo];

        if (op)
        {
          const taktTimeCoeff = pso.orderData.taktTimeCoeff[op.workCenter]
            || pso.orderData.taktTimeCoeff['*']
            || 1;

          pso.laborTime *= taktTimeCoeff;
        }
      }
