...
const shiftUtil = require('util/shifts');
...
{
  const {ProdShift, ProdShiftOrder, ProdDowntime} = module;
  ...
  const now = Date.now();
  const currentShiftInfo = shiftUtil.getCurrentShiftInfo(false);
  ...
  function findProdShiftsStep()
  {
    ...
    const cursor = ProdShift.find(conditions).select(fields).lean().cursor();
    ...
    cursor.on('data', prodShift =>
    {
      ...
      for (let ii = 0; ii < 8; ++ii)
      {
        ...
        {
          calcQuantitiesDone(startedAt, prodShift.quantitiesDone[ii].actual);
        }
      }
    });
  }

  function calcQuantitiesDone(startedAt, count)
  {
    if (count === 0)
    {
      return;
    ...
    const groupKey = util.createGroupKey(options.interval, startedAt);

    if (typeof results.coeffs[groupKey] === 'undefined')
    {
      results.coeffs[groupKey] = {quantityDone: count};
    }
    else
    {
      results.coeffs[groupKey].quantityDone += count;
  ...
  function findProdDowntimesStep()
  {
    ...
    cursor.on('data', prodDowntime =>
    {
      ...
      {
        const shiftEndTime = prodDowntime.date.getTime() + shiftUtil.SHIFT_LENGTH * 3600 * 1000;
        ...
        prodDowntime.finishedAt = new Date(now);
      }

      prodDowntime.duration = (prodDowntime.finishedAt.getTime() - prodDowntime.startedAt.getTime()) / 3600000;

      if (prodDowntime.duration <= 0)
      {
        return;
      }
  ...
  function findProdShiftOrdersStep(err)
  {
    ...
    conditions.laborTime = {$gt: 0};
    conditions.workerCount = {$gt: 0};
    ...
    const cursor = ProdShiftOrder.find(conditions).select(fields).lean().cursor();
    const next = _.once(this.next());
    ...
    cursor.on('data', pso =>
    {
      if (!pso.finishedAt)
      {
        const shiftEndTime = pso.date.getTime() + shiftUtil.SHIFT_LENGTH * 3600 * 1000;
        ...
        pso.finishedAt = new Date(now);
      }

      if (pso.workDuration <= 0)
      {
        pso.workDuration = pso.finishedAt.getTime() - pso.startedAt.getTime();

        (this.orderToDowntimes[pso._id] || []).forEach(dt =>
        {
          if (options.downtimeReasons.breaks[dt.reason])
          {
            pso.workDuration -= dt.duration;
          }
        });

        pso.workDuration /= 3600000;
      }

      pso.laborTime *= ProdShiftOrder.getTaktTimeCoeff(pso);

      this.orderToWorkerCount[pso._id] = pso.workerCount;

      if (options.interval === 'hour')
      {
        pso.workDuration = null;

        splitProdShiftOrder(this.groupedProdShiftOrders, pso, this.orderToDowntimes);
      }
      else
      {
        groupObjects(this.groupedProdShiftOrders, pso);
      }
    });
  }
    ...
    _.forEach(orderToDowntimes, function(prodDowntimes, key)
    {
      ...
      _.forEach(prodDowntimes, function(prodDowntime)
      {
        if (options.downtimeReasons.breaks[prodDowntime.reason])
        {
          ...
          summary.breakDuration += prodDowntime.duration;
        }
        else
        {
          summary.count += 1;
          summary.duration += prodDowntime.duration;
          ...
          if (schedule === true)
          {
            ...
            summary.scheduledDuration += prodDowntime.duration;
          }
          else if (schedule === false)
          {
            summary.unscheduledCount += 1;
            summary.unscheduledDuration += prodDowntime.duration;
          }
          ...
          {
            incrementDowntimeFte(prodDowntime.aor, prodDowntime.reason, prodDowntime.duration * workerCount);
          }
        }
      });
  ...
  {
    calcFte.report(app, module, options, this.next());
  }
  ...
  function calcCoeffs(groupKey, orders, fteGroupResult, orderToDowntimes)
  {
    ...
    _.forEach(orders, function(order)
    {
      ...
      const hasWorkDuration = order.workDuration > 0;
      let workDuration = hasWorkDuration
        ? order.workDuration
        : ((order.finishedAt.getTime() - order.startedAt.getTime()) / 3600000);
