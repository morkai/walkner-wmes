...
exports.report = (app, {mongoose}, options, done) =>
{
  ...
  step(
    ...
    function()
    {
      ...
      cursor.on('data', order =>
      {
        ...
        if (!anyStatusChanges)
        {
          order.changes.unshift({
            time: order.importTs,
            oldValues: {statuses: order.statuses},
            newValues: {statuses: order.statuses},
            comment: ''
          });
        }
        ...
        const group = getGroup(order[options.findDateProperty], order.mrp);

        group.orderCount += 1;
        group.mrps[order.mrp][0] += 1;
        ...
        order.statuses.forEach(status =>
        {
          if (options.productionStatuses.includes(status))
          {
            ...
            group.mrps[order.mrp][1] += 1;
          }

          if (options.endToEndStatuses.includes(status))
          {
            group.endToEndCount += 1;
            group.mrps[order.mrp][2] += 1;
          }
        });
    ...
    function(err)
    {
      ...
      if (!options.export && options.hash)
      {
    ...
    function(err)
    {
      ...
      if (!options.export && options.hash && results.orders.length)
      {
  ...
  function getGroup(date, mrp)
  {
    ...
    if (mrp && !results.groups[key].mrps[mrp])
    {
      results.groups[key].mrps[mrp] = [0, 0, 0];
    }
