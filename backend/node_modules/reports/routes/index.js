...
const businessDaysRoute = require('./businessDays');

module.exports = (app, module) =>
{
  const {express, user, settings, downloads} = module;

  const canView = user.auth('REPORTS:*');
  const canManage = user.auth('REPORTS:MANAGE');

  express.get('/reports/businessDays', user.auth('LOCAL', 'USER'), businessDaysRoute.bind(null, app, module));
  ...
  if (module.config.reports['9'])
  {
    ...
    express.get(
      '/reports/9;12nc-to-cags',
      user.auth('LOCAL', 'REPORTS:VIEW', 'REPORTS:9:VIEW'),
      require('./nc12ToCags').bind(null, app, module)
    );
  }

  if (module.config.reports.rearm)
  {
    express.get(
      '/reports/rearm',
      user.auth('REPORTS:VIEW', 'REPORTS:REARM:VIEW'),
      helpers.sendCachedReport.bind(null, 'rearm'),
      require('./rearm').bind(null, app, module)
    );
  }
  ...
  express.post('/reports;download', (req, res) =>
  {
    ...
    res.json(downloads.set(null, {
      attachment: `${req.query.filename}.csv`,
      body: Buffer.concat([Buffer.from([0xEF, 0xBB, 0xBF]), Buffer.from(req.body, 'utf8')])
    }));
  });

  express.get('/reports;download', (req, res, next) =>
  {
