...
module.exports = function setUpWhRoutes(app, module)
{
  ...
  const canUpdate = userModule.auth('LOCAL', 'WH:VIEW');
  const canManage = userModule.auth('WH:MANAGE', 'WH:MANAGE:USERS');
  ...
  ['wh-pickup', 'wh-kitting', 'wh-packing'].forEach(appId =>
  {
    express.get(`/${appId}`, canView, (req, res) =>
    {
      res.format({
        'text/html': function()
        {
          res.render('index', updaterModule.getAppTemplateData(appId, req));
        }
      });
    });
  });
  ...
  function actRoute(req, res, next)
  {
    ...
    req.body.user = userModule.createUserInfo(req.session.user, req);
