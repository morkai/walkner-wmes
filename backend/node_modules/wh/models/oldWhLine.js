...
exports.setUp = (app, mongoose) =>
{
  ...
  lineSchema.statics.lockLines = async function(lines)
  {
    const WhLine = this;

    return Promise.all(lines.map(({_id}) => app.wh.state.lock(`${WhLine.TOPIC_PREFIX}.${_id}`)));
  };
  ...
  lineSchema.statics.recount = async function(lineId, recountPickup, recountComponents, session)
  {
    ...
    const [whLine, whOrders, whDeliveredOrders] = await Promise.all([
      ...
      !recountPickup ? null : WhOrder
        ...
        .select({status: 1, date: 1, lines: 1, set: 1})
        .lean()
        .session(session)
        .exec(),
      !recountComponents ? null : WhDeliveredOrder
        ...
        .select({qtyRemaining: 1, pceTime: 1})
        .lean()
        .session(session)
        .exec()
    ]);
    ...
    if (recountPickup)
    {
      ...
      whOrders.forEach(whOrder =>
      {
        ...
        pickup.sets.add(`${whOrder.date.getTime()}:${whOrder.set}`);
        pickup.qty += whOrderLine.qty;
        pickup.time += whOrderLine.qty * whOrderLine.pceTime;
      });

      pickup.sets = pickup.sets.size;
    ...
    if (recountComponents)
    {
      ...
      {
        components.qty += whDeliveredOrder.qty;
        components.time += whDeliveredOrder.qty * whDeliveredOrder.pceTime;
      });
