...
exports.setUp = (app, mongoose) =>
{
  ...
  eventSchema.statics.setCarts = function(setCartsResults, whOrder)
  {
    ...
    {
      setCartsResults.remaining.forEach(addSetCart);
    }

    if (Array.isArray(setCartsResults.added))
    {
      setCartsResults.added.forEach(addSetCart);
    }

    if (Array.isArray(setCartsResults.updated))
    {
      setCartsResults.updated.forEach(addSetCart);
    }

    return setCarts;

    function addSetCart(setCart)
    {
      const id = setCart._id.toString();
    ...
    ['lines', 'redirLines', 'setCarts'].forEach(prop =>
    {
      ...
      if (Array.isArray(data[prop]))
      {
        data[prop].forEach(item =>
        {
          ...
          if (item.kind && item.cart)
          {
            objects.add(`cart-${item.cart}`);
            objects.add(`cart-${item.kind}-${item.cart}`);
          }
        });
      }
    });
