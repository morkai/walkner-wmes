...
module.exports = (app, module) =>
{
  ...
  actions.updateOrder = (data, done) =>
  {
    ...
    step(
      ...
      function(err)
      {
        ...
        app.broker.publish(`${WhOrder.TOPIC_PREFIX}.updated`, {orders: [this.order]});
  ...
  {
    step(
      function findLastProblemLinesStep()
      {
        findLastProblemLines(date, this.parallel());

        // TODO remove
        settings.findById('wh.planning.enabledMrps', this.parallel());
      },
      function findNextPendingOrderStep(err, problemLines, mrpsSetting)
      {
        if (err)
        {
          return this.skip(err);
        ...
        const mrps = mrpsSetting ? mrpsSetting.value : [];

        if (problemLines.length)
        {
          tryFindNextPendingOrderForNextLine(date, problemLines, mrps, [], this.next());
        }
        else
        {
          tryFindNextPendingOrder(date, null, mrps, [], this.next());
        }
      },
      done
    );
  ...
  function tryFindNextPendingOrderForNextLine(date, lines, mrps, ignore, done)
  {
    tryFindNextPendingOrder(date, lines.shift()._id, mrps, ignore, (err, whOrder) =>
    {
      if (err && err.code === 'NO_PENDING_ORDERS')
      {
        if (lines.length)
        {
          return tryFindNextPendingOrderForNextLine(date, lines, mrps, ignore, done);
        }

        return tryFindNextPendingOrder(date, null, mrps, ignore, done);
      }
  ...
  function findLastProblemLines(date, done)
  {
    step(
      ...
      function(err, whOrder)
      {
        ...
        done(null, whOrder ? whOrder.lines : []);
      }
    );
  }
  ...
  function tryFindNextPendingOrder(date, line, mrps, ignore, done)
  {
    step(
      function()
      {
        settings.findById('wh.planning.ignorePsStatus', this.next());
      },
      function(err, ignorePsStatus)
      {
        if (err)
        {
          return this.skip(err);
        ...
        if (!ignorePsStatus || !ignorePsStatus.value)
        {
      ...
      function(err, whOrder)
      {
        ...
        setImmediate(this.next(), null, whOrder);
      },
      done
    );
  }
