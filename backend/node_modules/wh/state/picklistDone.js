...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  const newValue = typeof data.newValue === 'boolean' ? data.newValue : null;
  ...
  const [setCartsResult, eventsResult] = await Promise.all([
    WhSetCart.deleteByWhOrder(whOrder),
    WhEvent.record(events, session),
    whOrder.save()
  ]);

  const lineResults = whOrder.status === 'problem' || oldStatus === 'problem'
    ? (await WhLine.recountPickupByWhOrder(whOrder))
    : null;
