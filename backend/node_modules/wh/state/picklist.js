...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  switch (newValue)
  {
    ...
    case 'ignore':
      ...
      {
        whOrder.packStatus = 'finished';
      }
      else
      {
        const fmx = whOrder.funcs[0].status;
        const kitter = whOrder.funcs[1].status;

        whOrder.fifoStatus = fmx === 'finished' && kitter === 'finished' ? 'finished' : 'pending';
      }
      break;
