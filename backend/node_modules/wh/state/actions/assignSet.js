...
module.exports = async (app, module, {data, user, session}) =>
{
  ...
  const [eventsResults, downtimeResults] = await Promise.all([
    ...
    !pendingSetAssign ? null : WhPendingSetAssign.collection.updateOne(
      {_id: pendingSetAssign._id}, {$set: {[whUser.func]: true}}, {session}
    )
  ]);

  if (pendingSetAssign)
  {
    WhPendingSetAssign.collection.deleteMany({
      $or: [
        {fmx: true, kitter: true, packer: true, platformer: true, painter: true},
        {date: {$lt: moment.utc().startOf('day').subtract(2, 'days').toDate()}}
      ]
    }, {session});
  }
  ...
  if (whUser.func !== 'platformer' && whUser.func !== 'painter')
  {
    setImmediate(async () =>
    {
      try
