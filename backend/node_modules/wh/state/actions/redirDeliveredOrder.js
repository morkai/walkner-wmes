...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  const [sapOrder, redirReason] = await Promise.all([
    Order
      ...
      .exec(),
    WhRedirReason
      .findOne({_id: data.redirReason, active: true})
      .select({label: 1})
      .lean()
      .session(session)
      .exec()
  ...
  if (!redirReason)
  {
    throw app.createError('Invalid redir reason.', 'INPUT', 400);
  }
  ...
  if (sourceDeliveredOrders.length)
  {
    ...
    for (let i = sourceDeliveredOrders.length - 1; i >= 0; --i)
    {
      ...
      const editEvent = deliveredOrderEditedEvents.get(deliveredOrder);

      editEvent.data.newDeliveredOrder = _.pick(deliveredOrder, eventProps);

      if (!jsonDeepEqual(editEvent.data.oldDeliveredOrder, editEvent.data.newDeliveredOrder))
      {
        events.push(editEvent);

        deliveredOrdersResult.updated.push({
          _id: deliveredOrder._id,
          ...editEvent.data.newDeliveredOrder
        });
      }
    }
  }
  ...
  psos.forEach(pso =>
  {
    targetQtyDone += pso.quantityDone;
  });

  targetDeliveredOrders.forEach(deliveredOrder =>
  {
    targetQtyDone -= deliveredOrder.qtyDone;
  });

  targetDeliveredOrder.qtyDone = Math.max(targetQtyDone, 0);
