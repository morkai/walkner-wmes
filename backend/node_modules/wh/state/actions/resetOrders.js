...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  {
    oldStatuses.push(whOrder.status);
  ...
  if (data.orders)
  {
    const [setCartsResults] = await Promise.all([
      ...
      deliveredOrdersOps.length
        ...
        : null,
      WhPendingSetAssign.collection.deleteMany({date, set}, {session})
    ]);
  ...
  {
    const [setCartsResults] = await Promise.all([
      finish ? null : WhSetCart.deleteBySet(date, set, session),
      deliveredOrdersOps.length
        ? WhDeliveredOrder.collection.bulkWrite(deliveredOrdersOps, {session})
        : null,
      WhPendingSetAssign.collection.deleteMany({date, set}, {session})
  ...
  whOrders.forEach((whOrder, i) =>
  {
    orders.push(whOrder.toJSON());

    whOrder.checkProblemChange(oldStatuses[i], messages);
  });
