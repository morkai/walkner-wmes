// Part of <https://miracle.systems/p/walkner-wmes> licensed under <CC BY-NC-SA 4.0>

'use strict';

exports.detached = true;

exports.session = false;

exports.execute = async (app, module, {data, user}) =>
{
  const {
    OldWhEvent: WhEvent,
    OldWhOrder: WhOrder
  } = module;

  if (typeof data.qty !== 'number' || data.qty < 1 || data.qty > 10)
  {
    throw app.createError(`Invalid label quantity.`, 'INPUT', 400);
  }

  const whOrder = await WhOrder.findById(data.order).lean().exec();

  if (!whOrder)
  {
    throw app.createError(`Order not found.`, 'INPUT', 400);
  }

  const setOrders = await WhOrder
    .find({set: whOrder.set, date: whOrder.date})
    .select({_id: 1})
    .sort({startTime: 1})
    .lean()
    .exec();

  const labelOrders = setOrders.map(o => o._id === whOrder._id ? whOrder : null);

  await module.state.printLabels(labelOrders, data.qty, data.funcId);

  const {messages} = await WhEvent.record([{
    type: 'labelsPrinted',
    user,
    time: new Date(),
    data: {
      func: data.funcId,
      qty: data.qty,
      orders: WhEvent.orders([whOrder])
    }
  }]);

  return {messages};
};
