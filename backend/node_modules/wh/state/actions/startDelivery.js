// Part of <https://miracle.systems/p/walkner-wmes> licensed under <CC BY-NC-SA 4.0>

'use strict';

const {ObjectId} = require('mongoose').Types;
const resolveUser = require('./resolveUser');

exports.session = true;

exports.execute = async (app, module, {data, user, whUser, session}) =>
{
  const {
    Order,
    OldWhEvent: WhEvent,
    OldWhOrder: WhOrder,
    OldWhSetCart: WhSetCart,
    OldWhLine: WhLine,
    OldWhDeliveredOrder: WhDeliveredOrder,
    OldWhPendingSetAssign: WhPendingSetAssign
  } = module;

  const {
    kind,
    forceLine,
    personnelId
  } = data;

  if (!WhSetCart.KINDS.includes(kind))
  {
    throw app.createError('Invalid kind.', 'INPUT', 400);
  }

  if (!whUser)
  {
    whUser = await resolveUser(app, module, {data, session});
  }

  const localDate = new Date();
  const fifoDelivery = kind === 'components';
  const packDelivery = kind === 'packaging';
  const psDelivery = kind === 'ps';
  const minTimeForDeliveryPerMrp = module.state.settings.minTimeForDelivery;
  const maxSetCartsPerDelivery = fifoDelivery
    ? (module.state.settings.maxFifoCartsPerDelivery || 3)
    : packDelivery
      ? (module.state.settings.maxPackCartsPerDelivery || 6)
      : psDelivery
        ? (module.state.settings.maxPsCartsPerDelivery || 100)
        : 100;

  const setCartsToDeliver = [];

  const results = {
    deliveredOrders: {
      added: [],
      updated: [],
      messages: []
    },
    lines: {
      updated: [],
      messages: []
    },
    orders: {
      updated: [],
      messages: []
    },
    setCarts: {
      updated: [],
      messages: []
    }
  };
  const event = {
    type: 'deliveryStarted',
    time: localDate,
    user,
    data: {
      kind,
      whUser: WhEvent.whUser(whUser),
      orders: [],
      lines: new Set(),
      setCarts: null,
      forceLine: forceLine || null
    }
  };
  const problemLineOrders = new Set();
  const completedSapOrders = new Set();

  await fillPendingSetCarts();

  // Fill set carts to deliver only if:
  // it is FIFO delivery and there is still space for carts
  // or it is not FIFO delivery and there were no pending carts
  if ((fifoDelivery && setCartsToDeliver.length < maxSetCartsPerDelivery)
    || (!fifoDelivery && setCartsToDeliver.length === 0))
  {
    await fillRemainingSetCarts();
  }

  if (!setCartsToDeliver.length)
  {
    return {
      response: {
        result: 'nothingToDeliver'
      }
    };
  }

  await unblockDeliveredOrders();

  // Create operations
  const ops = {
    deliveredOrders: [],
    lines: [],
    setCarts: [],
    orders: []
  };

  results.deliveredOrders.added.forEach(document =>
  {
    ops.deliveredOrders.push({
      insertOne: {document}
    });
  });

  results.deliveredOrders.updated.forEach(update =>
  {
    ops.deliveredOrders.push({
      updateOne: {
        filter: {_id: update._id},
        update: {$set: update}
      }
    });
  });

  results.lines.updated.forEach(update =>
  {
    ops.lines.push({
      updateOne: {
        filter: {_id: update._id},
        update: {$set: update}
      }
    });
  });

  if (results.setCarts.updated.length)
  {
    const $set = {...results.setCarts.updated[0]};

    delete $set._id;

    ops.setCarts.push({
      updateMany: {
        filter: {_id: {$in: results.setCarts.updated.map(u => u._id)}},
        update: {$set}
      }
    });
  }

  const distStatusProp = WhOrder.DIST_STATUS_PROPS[kind];

  ops.orders.push({
    updateMany: {
      filter: {_id: {$in: []}},
      update: {$set: {
        distStatus: 'started',
        [distStatusProp]: 'started',
        setDistStarted: true
      }}
    }
  });

  setCartsToDeliver.sort((a, b) =>
  {
    let cmp = a.line.localeCompare(b.line, undefined, {numeric: true, ignorePunctuation: true});

    if (!cmp)
    {
      cmp = a.startTime - b.startTime;
    }

    return cmp;
  });

  event.data.setCarts = WhEvent.setCarts({remaining: setCartsToDeliver});

  const deliveringBy = {id: whUser._id, label: whUser.label};
  const whOrderIds = new Set();
  const sets = new Map();
  const groupedLines = new Map();

  setCartsToDeliver.forEach(setCart =>
  {
    if (setCart.lines.length > 1 && !setCart.pending)
    {
      if (!groupedLines.has(setCart.line))
      {
        groupedLines.set(setCart.line, {
          lines: setCart.lines,
          minStartTime: Number.MAX_SAFE_INTEGER
        });
      }

      const group = groupedLines.get(setCart.line);

      if (setCart.startTime < group.minStartTime)
      {
        group.minStartTime = setCart.startTime;
      }
    }

    setCart.deliveringBy = deliveringBy;

    const update = {
      status: 'delivering',
      pending: false,
      deliveringBy,
      deliveringAt: localDate,
      users: WhSetCart.collectUsers(setCart)
    };

    Object.assign(setCart, update);

    results.setCarts.updated.push(setCart);

    ops.setCarts.push({
      updateOne: {
        filter: {_id: setCart._id},
        update: {$set: update}
      }
    });

    WhEvent.lines(setCart, event.data.lines);

    setCart.orders.forEach(whSetCartOrder =>
    {
      const whOrderId = whSetCartOrder.whOrder;

      if (whOrderIds.has(whOrderId))
      {
        return;
      }

      whOrderIds.add(whOrderId);

      event.data.orders.push({
        whOrder: whOrderId,
        sapOrder: whSetCartOrder.sapOrder,
        date: setCart.date,
        set: setCart.set
      });

      sets.set(`${setCart.set}:${setCart.date}`, {
        date: setCart.date,
        set: setCart.set
      });

      ops.orders[0].updateMany.filter._id.$in.push(whOrderId);

      results.orders.updated.push({
        _id: whOrderId,
        ...ops.orders[0].updateMany.update.$set
      });
    });
  });

  // Handle possible skipped, older set carts due to line grouping
  if (groupedLines.size)
  {
    const $or = [];

    groupedLines.forEach(({lines, minStartTime}) =>
    {
      $or.push({
        status: 'completed',
        line: {$in: lines},
        startTime: {$lt: minStartTime}
      });
    });

    const skippedSetCarts = await WhSetCart
      .find({
        $or
      })
      .lean()
      .session(session)
      .exec();
    const skippedSets = new Map();

    skippedSetCarts.forEach(setCart =>
    {
      const key = `${setCart.set}:${setCart.date.getTime()}`;

      if (!skippedSets.has(key))
      {
        skippedSets.set(key, {
          line: setCart.line,
          startTime: Number.MAX_SAFE_INTEGER,
          setCarts: []
        });
      }

      const skippedSet = skippedSets.get(key);

      if (setCart.startTime < skippedSet.startTime)
      {
        skippedSet.startTime = setCart.startTime;
      }

      skippedSet.setCarts.push(setCart);
    });

    const sortedSkippedSets = Array.from(skippedSets.values()).sort((a, b) => a.startTime - b.startTime);
    const skippedLines = new Set();

    sortedSkippedSets.forEach(skippedSet =>
    {
      if (skippedLines.has(skippedSet.line))
      {
        return;
      }

      skippedLines.add(skippedSet.line);

      skippedSet.setCarts.forEach(setCart =>
      {
        results.setCarts.updated.push({
          _id: setCart._id,
          forced: true
        });

        ops.setCarts.push({
          updateOne: {
            filter: {_id: setCart._id},
            update: {$set: {forced: true}}
          }
        });
      });
    });
  }

  // Mark start of set delivery of not yet sent orders
  const notSentWhOrders = new Set();

  results.deliveredOrders.added.forEach(deliveredOrder =>
  {
    if (!whOrderIds.has(deliveredOrder.whOrder))
    {
      notSentWhOrders.add(deliveredOrder.whOrder);

      results.orders.updated.push({
        _id: deliveredOrder.whOrder,
        setDistStarted: true
      });
    }
  });

  if (notSentWhOrders.size)
  {
    ops.orders.push({
      updateMany: {
        filter: {
          _id: {$in: Array.from(notSentWhOrders)}
        },
        update: {$set: {
          setDistStarted: true
        }}
      }
    });
  }

  // Write operations
  const [eventsResult] = await Promise.all([
    WhEvent.record(event, session),
    ops.deliveredOrders.length ? WhDeliveredOrder.collection.bulkWrite(ops.deliveredOrders, {session}) : null,
    ops.lines.length ? WhLine.collection.bulkWrite(ops.lines, {session}) : null,
    ops.orders.length ? WhOrder.collection.bulkWrite(ops.orders, {session}) : null,
    ops.setCarts.length ? WhSetCart.collection.bulkWrite(ops.setCarts, {session}) : null,
    sets.size ? WhPendingSetAssign.collection.deleteMany({$or: Array.from(sets.values())}, {session}) : null
  ]);

  // Create messages
  results.deliveredOrders.messages.push({
    topic: `${WhDeliveredOrder.TOPIC_PREFIX}.updated`,
    message: {
      added: results.deliveredOrders.added,
      updated: results.deliveredOrders.updated
    }
  });

  results.lines.messages.push({
    topic: `${WhLine.TOPIC_PREFIX}.updated`,
    message: {
      updated: results.lines.updated
    }
  });

  results.orders.messages.push({
    topic: `${WhOrder.TOPIC_PREFIX}.updated`,
    message: {
      updated: results.orders.updated
    }
  });

  results.setCarts.messages.push({
    topic: `${WhSetCart.TOPIC_PREFIX}.updated`,
    message: {
      updated: results.setCarts.updated
    }
  });


  await session.commitTransaction();

  return {
    response: {
      result: 'deliveryStarted',
      setCarts: setCartsToDeliver,
      user: whUser,
      personnelId,
      completedSapOrders: Array.from(completedSapOrders)
    },
    results: [
      eventsResult,
      ...Object.values(results)
    ]
  };

  function resolveSetId(setCart)
  {
    return `${setCart.date.getTime()}:${setCart.set}`;
  }

  async function fillPendingSetCarts()
  {
    const conditions = {
      status: 'completed',
      pending: true,
      kind
    };

    if (forceLine)
    {
      conditions.lines = forceLine;
    }

    const allPendingSetCarts = await WhSetCart
      .find(conditions)
      .sort({startTime: 1})
      .hint({status: 1, date: -1})
      .lean()
      .session(session)
      .exec();

    const setToCarts = new Map();

    allPendingSetCarts.forEach(setCart =>
    {
      const setId = resolveSetId(setCart);

      if (!setToCarts.has(setId))
      {
        setToCarts.set(setId, []);
      }

      setToCarts.get(setId).push(setCart);
    });

    const groupedPendingSetCarts = Array.from(setToCarts.values());

    while (groupedPendingSetCarts.length > 0)
    {
      const pendingSetCarts = groupedPendingSetCarts.shift();

      // Do not include FIFO carts from other sets if they don't fit fully
      if (fifoDelivery
        && setCartsToDeliver.length > 0
        && (setCartsToDeliver.length + pendingSetCarts.length) > maxSetCartsPerDelivery)
      {
        continue;
      }

      while (pendingSetCarts.length > 0 && setCartsToDeliver.length < maxSetCartsPerDelivery)
      {
        const pendingSetCart = pendingSetCarts.shift();

        setCartsToDeliver.push(pendingSetCart);
      }

      if (!fifoDelivery || pendingSetCarts.length > 0)
      {
        break;
      }
    }
  }

  async function fillRemainingSetCarts()
  {
    const forcedSetCarts = await findForcedSetCarts();
    const availableLines = await findAvailableLines(forcedSetCarts);
    const availableSetCarts = await findAvailableSetCarts(availableLines);
    const lineToSets = mapLineToSets(availableSetCarts);
    const remainingSetCarts = findRemainingSetCarts(availableLines, lineToSets);
    const newOrderSets = await findNewOrderSets(remainingSetCarts);
    const allNewWhOrders = await findAllNewWhOrders(newOrderSets);
    const lineStats = createDeliveredOrders(allNewWhOrders);

    await Promise.all([
      updateLines(lineStats),
      updateAllPendingSetCarts(newOrderSets)
    ]);
  }

  async function findForcedSetCarts()
  {
    if (forceLine)
    {
      return [];
    }

    return await WhSetCart
      .find({
        kind,
        status: 'completed',
        pending: false,
        forced: true
      })
      .sort({
        completedAt: 1
      })
      .lean()
      .session(session)
      .exec();
  }

  async function findAvailableLines(forcedSetCarts)
  {
    const conditions = {};

    if (forceLine)
    {
      conditions._id = forceLine;
    }
    else if (forcedSetCarts.length)
    {
      const lineIds = new Set();

      forcedSetCarts.forEach(setCart =>
      {
        setCart.lines.forEach(lineId =>
        {
          lineIds.add(lineId);
        });
      });

      conditions._id = {$in: Array.from(lineIds)};
    }
    else
    {
      const maxMinTimeForDelivery = (Math.max.apply(Math, Object.values(minTimeForDeliveryPerMrp)) || 45) * 60 * 1000;

      conditions['available.time'] = {$lt: maxMinTimeForDelivery};
    }

    const list = await WhLine
      .find(conditions)
      .lean()
      .session(session)
      .exec();

    const map = new Map();

    list.forEach(line =>
    {
      if (conditions._id)
      {
        map.set(line._id, line);

        return;
      }

      let minTimeForDelivery = Math.max.apply(Math, line.mrps.map(mrp => minTimeForDeliveryPerMrp[mrp] || 0));

      if (!(minTimeForDelivery > 0))
      {
        minTimeForDelivery = minTimeForDeliveryPerMrp['*'] || 45;
      }

      if (line.available.time < (minTimeForDelivery * 60 * 1000))
      {
        map.set(line._id, line);
      }
    });

    return map;
  }

  async function findAvailableSetCarts(availableLines)
  {
    const availableSetCarts = [];

    if (!availableLines.size)
    {
      return availableSetCarts;
    }

    const allAvailableSetCarts = await findAllAvailableSetCarts(availableLines);

    allAvailableSetCarts.forEach(lineSetCarts =>
    {
      const carts = [];
      let startTime = Number.MAX_SAFE_INTEGER;
      let availableTime = Number.MAX_SAFE_INTEGER;
      let completedAt = Number.MAX_SAFE_INTEGER;

      lineSetCarts.carts.sort((a, b) => a.startTime - b.startTime);

      lineSetCarts.carts.forEach(cart =>
      {
        startTime = Math.min(cart.startTime.getTime(), startTime);
        availableTime = Math.min.apply(Math, cart.lines.map(
          lineId => availableLines.has(lineId) ? availableLines.get(lineId).available.time : Number.MAX_SAFE_INTEGER
        ));
        completedAt = Math.min(cart.completedAt.getTime(), completedAt);

        carts.push(cart);
      });

      availableSetCarts.push({
        _id: lineSetCarts._id,
        carts,
        startTime,
        availableTime,
        completedAt
      });
    });

    availableSetCarts.sort((a, b) =>
    {
      if (a.carts[0].forced && !b.carts[0].forced)
      {
        return -1;
      }

      if (!a.carts[0].forced && b.carts[0].forced)
      {
        return 1;
      }

      let cmp = a.availableTime - b.availableTime;

      if (!cmp)
      {
        cmp = a.startTime - b.startTime;

        if (!cmp)
        {
          cmp = a.completedAt - b.completedAt;
        }
      }

      return cmp;
    });

    return availableSetCarts;
  }

  async function findAllAvailableSetCarts(availableLines)
  {
    const pipeline = [
      {
        $match: {
          status: {$in: ['completing', 'completed']},
          pending: false,
          lines: {$in: Array.from(availableLines.keys())}
        }
      },
      {
        $group: {
          _id: {
            line: '$line',
            date: '$date',
            set: '$set'
          },
          minStartTime: {$min: '$startTime'},
          carts: {
            $addToSet: {
              _id: '$_id',
              kind: '$kind',
              status: '$status',
              startTime: '$startTime',
              lines: '$lines',
              cart: '$cart',
              forced: '$forced',
              completedAt: '$completedAt'
            }
          }
        }
      },
      {
        $sort: {
          minStartTime: 1
        }
      }
    ];

    const allSetCarts = await WhSetCart
      .aggregate(pipeline)
      .session(session)
      .exec();

    const lineToSetCarts = new Map();

    allSetCarts.forEach(setCarts =>
    {
      const validCarts = [];
      const otherCarts = [];

      setCarts.carts.forEach(cart =>
      {
        if (cart.status === 'completed' && cart.kind === kind)
        {
          validCarts.push(cart);
        }
        else
        {
          otherCarts.push(cart);
        }
      });

      const lineSetCarts = {
        _id: setCarts._id,
        minStartTime: setCarts.minStartTime,
        validCarts,
        otherCarts
      };

      (validCarts.length ? validCarts[0] : otherCarts[0]).lines.forEach(line =>
      {
        if (!lineToSetCarts.has(line))
        {
          lineToSetCarts.set(line, []);
        }

        lineToSetCarts.get(line).push(lineSetCarts);
      });
    });

    const allAvailableSetCarts = [];
    const usedSets = new Set();

    lineToSetCarts.forEach(lineSets =>
    {
      lineSets.forEach(lineSet =>
      {
        if (lineSet.validCarts.length && (forceLine || lineSet.validCarts[0].forced))
        {
          usedSets.add(lineSet._id);

          allAvailableSetCarts.push({
            _id: lineSet._id,
            carts: lineSet.validCarts
          });
        }
      });
    });

    lineToSetCarts.forEach(lineSets =>
    {
      while (lineSets.length)
      {
        const lineSet = lineSets.shift();

        if (!lineSet.validCarts.length)
        {
          break;
        }

        if (usedSets.has(lineSet._id))
        {
          continue;
        }

        usedSets.add(lineSet._id);

        allAvailableSetCarts.push({
          _id: lineSet._id,
          carts: lineSet.validCarts
        });
      }
    });

    return Array.from(allAvailableSetCarts);
  }

  function mapLineToSets(availableSetCarts)
  {
    const lineToSets = new Map();

    availableSetCarts.forEach(lineSetCarts =>
    {
      const {date, set} = lineSetCarts._id;
      const setId = `${set}:${date.getTime()}`;

      lineSetCarts.carts.forEach(setCart =>
      {
        setCart.lines.forEach(line =>
        {
          if (!lineToSets.has(line))
          {
            lineToSets.set(line, new Map());
          }

          const lineSets = lineToSets.get(line);

          if (!lineSets.has(setId))
          {
            lineSets.set(setId, {
              date,
              set,
              carts: []
            });
          }

          lineSets.get(setId).carts.push(setCart);
        });
      });
    });

    lineToSets.forEach((lineSets, lineId) =>
    {
      lineToSets.set(lineId, Array.from(lineSets.values()));
    });

    return lineToSets;
  }

  function findRemainingSetCarts(availableLines, lineToSets)
  {
    const remainingSetCarts = [];
    const usedSets = new Set();
    const usedSetCarts = new Set();

    setCartsToDeliver.forEach(setCart => usedSetCarts.add(setCart._id.toString()));

    for (const lineId of availableLines.keys())
    {
      const lineSets = lineToSets.get(lineId);

      if (!lineSets)
      {
        continue;
      }

      while (lineSets.length)
      {
        const lineSet = lineSets.shift();
        const setId = `${lineSet.set}:${lineSet.date.getTime()}`;

        if (usedSets.has(setId))
        {
          continue;
        }

        usedSets.add(setId);

        let totalCartCount = setCartsToDeliver.length + remainingSetCarts.length;

        // Do not include FIFO carts from other sets if they don't fit fully
        if (fifoDelivery
          && totalCartCount > 0
          && (totalCartCount + lineSet.carts.length) > maxSetCartsPerDelivery)
        {
          break;
        }

        while (lineSet.carts.length && totalCartCount < maxSetCartsPerDelivery)
        {
          const setCartId = lineSet.carts.shift()._id;
          const usedSetCartId = setCartId.toString();

          if (usedSetCarts.has(usedSetCartId))
          {
            continue;
          }

          remainingSetCarts.push(setCartId);
          usedSetCarts.add(usedSetCartId);

          totalCartCount += 1;
        }

        // Stop if:
        // it is not a FIFO delivery: only one set per packaging or paint-shop delivery
        // or the set didn't fit fully
        // or we reached the maximum
        if ((!fifoDelivery && remainingSetCarts.length)
          || lineSet.carts.length > 0
          || totalCartCount === maxSetCartsPerDelivery)
        {
          return remainingSetCarts;
        }
      }
    }

    return remainingSetCarts;
  }

  async function findNewOrderSets(remainingSetCarts)
  {
    const newOrderSets = new Map();

    if (!remainingSetCarts.length)
    {
      return newOrderSets;
    }

    const newSetCarts = await WhSetCart
      .find({_id: {$in: remainingSetCarts}})
      .lean()
      .session(session)
      .exec();

    newSetCarts.forEach(setCart =>
    {
      setCartsToDeliver.push(setCart);

      const setId = resolveSetId(setCart);

      if (!newOrderSets.has(setId))
      {
        newOrderSets.set(setId, {
          set: setCart.set,
          date: setCart.date
        });
      }
    });

    return newOrderSets;
  }

  async function findAllNewWhOrders(newOrderSets)
  {
    if (!newOrderSets.size)
    {
      return [];
    }

    const allNewWhOrders = await WhOrder
      .find({
        $or: Array.from(newOrderSets.values())
      })
      .select({
        status: 1,
        order: 1,
        lines: 1,
        date: 1,
        set: 1,
        startTime: 1
      })
      .lean()
      .session(session)
      .exec();
    const sapOrderIds = new Set();

    allNewWhOrders.forEach(whOrder =>
    {
      sapOrderIds.add(whOrder.order);

      if (whOrder.status !== 'problem')
      {
        return;
      }

      whOrder.lines.forEach(whOrderLine =>
      {
        problemLineOrders.add(`${whOrderLine._id}:${whOrder.order}`);
      });
    });

    const sapOrders = await Order
      .find({
        _id: {$in: Array.from(sapOrderIds)},
        statuses: {$in: Order.COMPLETED_STATUSES}
      })
      .select({_id: 1})
      .lean()
      .session(session)
      .exec();

    sapOrders.forEach(sapOrder =>
    {
      completedSapOrders.add(sapOrder._id);
    });

    return allNewWhOrders;
  }

  function createDeliveredOrders(allNewWhOrders)
  {
    const lineStats = new Map();

    allNewWhOrders.forEach(whOrder =>
    {
      whOrder.lines.forEach(whOrderLine =>
      {
        if (!lineStats.has(whOrderLine._id))
        {
          lineStats.set(whOrderLine._id, {
            sets: new Set(),
            qty: 0,
            time: 0,
            totalQty: 0,
            totalTime: 0
          });
        }

        const line = lineStats.get(whOrderLine._id);
        const completed = completedSapOrders.has(whOrder.order);
        const blocked = !completed && problemLineOrders.has(`${whOrderLine._id}:${whOrder.order}`);

        line.sets.add(`${whOrder.date.getTime()}:${whOrder.set}`);

        line.totalQty += whOrderLine.qty;
        line.totalTime += whOrderLine.qty * whOrderLine.pceTime;

        if (!completed && !blocked)
        {
          line.qty += whOrderLine.qty;
          line.time += whOrderLine.qty * whOrderLine.pceTime;
        }

        results.deliveredOrders.added.push({
          _id: new ObjectId(),
          status: completed ? 'done' : blocked ? 'blocked' : 'todo',
          line: whOrderLine._id,
          redirLine: null,
          qtyTodo: whOrderLine.qty,
          qtyDone: completed ? whOrderLine.qty : 0,
          qtyRemaining: completed ? 0 : whOrderLine.qty,
          pceTime: whOrderLine.pceTime,
          sapOrder: whOrder.order,
          whOrder: whOrder._id,
          date: whOrder.date,
          set: whOrder.set,
          startTime: whOrderLine.startTime || whOrder.startTime
        });
      });
    });

    return lineStats;
  }

  async function updateLines(lineStats)
  {
    if (!lineStats.size)
    {
      return;
    }

    const lineIds = Array.from(lineStats.keys());

    await WhLine.lockLines(lineIds);

    const whLines = await WhLine
      .find({_id: {$in: lineIds}})
      .lean()
      .session(session)
      .exec();

    whLines.forEach(whLine =>
    {
      const stats = lineStats.get(whLine._id);
      const sets = stats.sets.size;

      whLine.pickup.finished.sets = Math.max(0, whLine.pickup.finished.sets - sets);
      whLine.pickup.finished.qty = Math.max(0, whLine.pickup.finished.qty - stats.totalQty);
      whLine.pickup.finished.time = Math.max(0, whLine.pickup.finished.time - stats.totalTime);
      whLine.pickup.total.sets = whLine.pickup.started.sets + whLine.pickup.finished.sets;
      whLine.pickup.total.qty = whLine.pickup.started.qty + whLine.pickup.finished.qty;
      whLine.pickup.total.time = whLine.pickup.started.time + whLine.pickup.finished.time;

      whLine.available.qty += stats.qty;
      whLine.available.time += stats.time;

      results.lines.updated.push({
        _id: whLine._id,
        pickup: whLine.pickup,
        available: whLine.available
      });
    });
  }

  async function updateAllPendingSetCarts(newOrderSets)
  {
    if (!newOrderSets.size)
    {
      return;
    }

    const pendingSetCarts = await WhSetCart
      .find({
        pending: false,
        $or: Array.from(newOrderSets.values()),
        _id: {$nin: setCartsToDeliver.map(c => c._id)}
      })
      .select({_id: 1})
      .hint({set: 1, date: -1})
      .lean()
      .exec();

    pendingSetCarts.forEach(setCart =>
    {
      setCart.pending = true;

      results.setCarts.updated.push(setCart);
    });
  }

  async function unblockDeliveredOrders()
  {
    const $or = [];
    const sapOrderIds = new Set();
    const whOrderIds = new Set();
    const missingLineIds = new Set();

    setCartsToDeliver.forEach(setCart =>
    {
      setCart.orders.forEach(({whOrder, sapOrder}) =>
      {
        if (!completedSapOrders.has(sapOrder))
        {
          sapOrderIds.add(sapOrder);
        }

        whOrderIds.add(whOrder);

        setCart.lines.forEach(line =>
        {
          if (!problemLineOrders.has(`${line}:${sapOrder}`))
          {
            missingLineIds.add(line);
            $or.push({sapOrder, line, status: 'blocked'});
          }
        });
      });
    });

    if (!$or.length)
    {
      return;
    }

    const whLines = new Map();

    results.lines.updated.forEach(whLine =>
    {
      missingLineIds.delete(whLine._id);
      whLines.set(whLine._id, whLine);
    });

    if (missingLineIds.size)
    {
      await WhLine.lockLines(Array.from(missingLineIds));
    }

    const [completedOrders, problemOrders, deliveredOrders, missingLines] = await Promise.all([
      !sapOrderIds.size ? [] : Order
        .find({
          _id: {$in: Array.from(sapOrderIds)},
          statuses: {$in: Order.COMPLETED_STATUSES}
        })
        .select({_id: 1})
        .lean()
        .session(session)
        .exec(),
      WhOrder
        .find({
          _id: {$in: Array.from(whOrderIds)},
          status: 'problem'
        })
        .select({
          lines: 1,
          order: 1
        })
        .lean()
        .hint({_id: 1})
        .session(session)
        .exec(),
      WhDeliveredOrder
        .find({$or})
        .select({
          sapOrder: 1,
          line: 1,
          status: 1,
          qtyTodo: 1,
          qtyDone: 1,
          qtyRemaining: 1,
          pceTime: 1
        })
        .lean()
        .hint({sapOrder: 1, line: 1, status: 1})
        .session(session)
        .exec(),
      !missingLineIds.size ? [] : WhLine
        .find({_id: {$in: Array.from(missingLineIds)}})
        .select({available: 1})
        .lean()
        .session(session)
        .exec()
    ]);

    if (!deliveredOrders.length)
    {
      return;
    }

    completedOrders.forEach(sapOrder =>
    {
      completedSapOrders.add(sapOrder._id);
    });

    missingLines.forEach(whLine =>
    {
      results.lines.updated.push(whLine);
      whLines.set(whLine._id, whLine);
    });

    const problems = new Set();

    problemOrders.forEach(whOrder =>
    {
      whOrder.lines.forEach(whOrderLine =>
      {
        problems.add(`${whOrderLine._id}:${whOrder.order}`);
      });
    });

    deliveredOrders.forEach(deliveredOrder =>
    {
      const whLine = whLines.get(deliveredOrder.line);

      if (completedSapOrders.has(deliveredOrder.sapOrder))
      {
        if (whLine)
        {
          whLine.available.qty = Math.max(0, whLine.available.qty - deliveredOrder.qtyRemaining);
          whLine.available.time = Math.max(
            0, whLine.available.time - deliveredOrder.qtyRemaining * deliveredOrder.pceTime
          );
        }

        deliveredOrder.status = 'done';
        deliveredOrder.qtyRemaining = 0;
        deliveredOrder.qtyDone = deliveredOrder.qtyTodo;

        results.deliveredOrders.updated.push(deliveredOrder);

        return;
      }

      if (problems.has(`${deliveredOrder.line}:${deliveredOrder.sapOrder}`))
      {
        return;
      }

      if (whLine)
      {
        whLine.available.qty += deliveredOrder.qtyRemaining;
        whLine.available.time += deliveredOrder.qtyRemaining * deliveredOrder.pceTime;
      }

      deliveredOrder.status = 'todo';

      results.deliveredOrders.updated.push(deliveredOrder);
    });
  }
};
