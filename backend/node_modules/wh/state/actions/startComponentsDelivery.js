  ...
  if (setCarts.length < maxSetCartsPerDelivery)
  {
    ...
    const availableSetCarts = await WhSetCart
      .aggregate([
        ...
        {$match: {
          cartCount: {$lte: remainingSetCartCount}
        }},
        {$sort: {startTime: 1}}
      ])
      .hint({status: 1, date: -1})
    ...
    const blockedDeliveredOrders = !allNewWhOrders.length ? [] : (await WhDeliveredOrder
      .find({
    ...
    {
      blocked.add(`${deliveredOrder.line}:${deliveredOrder.sapOrder}`);
    });
    ...
    allNewWhOrders.forEach(whOrder =>
    {
      whOrder.lines.forEach(whOrderLine =>
      {
        ...
        deliveredOrdersResult.added.push({
          _id: new ObjectId(),
          status: blocked.has(`${whOrderLine._id}:${whOrder.order}`) ? 'blocked' : 'todo',
