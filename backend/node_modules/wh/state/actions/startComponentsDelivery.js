...
module.exports = async (app, module, {data, user, whUser, session}) =>
{
    ...
    utcNow)
  {
    ...
    const [allAvailableSetCarts, plans] = await Promise.all([
      WhSetCart
        .aggregate([
          {$match: {
            status: 'completed',
      ...
      Plan
        .find({
          _id: {$gte: shifts.getPlanDate(Date.now(), false).subtract(7, 'days').utc(true).toDate()}
        })
        .select({
          'stats.startAt': 1
        })
    ...
    allAvailableSetCarts.forEach(lineSetCarts =>
    {
      ...
      {
        return forcedLine
          || (cart.startTime.getTime() - utcNow) < maxDeliveryStartTime
