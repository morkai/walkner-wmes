...
module.exports = async (app, module, {data, user, session}) =>
{
  ...
  const now = new Date();
  const priorityThreshold = module.state.settings.pickupPriorityThreshold > 0
    ? (module.state.settings.pickupPriorityThreshold * 60 * 1000)
    : 0;
  ...
  const {
    ...
    ignoredCandidates,
    priorityPickup
  } = await findNextPendingOrder(availableLines);
