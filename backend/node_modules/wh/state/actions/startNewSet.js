...
module.exports = async (app, module, {data, user, session}) =>
{
  ...
  const now = new Date();
  const priorityThreshold = settings.pickupPriorityThreshold > 0
    ? (settings.pickupPriorityThreshold * 60 * 1000)
    : 0;
  const minDowntimeDuration = (settings.minPickupDowntime || 120) * 1000;
  const maxDowntimeDuration = (settings.maxPickupDowntime || 480) * 1000;
  ...
  const {
    ...
    ignoredCandidates,
    priorityPickup
  } = await findNextPendingOrder(availableLines, null);
  ...
  if (pendingOrder.lines.length > 1)
  {
    const groupLines = new Map();
    const missingLineIds = pendingOrder.lines.filter(line => !candidateLines.has(line._id));

    if (missingLineIds.length)
    {
      const missingLines = await findAvailableLines(missingLineIds);

      missingLines.forEach(line =>
      {
        groupLines.set(line._id, line);
      });
    }

    pendingOrder.lines.forEach(line =>
    {
      if (candidateLines.has(line._id))
      {
        groupLines.set(line._id, candidateLines.get(line._id).line);
      }
    });

    const {pendingLines} = await findNextPendingOrder(groupLines, pendingOrder.date);

    pendingLines.forEach(pendingLine =>
    {
      if (!pendingPriorityLines.includes(pendingLine._id[0]))
      {
        pendingPriorityLines.push(pendingLine._id[0]);
      }
    });
  }
  ...
  if (priorityPickup)
  {
    event.data.priorityPickup = true;
  }

  if (forceDelivery)
  {
    event.data.forceDelivery = true;
  }
  ...
  const whOrdersOps = [];
  const mrps = new Set();
  const forceRedirLine = !!forceLine && !!redirLine && forceLine !== redirLine;
  ...
  setOrders.forEach(whOrder =>
  {
    ...
    if (forceRedirLine)
    {
      $set.forcedRedir = whOrder.forcedRedir = {
        redirLine: whOrder.redirLine,
        redirLines: whOrder.redirLines
      };
      $set.redirLine = whOrder.redirLine = whOrder.line;
      $set.redirLines = whOrder.redirLines = whOrder.lines.map(l => l._id);
      $set.lines = whOrder.lines;

      whOrder.lines.forEach(whOrderLine =>
      {
        if (whOrderLine._id === forceLine)
        {
          whOrderLine._id = redirLine;
        }
      });

      if (whOrder.lines.length === 1)
      {
        $set.line = whOrder.line = redirLine;
      }
    }

    if (forceDelivery)
    {
      $set.forceDelivery = whOrder.forceDelivery = true;
    }
  ...
  lineResults.updated.forEach(line =>
  {
    if (priorityThreshold > 0
      && line.pickup.total.time < priorityThreshold
      && !pendingPriorityLines.includes(line._id))
    {
      pendingPriorityLines.push(line._id);
    }
  });
  ...
  setImmediate(async () =>
  {
    try
    {
      await module.state.printLabels(setOrders, 1, 'fmx');
      await module.state.printLabels(setOrders, 1, 'kitter');
      await module.state.printLabels(setOrders, 1, 'packer');
    }
    catch (err)
    {
  ...
  async function findAvailableLines(lineIds)
  {
    const maxSetsPerLine = settings.maxSetsPerLine || 2;
    const conditions = {};

    if (lineIds)
    {
      conditions._id = {$in: lineIds};
    }
    else if (forceLine)
    {
    ...
    allLines.forEach(line =>
    {
      ...
      if (lineIds
        || forceLine
        || line.pickup.total.sets < maxSetsPerLine
        || pendingPriorityLines.includes(line._id))
      {
  ...
  async function findNextPendingOrder(availableLines, maxDate)
  {
    const {ignorePsStatus, enabledMrps} = settings;
    let {availableTimeThreshold} = settings;
    ...
    if (maxDate)
    {
      conditions.date.$lt = maxDate;
      conditions.line = {$in: lineIds};
      availableTimeThreshold = Number.MAX_SAFE_INTEGER;
    }
    ...
    if (!pendingLines.length || maxDate)
    {
      return result;
    }
    ...
    const priorityCandidateI = pendingPriorityLines.findIndex(lineId => candidateLines.has(lineId));
    let bestCandidate = null;

    if (priorityCandidateI !== -1)
    {
      result.priorityPickup = true;
      bestCandidate = candidateLines.get(pendingPriorityLines[priorityCandidateI]);

      pendingPriorityLines.splice(priorityCandidateI, 1);
    }
    else
    {
    ...
    result.pendingOrder = await WhOrder
      .findOne(conditions)
      .sort({startTime: 1})
      .lean()
  ...
  {
    const groupDuration = (settings.groupDuration || 2) * 3600 * 1000;
    const minSetDuration = (settings.minSetDuration || 120) * 60 * 1000;
    const maxSetDuration = (settings.maxSetDuration || 120) * 60 * 1000;
    const maxSetDifference = (settings.maxSetDifference || 120) * 60 * 1000;
    const maxSetSize = settings.maxSetSize || 5;
    ...
    if (Array.isArray(settings.ignorePsStatus) && settings.ignorePsStatus.length)
    {
      conditions.psStatus = {$nin: settings.ignorePsStatus};
    }
    ...
    while (whOrders.length && setOrders.length < maxSetSize)
    {
      ...
      if (whOrder.lines.length > 1 && whOrder.lines[0].startTime)
      {
        orderDuration = whOrder.lines.map(l => l.finishTime - l.startTime).sort((a, b) => b - a)[0];

        /* TODO Change to sum of merged intervals?
        const mergedIntervals = mergeIntervals(whOrder.lines.map(
          whOrderLine => [whOrderLine.startTime.getTime(), whOrderLine.finishTime.getTime()]
        ));

        mergedIntervals.forEach(interval =>
        {
          orderDuration += interval[1] - interval[0];
        });
        */
      }
      else
      {
        orderDuration = whOrder.finishTime - whOrder.startTime;
      }
...
function mergeIntervals(intervals) // eslint-disable-line no-unused-vars
{
