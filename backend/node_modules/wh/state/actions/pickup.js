...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  if (data.edit && data.newValue !== 'success')
  {
  ...
  const [setCartsResults] = await Promise.all([
    problem ? WhSetCart.deleteByWhOrder(whOrder) : WhSetCart.updateByFunc(whOrder, func._id),
  ...
  const [lineResults, setResults, eventsResults] = await Promise.all([
    whOrder.status !== oldStatus ? WhLine.recountPickupByWhOrder(whOrder) : null,
