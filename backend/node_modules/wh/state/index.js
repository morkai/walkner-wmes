...
const updateNextShift = require('./updateNextShift');
...
module.exports = (app, module) =>
{
  ...
  const doUpdateNextShift = updateNextShift.bind(null, app, module);
  const scheduleNextShiftUpdate = _.debounce(doUpdateNextShift, 15000);
  ...
  {
    const psStatus = message.change.newValues.psStatus;
  ...
  async function snapshotLines()
  {
    try
    {
      ...
      ops.push({
        deleteMany: {
          filter: {
            time: {$lt: new Date(time.getTime() - 14 * 24 * 3600 * 1000)}
          }
        }
      });
