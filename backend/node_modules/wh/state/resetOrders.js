...
exports.execute = async (app, module, {data, user, session}) =>
{
  ...
  for (const whOrder of whOrders)
  {
    ...
    events.push({
      type: data.cancel ? 'orderCancelled' : 'orderReset',
      time,
      user,
      order: whOrder._id,
      data: {
  ...
  if (data.orders)
  {
    ...
    {
      results.push(await WhPendingDelivery.deleteBySet(whOrders[0].date, whOrders[0].set));
    }
  ...
  {
    const [setCartsResults, lineResults, pendingDeliveryResults] = await Promise.all([
      WhSetCart.deleteBySet(data.date, data.set, session),
      WhLine.recountPickupByWhOrder(whOrders[0]),
      WhPendingDelivery.deleteBySet(data.date, data.set, session)
    ]);

    results.push(
      setCartsResults,
      lineResults,
      pendingDeliveryResults
    );
  }
