  ...
  return new Promise((resolve, reject) =>
  {
    step(
      ...
      function(err, psEvents)
      {
        ...
        {
          const leadingNo = noToLeadingNo.get(psEvent.order);

          if (!groupedPsEvents.has(leadingNo))
          {
            groupedPsEvents.set(leadingNo, []);
          }

          groupedPsEvents.get(leadingNo).push(psEvent.data.comment);
        });
        ...
        this.psOrders.forEach(psOrder =>
        {
          ...
          if (!groupedPsOrders.has(leadingNo))
          {
            ...
            groupedPsOrders.set(leadingNo, {
              psOrder: psOrderMap.get(leadingNo) || {
                _id: '',
                order: leadingNo,
          ...
          const {byPaint, comments} = groupedPsOrders.get(leadingNo);

          (groupedPsEvents.get(psOrder._id) || []).forEach(comment => comments.push(comment));

          const sapOrders = groupedSapOrders.get(leadingNo);
