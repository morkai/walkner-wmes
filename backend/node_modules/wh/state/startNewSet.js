...
module.exports = async (app, module, {data, user, session}) =>
{
  ...
  if (downtime && !data.downtimeReason)
  {
    return {
      response: {
  ...
  const problemLines = await findLastProblemLines();
  ...
  setOrders.forEach(order =>
  {
    ...
    $set.startedAt = order.startedAt = now;
  ...
  const [eventsResults, downtimeResults] = await Promise.all([
    ...
    WhDowntime.createDowntime({
      reason: data.downtimeReason,
      startedAt: whUser.lastPickupAt,
      user: {id: whUser._id, label: whUser.label},
      comment: data.downtimeComment,
      session
    }),
