...
module.exports = async (app, module, serialNumber) =>
{
  ...
  try
  {
    ...
    if (whDeliveredOrder)
    {
      if (whDeliveredOrder.qtyRemaining === 0)
      {
        whDeliveredOrder.status = 'done';

        await WhDeliveredOrder.collection.updateOne(
          {_id: whDeliveredOrder._id},
          {$set: {status: whDeliveredOrder.status}},
          {session}
        );
      }

      if (whDeliveredOrder.status !== 'blocked')
      {
        const lineResult = await WhLine.collection.findOneAndUpdate(
          {_id: serialNumber.prodLine},
          {$inc: {
            'available.qty': -1,
            'available.time': deliveredOrderResult.value.pceTime * -1
          }},
          {
            projection: {
              available: 1
