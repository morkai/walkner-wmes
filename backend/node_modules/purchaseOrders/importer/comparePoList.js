...
module.exports = function comparePoList(app, importerModule, purchaseOrders, done)
{
  ...
  importerModule.info(`Comparing ${orderIds.length} POs...`);
  ...
  step(
    ...
    function closeOpenPosStep(err, openOrders, orderModels)
    {
      if (err)
      {
        importerModule.error(err, 'Failed to find POs for comparison.');
    ...
    function(err)
    {
      ...
      if (err)
      {
        importerModule.error(err, `Failed to close ${closedOrderIds.length} POs.`, {closedOrderIds});
      }
      else if (closedOrderIds.length)
      {
        importerModule.info(`Closed ${closedOrderIds.length} POs.`, {closedOrderIds});
      }
      ...
      if (l)
      {
        for (i = 0; i < l; ++i)
        {
          ...
          else
          {
            ...
            importerModule.warn('Invalid order for insert.', {
              order: this.insertList[i]
            });
          }
        }
      }
      ...
      if (l)
      {
        for (i = 0; i < l; ++i)
        {
          ...
          else
          {
            ...
            importerModule.warn('Invalid order for update.', {order: this.updateList[i]});
          }
        }
      }
    },
    function(err)
    {
      if (err)
      {
        return importerModule.error(
          err,
          `Failed to sync ${this.insertList.length} new and ${this.updateList.length} existing POs.`
        );
      }
      ...
      importerModule.info(`Synced ${created} new and ${updated} existing POs.`);
