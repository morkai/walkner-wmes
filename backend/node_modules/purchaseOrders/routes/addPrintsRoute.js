...
module.exports = (app, poModule, req, res, next) =>
{
  ...
  {
    return next(app.createError('LOCKED'));
  }
  ...
  {
    return next(app.createError('INVALID_PAPER'));
  }

  if (!PurchaseOrder.BARCODES[barcode])
  {
    return next(app.createError('INVALID_BARCODE'));
  }
  ...
  if (items.length === 0)
  {
    return next(app.createError('INVALID_ITEMS'));
  }
  ...
  step(
    ...
    function handleFindPoResultStep(err, po)
    {
      ...
      if (!po)
      {
        return this.skip(app.createError('PO_NOT_FOUND'));
      }
      ...
      for (let i = 0, l = items.length; i < l; ++i)
      {
        ...
        if (!item.model)
        {
          return this.skip(app.createError('ITEM_NOT_FOUND'));
        }
      }
    ...
    function handleCreatePoPrintsResultStep(err)
    {
      if (err)
      {
        poModule.error(err, 'Failed to create prints.', {poId: this.po._id});
    ...
    function handleSaveModifiedPoResultStep(err)
    {
      if (err)
      {
        poModule.error(err, 'Failed to save PO after modifying printed quantities.', {poId: this.po._id});
