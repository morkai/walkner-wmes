...
exports.setUp = (app, mongoose) =>
{
  ...
  entrySchema.index({'oldComponents._id': 1});
  entrySchema.index({'newComponents._id': 1});
  entrySchema.index({'orders.orderNo': 1});
  entrySchema.index({reason: 1});
  entrySchema.index({status: 1});
  entrySchema.index({mrps: 1, status: 1});
  entrySchema.index({valid: 1});
  ...
  entrySchema.statics.updateStatus = function(entry, changes)
  {
    const funcStatuses = {
      pending: 0,
      rejected: 0,
    ...
    else if (funcStatuses.accepted === entry.funcs.length)
    {
      newStatus = 'accepted';
    }
    ...
    if (newStatus !== entry.status)
    {
      ...
      {
        changes.acceptedAt = [entry.acceptedAt, entry.updatedAt];
        entry.acceptedAt = entry.updatedAt;
      }
    }
  };
  ...
  {
    this.constructor.updateStatus(this, changes);
  };
  ...
  {
    const reset = !!changes.oldComponents || !!changes.newComponents;
  ...
  entrySchema.methods.updateValidity = async function()
  {
    const CompRelEntry = this.constructor;

    const {valid, validOrders, invalidOrders} = await this.checkValidity();

    if (this.valid === valid && !validOrders.length && !invalidOrders.length)
    {
      return;
    }
    ...
    if (this.valid !== valid)
    {
      change.data.valid = [this.valid, valid];
      this.valid = valid;
    }
    ...
    if (validOrders.length)
    {
      ...
      change.data.validOrders = [null, orderNos];
    }
    ...
    if (invalidOrders.length)
    {
      ...
      change.data.invalidOrders = [null, orderNos];
    }
    ...
    app.broker.publish(`${CompRelEntry.TOPIC_PREFIX}.updated.${this._id}`, {
      _id: this._id,
      change
    });
  };

  entrySchema.methods.checkValidity = async function()
  {
    const invalidOrders = [];
    const validOrders = [];
    const orderMap = new Map();
    const invalidOrderMap = new Map();
    ...
    this.orders.forEach(o =>
    {
      ...
      orderMap.set(o.orderNo, o);

      if (!o.valid)
      {
        invalidOrderMap.set(o.orderNo, o);
      }
    });
    ...
    const sapOrders = this.oldComponents.length === 0 ? [] : await Order
      .find({
        _id: {$in: Array.from(orderMap.keys())},
        'bom.nc12': {$in: this.oldComponents.map(c => c._id)}
      })
      .select({_id: 1})
      .lean()
      .exec();
    ...
    {
      if (invalidOrderMap.has(sapOrder._id))
      {
        invalidOrderMap.delete(sapOrder._id);
      }
      else
      {
        invalidOrders.push(orderMap.get(sapOrder._id));
      }
    });

    invalidOrderMap.forEach(crOrder =>
    {
      validOrders.push(crOrder);
    });

    return {
      valid: sapOrders.length === 0,
      invalidOrders,
      validOrders
    };
  };
