...
const uploadSnfImagesRoute = require('./snf/uploadImages');
const sendSnfImageRoute = require('./snf/sendImage');
const editSnfImageRoute = require('./snf/editImage');
const deleteSnfImageRoute = require('./snf/deleteImage');
...
module.exports = (app, module) =>
{
  ...
  const canView = user.auth('XICONF:VIEW');
  const canViewLocal = user.auth('LOCAL', 'XICONF:VIEW');
  const canManage = user.auth('XICONF:MANAGE');
  const canManageHidLamps = user.auth('XICONF:MANAGE', 'XICONF:MANAGE:HID_LAMPS');
  const canManageLocal = user.auth('LOCAL', 'XICONF:MANAGE');
  ...
  express.get('/maxos', (req, res, next) =>
  {
  ...
  express.post(
    ...
    multer({
      storage: multer.diskStorage({
        destination: module.config.updatesPath,
  ...
  //
  express.post('/xiconf;execute', user.auth('LOCAL'), function(req, res, next)
  {
    ...
    if (_.isEmpty(rid)
      ...
      || _.isEmpty(action)
      || !_.isString(action)
      || !_.isFunction(module.remote[action]))
    {
    ...
    module.remote[action](req.body, function(err, result)
    {
  ...
  express.post('/xiconf/orders;import', importOrdersRoute.bind(null, app, module));
  ...
  // Results
  //
  express.post('/xiconf;import', importResultsRoute.bind(null, app, module));
  ...
  express.get(
    ...
    canView,
    downloadRoute.bind(null, 'workflow', app, module, XiconfResult)
  );

  express.get(
    '/xiconf/results/:id;feature',
    canView,
    downloadRoute.bind(null, 'feature', app, module, XiconfResult)
  );

  express.get(
    '/xiconf/results/:id;gprsOrder',
    canView,
    downloadRoute.bind(null, 'gprsOrder', app, module, XiconfResult)
  );

  express.get(
    '/xiconf/results/:id;gprsInput',
    canView,
    downloadRoute.bind(null, 'gprsInput', app, module, XiconfResult)
  );

  express.get(
    '/xiconf/results/:id;gprsOutput',
    canView,
    downloadRoute.bind(null, 'gprsOutput', app, module, XiconfResult)
  );
  ...
  express.post('/xiconf/programs;sync', syncProgramsRoute.bind(null, app, module));
  ...
  express.post(
    ...
    uploadSnfImagesRoute.bind(null, app, module)
  );
  express.get('/snf/programs/:program/images/:image', canViewLocal, sendSnfImageRoute.bind(null, app, module));
  express.put('/snf/programs/:program/images/:image', canManage, editSnfImageRoute.bind(null, app, module));
  express.delete('/snf/programs/:program/images/:image', canManage, deleteSnfImageRoute.bind(null, app, module));
  ...
  express.post('/xiconf/hidLamps;printLabel', canManageLocal, printHidLabelRoute.bind(null, app, module));
  ...
  express.post(
    '/xiconf/clients;settings',
    canManageLocal,
    getClientsSettingsRoute.bind(null, app, module)
  );
  ...
  express.get(
    ...
    canViewLocal,
    crossOrigin,
    getClientLicensesRoute.bind(null, app, module)
  );
  express.post(
    '/xiconf/clients;licenses',
    canManageLocal,
    crossOrigin,
    addClientLicenseRoute.bind(null, app, module)
  );

  express.get('/xiconf/clients/:id;goTo', canView, goToClientsPageRoute.bind(null, app, module));

  express.get('/xiconf/clients/:id;downloadVNC', canView, downloadClientsVNCRoute.bind(null, app, module));

  express.delete('/xiconf/clients/:id', canManage, express.crud.deleteRoute.bind(null, app, XiconfClient));

  express.get('/xiconf/updates/:version', sendUpdateRoute.bind(null, app, module));
  ...
  {
    if (_.isFunction(module.getRemoteCoordinatorDebugInfo))
    {
      res.json(module.getRemoteCoordinatorDebugInfo(req.query.enable !== undefined));
    }
    else
    {
      res.sendStatus(500);
  ...
  {
    if (_.isFunction(module.clearOrderData))
    {
      module.clearOrderData(req.query.order);

      res.json(module.getRemoteCoordinatorDebugInfo().ordersToDataMap);
    }
    else
    {
      res.sendStatus(500);
  ...
  function updateRemoteDataRoute(req, res)
  {
    ...
    else if (_.isFunction(module.updateRemoteData))
    {
      module.updateRemoteData(req.query.prodLine);

      res.json(module.getRemoteCoordinatorDebugInfo().prodLinesToDataMap[req.query.prodLine] || null);
    }
    else
    {
      res.sendStatus(500);
  ...
  function readFileContents(xiconfResult, property, fileHash, done)
  {
    ...
    const filePath = path.join(module.config.featureDbPath, fileHash + '.xml');
  ...
  function prepareXiconfClient(totalCount, clients, formatResult)
  {
    if (Array.isArray(clients))
    {
      clients.forEach(client =>
      {
        ...
        client.remoteAddress = socket ? resolveIpAddress(socket) : null;
      });
    }
