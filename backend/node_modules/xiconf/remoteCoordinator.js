...
module.exports = function setUpXiconfCommands(app, xiconfModule)
{
  ...
  function onAppStarted()
  {
    settingsModule.findById('xiconf.notifier.delay', function(err, setting)
    {
      if (err)
      {
        return xiconfModule.error(err, 'Failed to read the delay setting.');
      }
  ...
  function onLicenseEdited(message)
  {
    ...
    XiconfClient.find({license: licenseId, socket: {$ne: null}}, {socket: 1}).lean().exec(function(err, xiconfClients)
    {
      if (err)
      {
        return xiconfModule.error(err, 'Failed find a client on license edit.');
      }
      ...
      _.forEach(xiconfClients, function(xiconfClient)
      {
        handleConfigureRequest({socket: xiconfClient.socket, settings: {licenseKey: licenseKey}}, function(err)
        {
          if (err)
          {
            return xiconfModule.error(err, `Failed to update the client's license key on license edit.`);
          }
        });
      });
    });
  ...
  function onOrdersSynced()
  {
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          return xiconfModule.error(err, 'Failed to update cached XiconfOrders data after Orders were synced.');
        }
      }
    );
  }
  ...
  function onXiconfOrdersSynced(message)
  {
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          return xiconfModule.error(err, 'Failed to remove cached XiconfOrder data after XiconfOrders were synced.');
        }
      }
    );
  }
  ...
  function recountOrderResults()
  {
    ...
    step(
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to recount orders after results sync.');
        }
  ...
  function onClientsStateChanged(newState)
  {
    ...
    XiconfClient.collection.updateOne({_id: socket.xiconf.srcId}, {$set: newState}, function(err)
    {
      if (err)
      {
        return xiconfModule.error(err, 'Failed to update client after changing state.', {
          srcId: socket.xiconf.srcId
        });
      }
  ...
  function handleClientDisconnect()
  {
    ...
    xiconfModule.info('Client disconnected.', {
      clientId,
      prodLineId: socket.xiconf.prodLineId,
      socketId: socket.id
    });
    ...
    XiconfClient.collection.updateOne({_id: clientId, socket: socket.id}, {$set: clientChanges}, function(err)
    {
      if (err)
      {
        xiconfModule.error(err, 'Failed to update client after disconnection.', {clientId});
      }
      else
      {
  ...
  function handleClientConnect(data)
  {
    ...
    if (socket.xiconf.prodLineId !== null)
    {
      if (data.prodLineId !== socket.xiconf.prodLineId)
      {
        ...
        xiconfModule.warn('Client changed prod line.', {
          clientId: data.srcId,
          oldProdLineId: socket.xiconf.prodLineId,
          newProdLineId: data.prodLineId,
          socketId: socket.id
        });
      }
      else
    ...
    {
      xiconfModule.info('Client connected.', {
        clientId: data.srcId,
    ...
    XiconfClient.collection.replaceOne({_id: xiconfClient._id}, xiconfClient, {upsert: true}, function(err)
    {
      if (err)
      {
        xiconfModule.error(err, 'Failed to update client after connection.', {clientId: xiconfClient._id});
      }
      else
      {
  ...
  function checkSerialNumber(input, reply)
  {
    step(
      ...
      function checkStep(err, snOrder)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to check serial number for order and 12NC.', {
  ...
  function checkHidLamp(input, reply)
  {
    step(
      ...
      function checkStep(err, orderData, hidLamp)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to check HID lamp.', {
  ...
  function checkComponentWeight(input, reply)
  {
    step(
      ...
      function checkStep(err, componentWeights, order, xiconfOrder)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to check serial number.', {
            serialNumber: input.serialNumber,
            orderNo: input.orderNo,
            nc12: input.nc12
          });
  ...
  function recountNextOrder(data, done)
  {
    step(
      ...
      function(err)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to recount order.', {orderNo: data.orderNo});
        }
        else
        {
          ...
          debug('[recountNextOrder]', {orderNo: data.orderNo, quantityDone: this.orderData.quantityDone});
  ...
  function generateNextServiceTag(data, done)
  {
    step(
      ...
      function replyStep(err, serviceTag)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to generate a new Service Tag for order.', {orderNo: data.orderNo});
  ...
  function acquireNextServiceTag(data, done)
  {
    step(
      ...
      function applyChangesStep(err)
      {
        ...
        if (data.resultId)
        {
          ...
          XiconfResult.updateOne({_id: data.resultId}, {$set: {cancelled: false}}, function(err)
          {
            if (err)
            {
              xiconfModule.error(err, 'Failed to restore result.', {resultId: data.resultId});
            }
      ...
      function replyStep(err)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to acquire a new Service Tag for order.', {orderNo: data.orderNo});
  ...
  function releaseNextServiceTag(data, done)
  {
    step(
      ...
      function toggleResultStep()
      {
        if (data.resultId)
        {
          ...
          XiconfResult.updateOne({_id: data.resultId}, {$set: {cancelled: true}}, function(err)
          {
            if (err)
            {
              xiconfModule.error(err, 'Failed to cancel result.', {resultId: data.resultId});
            }
      ...
      function replyStep(err)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to release Service Tag.', {
  ...
  function recordNextInvalidLed(data, done)
  {
    ...
    invalidLed.save(function(err)
    {
      if (err)
      {
        xiconfModule.error(err, 'Failed to record an invalid LED.');
      }
      else
      {
  ...
  function getOrderData(orderNo, done)
  {
    ...
    step(
      ...
      function createXiconfOrder(err, order)
      {
        ...
        if (!order)
        {
          debug('[getOrderData] Order not found.', {orderNo});

          return this.skip(null, null);
        }

        debug('[getOrderData] Creating XiconfOrder from Order.', {orderNo});
      ...
      function sendResultsStep(err, xiconfOrder)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to get data for order.', {orderNo});
  ...
  {
    debug('[updateProdLinesRemoteData]', {prodLineId, socketId: optSocket ? optSocket.id : null});
  ...
  function emitRemoteDataForProdLineUpdate(prodLineId, optSocket, newOrdersNos, remoteData)
  {
    ...
    if (optSocket)
    {
      debug('[emitRemoteDataForProdLineUpdate]', {prodLineId, socketId: optSocket.id, newOrdersNos});

      return optSocket.emit('xiconf.remoteDataUpdated', remoteData);
    }

    debug('[emitRemoteDataForProdLineUpdate]', {
      prodLineId,
      newOrdersNos,
      sockets: Object.keys(prodLineData.sockets)
    });
  ...
  function emitRemoteDataToOrderWorkers(orderNo)
  {
    getOrderData(orderNo, function(err, orderData)
    {
      if (err)
      {
        return xiconfModule.error(err, 'Failed to emit remote data to order workers.', {orderNo});
      }
      ...
      if (!orderToProdLinesMap)
      {
        return debug('[emitRemoteDataToOrderWorkers] NO_PROD_LINES_FOR_ORDER', {orderNo});
      }
      ...
      {
        return debug('[emitRemoteDataToOrderWorkers] NO_PROD_LINES_IN_ORDER', {orderNo});
      }
      ...
      _.forEach(prodLineIds, function(prodLineId)
      {
        ...
        if (!prodLineData)
        {
          return debug('[emitRemoteDataToOrderWorkers] NO_PROD_LINE', {orderNo, prodLineId});
        }

        debug('[emitRemoteDataToOrderWorkers]', {
          orderNo,
          prodLineId,
          sockets: Object.keys(prodLineData.sockets)
        });
  ...
  function scheduleWorkingOrderChangeCheck(orderNo)
  {
    ...
    debug('[scheduleWorkingOrderChangeCheck]', {orderNo, delay});
  ...
  function checkWorkingOrderChange(orderNo)
  {
    ...
    if (!orderToProdLines)
    {
      return debug('[checkWorkingOrderChange] EMPTY', {orderNo});
    }
    ...
    {
      debug('[checkWorkingOrderChange] NO_PROD_LINES', {orderNo});
    ...
    debug('[checkWorkingOrderChange]', {orderNo, changed: !anyCurrent});
  }
  ...
  function checkClientsLicenseKey(socket, licenseId, clientsLicenseKey)
  {
    ...
    License.findById(licenseId, function(err, license)
    {
      if (err)
      {
        return xiconfModule.error(err, 'Failed to find license.', {licenseId});
      }
      ...
      handleConfigureRequest({socket: socket, settings: {licenseKey: license.key}}, function(err)
      {
        if (err)
        {
          xiconfModule.error(err, `Failed to update the client's license key on client connect.`);
        }
      });
    });
  }
