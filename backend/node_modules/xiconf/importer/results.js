...
module.exports = function setUpXiconfResultsImporter(app, xiconfModule)
{
  ...
  function prepareResult(fileInfo, meta, orderIdToNo, result)
  {
    ...
    if (xiconfResult.orderNo)
    {
      search.add(xiconfResult.orderNo);
    }

    if (xiconfResult.serviceTag)
    {
      search.add(xiconfResult.serviceTag.toUpperCase());
    }

    if (xiconfResult.nc12)
    {
      search.add(xiconfResult.nc12.toUpperCase());
    }

    if (xiconfResult.program)
    {
      search.add(xiconfResult.program._id.toUpperCase());
    }

    _.forEach(xiconfResult.leds, led =>
    {
      search.add(led.nc12);

      if (led.serialNumber)
      {
        search.add(led.serialNumber.toUpperCase());
      }
    });

    _.forEach(xiconfResult.hidLamps, hid =>
    {
      search.add(hid.nc12);
    });

    _.forEach(xiconfResult.log, log =>
    {
      if (log.nc12)
      {
        search.add(log.nc12);
    ...
    xiconfResult.search = Array.from(search);
