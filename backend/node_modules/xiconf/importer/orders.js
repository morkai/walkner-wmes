...
exports.start = function startXiconfOrdersImporterModule(app, module)
{
  ...
  function importNext()
  {
    ...
    importFile(fileInfo, function(err, summary)
    {
      ...
      {
        module.info('[%s] Imported in %d ms: %s', fileInfo.timeKey, Date.now() - startTime, JSON.stringify(summary));
  ...
  function importFile(fileInfo, done)
  {
    step(
      ...
      function parseFileStep(err, fileContents)
      {
        ...
        module.info('[%s] Parsed %d orders in %d ms!', fileInfo.timeKey, this.parsedOrdersList.length, Date.now() - t);
