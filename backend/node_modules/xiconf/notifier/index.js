...
const {formatDateTime, formatTime} = require('util/dateFormatter');
...
module.exports = function setUpXiconfNotifier(app, xiconfModule)
{
  ...
  const EMAIL_URL_PREFIX = app.options.emailUrlPrefix;
  ...
  function checkOrderStatus(orderNo)
  {
    step(
      ...
      function findConcernedUsersStep()
      {
        ...
        _.forEach(this.prodShiftOrders, function(pso)
        {
          ...
          prodLineMap[pso.prodLine].times.push({
            from: formatDateTime(pso.startedAt),
            to: formatTime(pso.finishedAt),
            qty: qty
          });
      ...
      function sendEmailStep(err, users)
      {
        ...
        {
          xiconfModule.warn(
            `Can't notify anyone about a status of an order: no user with a valid e-mail address.`,
            {orderNo}
          );
        ...
        const orderUrl = this.order ? (EMAIL_URL_PREFIX + 'r/productionOrder/' + orderNo) : '-';
        const xiconfOrderUrl = EMAIL_URL_PREFIX + 'r/xiconfOrder/' + orderNo;
        const prodShiftOrdersUrl = EMAIL_URL_PREFIX + 'r/prodShiftOrders/' + orderNo;
        ...
        {
          subject = '[WMES] [Testy] Przerwane zlecenie ' + orderNo;

          text.push('Przerwano wykonywanie zlecenia, w którym:');
        }
        else
        {
          subject = '[WMES] [Testy] Nieukończone zlecenie ' + orderNo;
      ...
      function finalizeStep(err)
      {
        if (err)
        {
          xiconfModule.error(err, 'Failed to notify users about a status of an order.', {orderNo});
        }
      }
    );
  }
