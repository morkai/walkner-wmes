...
module.exports = (app, module) =>
{
  ...
  {
    if (FORCED_WORK_DAYS[moment(date).format('YYYY-MM-DD')])
    {
      ...
      const lineState = {
        ...
        smallOrders: {
          count: orderStateQueue.reduce(
            (count, orderState) => count + (orderState.order.kind === 'small' ? 1 : 0),
            0
          ),
          limit: new Map(),
          planned: new Map()
  ...
  function fillLines(state, done)
  {
    ...
    if (incompleteMrps.size && state.incompleteMrps)
    {
      incompleteMrps.forEach((orderNos, mrp) =>
      {
        ...
        if (deepEqual(orderNos, oldOrderNos))
        {
    ...
    steps.push(function() { done(null, true); });
