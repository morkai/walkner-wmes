...
module.exports = function setUpGenerator(app, module)
{
  ...
  const UNFROZEN_PLANS = DEV ? ['2019-02-25'] : [];
  const LOG_LINES = {
    'MFP-14': true
  };
  ...
  const log = LOG && DEV
    ? m => console.log(m)
    : () => {};
  const logLine = (lineState, m) => !LOG_LINES || LOG_LINES[lineState._id]
    ? log(m)
    : undefined;
  ...
  function createOrderStateQueues(state, done)
  {
    ...
    state.plan.orders.forEach(planOrder =>
    {
      ...
      // TODO remove time condition after 25.02.2019
      if (planOrder.urgent || (state.time >= 1551139200000 && sapOrder.priority === 'E'))
      {
  ...
  {
    logLine(lineState, `[${lineState._id}] Generating (${state.generateCallCount})...`);

    if (lineState.locked)
    {
      logLine(lineState, `[${lineState._id}] Locked!`);
    }
    ...
    {
      logLine(lineState, `[${lineState._id}] No workers!`);
    }
  ...
  function completeLine(state, lineState, done)
  {
    ...
    if (!locked && lineState.activeTimes.length)
    {
      ...
      if (lineState.orderStateQueue.length)
      {
        ...
        logLine(lineState, `Continuing ${lineState._id} from ${lineState.activeFrom.format('HH:mm:ss')}...`);
    ...
    if (!oldPlanLine)
    {
      ...
      logLine(lineState, `Completed: pushed new line: ${lineState._id}`);
    ...
    if (oldPlanLine.hash !== newPlanLine.hash)
    {
      ...
      logLine(lineState, `Completed: changed existing line: ${lineState._id}. New version: ${oldPlanLine.version}`);
    ...
    if (locked)
    {
      oldPlanLine.orders.forEach(lineOrder =>
      {
        ...
        if (!orderState)
        {
          module.warn('Locked order state for line order not found.', {lineOrder});
        ...
        order.incomplete -= lineOrder.quantity;
    ...
    logLine(lineState, `Completed: no changes: ${lineState._id}!`);
  ...
  function handleSmallOrder(state, lineState, orderState)
  {
    ...
    if (candidate.quantityRemaining > 0)
    {
      ...
      logLine(lineState, `        NOT PLANNED IN FULL: ${candidate.quantityRemaining} remaining`);
      ...
      if (state.generateCallCount === 1)
      {
        ...
        {
          logLine(lineState, `        IGNORED ${completion}%: ${bestCandidate.orderState.order._id}`);
        ...
        {
          logLine(lineState, `        IGNORED 1 PCE: ${bestCandidate.orderState.order._id}`);

          return;
        }
      }

      logLine(lineState, `        BEST ${completion}%: ${bestCandidate.orderState.order._id}`);
  ...
  function trySmallOrder(state, lineState, orderState, options)
  {
    ...
    // TODO: remove time condition after 25.02.2019
    if (state.time >= 1551139200000 && sapOrder.priority === 'E')
    {
  ...
  function getLinesForBigOrder(state, order)
  {
    ...
    return state.settings.mrp(order.mrp).lines
      .filter(mrpLineSettings =>
      {
        ...
        if (!mrpLineSettings.workerCount.some(c => c > 0)
