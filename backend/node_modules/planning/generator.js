...
module.exports = (app, module) =>
{
  ...
  // sortOrdersByManHours sortOrdersByLeven sortOrdersByNameParts
  const SMALL_ORDERS_SORTER = sortOrdersByNameParts;
  const EASY_ORDERS_SORTER = sortOrdersByNameParts;
  ...
  function createPlanGeneratorState(key)
  {
    ...
    return {
      ...
      log: (message, ...args) =>
      {
        module.info(`[${key}] ${message}`, {submodule: 'generator'}, ...args);
      }
    };
  }
  ...
  function preparePlanOrder(state, planOrder)
  {
    ...
    planOrder.manHours = getManHours(planOrder.operation, quantityTodo, mrpSettings);
    ...
    if (mrpSettings.groups.length)
    {
      ...
      mrpSettings.groups.forEach(group =>
      {
        if (group.components.some(nc12 => sapOrder.bom.has(nc12)))
        {
  ...
  function isWorkDay(state, localDate, done)
  {
    step(
      function()
      {
        PlanSettings
          .findOne({
            _id: moment(localDate.getTime()).utc(true).toDate(),
            forceWorkDay: true
          })
          .select({_id: 1})
          .lean()
  ...
  function loadIncompleteOrders(state, done)
  {
    ...
    step(
      ...
      function(err, prevLocalPlanId)
      {
        ...
        this.prevPlanId = moment.utc(prevLocalPlanId.format('YYYY-MM-DD'), 'YYYY-MM-DD').toDate();
  ...
  function createOrderStateQueues(state, done)
  {
    ...
    for (const orderStateQueues of state.orderStateQueues.values())
    {
      ...
      SMALL_ORDERS_SORTER(orderStateQueues, 'small', null);
      EASY_ORDERS_SORTER(orderStateQueues, 'easy', _.last(orderStateQueues.small));
    }
  ...
  {
    orderStateQueues[kind].sort(sortOrdersByManHoursDesc);
  }
  ...
  function sortOrdersByLeven(orderStateQueues, kind) // eslint-disable-line no-unused-vars
  {
    ...
    unsortedQueue.sort(sortOrdersByManHoursDesc);
  ...
  function sortOrdersByNameParts(orderStateQueues, kind, lastPrevKind) // eslint-disable-line no-unused-vars
  {
    ...
    unsortedQueue.sort(kind === 'small' ? sortOrdersByQuantityAsc : sortOrdersByManHoursDesc);
  ...
  function sortOrdersByQuantityAsc(a, b)
  {
    return a.order.quantityTodo - b.order.quantityTodo;
  }
  ...
  function trySmallOrder(state, lineState, orderState, options)
  {
    ...
    let pceTime = getPceTime(order.operation, workerCount, mrpSettings);
    ...
    while (quantityPlanned <= quantityTodo)
    {
      ...
      if (newFinishAt > activeTo || (shiftNo === 3 && h >= 6 && h < 22))
      {
        if (quantityPlanned > 0)
        {
          ...
          plannedOrders.push({
            ...
            pceTime,
            manHours: getManHours(order.operation, quantityPlanned, mrpSettings),
      ...
      if ((shiftNo === 1 && h >= 14) || (shiftNo === 2 && (h >= 22 || h < 6)))
      {
        ...
        if (quantityPlanned === 0)
        {
          ...
          pceTime = getPceTime(order.operation, workerCount, mrpSettings);
  ...
  function getLinesForBigOrder(state, order)
  {
    ...
    return state.settings.mrp(order.mrp).lines
      .filter(mrpLineSettings =>
      {
        ...
        const pceTime = getPceTime(order.operation, workerCount, mrpSettings);
  ...
  function getPceTime(operation, workerCount, mrpSettings)
  {
    ...
    const {schedulingRate, taktTimeCoeffs} = mrpSettings;
    const taktTimeCoeff = taktTimeCoeffs[operation.workCenter] || taktTimeCoeffs['*'];

    return Math.ceil(operation.laborTime / 100 / workerCount * 3600 * schedulingRate * taktTimeCoeff) * 1000;
  }

  function getManHours(operation, quantityTodo, mrpSettings)
  {
    if (!operation.laborTime)
    {
      return 0;
    ...
    return ((operation.laborTime / 100 * quantityTodo) + operation.laborSetupTime) * schedulingRate * taktTimeCoeff;
  }
  ...
  function getOrderStartOverhead(settings, lineState, orderState)
  {
    ...
    const {
      ...
      schedulingRate,
      taktTimeCoeffs
    } = settings.mrp(orderState.order.mrp);
    const hours = lineState.activeFrom.hours();
    const seconds = lineState.activeFrom.minutes() * 60;
    const taktTimeCoeff = taktTimeCoeffs[orderState.order.operation.workCenter] || taktTimeCoeffs['*'];
    const laborSetupTime = orderState.order.operation.laborSetupTime * schedulingRate * taktTimeCoeff;
