...
exports.report = (app, module, options, done) =>
{
  ...
  function getLineOrderExecution(shiftOrders, lineOrder, planOrder, workingTimes)
  {
    ...
    sumExecutionOrders(shiftOrders, orderNo).forEach(pso =>
    {
      ...
      if (shifts.getShiftInfo(startedAt, false).startTime === shiftStartTime)
      {
  ...
  function sumExecutionOrders(shiftOrders, orderNo)
  {
    const summed = [];

    shiftOrders.filter(o => o.orderId === orderNo).forEach(shiftOrder =>
    {
      const pso = {
        prodLine: shiftOrder.prodLine,
        quantityDone: shiftOrder.quantityDone,
        operationNo: shiftOrder.operationNo,
        startedAt: shiftOrder.startedAt.getTime(),
        finishedAt: shiftOrder.finishedAt ? shiftOrder.finishedAt.getTime() : now
      };
  ...
  function getWorkingTimes({shiftData}, workingLine)
  {
    return {
      ...
      nextAt: shiftData.map((data, shiftNo) =>
      {
        ...
        return workingLine ? workingLine.startAt : 0;
      })
    };
  }
};
