...
exports.report = (app, module, options, done) =>
{
  ...
  function doNextDate(dateQueue, done)
  {
    ...
    step(
      function()
      {
        ...
        Plan
          .find({
            _id: {
              $gt: date,
              $lt: moment.utc(date.getTime()).add(5, 'days').toDate()
            }
          })
          .select({
            'lines._id': 1,
      ...
      function(err, plan, planSettings, dailyMrpCounts, nextPlans)
      {
        if (err)
        {
          return this.skip(err);
        ...
        for (const plan of nextPlans)
        {
          ...
          {
            if (this.workingLines.has(line._id))
            {
              continue;
            }
            ...
            {
              this.workingLines.set(line._id, {
                _id: line._id,
      ...
      function()
      {
        ...
        {
          const nextStartTime = getNextStartTime(this.planLines.get(line), this.workingLines.get(line));
          ...
          lineOrders.forEach(lineOrder =>
          {
            ...
            const execution = getLineOrderExecution(this.shiftOrders[line] || [], lineOrder, planOrder, nextStartTime);
  ...
  function getLineOrderExecution(shiftOrders, lineOrder, planOrder, nextStartTime)
  {
    const execution = {
    ...
    const requiredShiftStartTime = shiftInfo.startTime;
    const requiredDayStartTime = shifts.getFirstShiftStartTime(requiredShiftStartTime, false);
    const requiredDayEndTime = requiredDayStartTime + shifts.SHIFT_LENGTH * 3 * 3600 * 1000;
    const shiftEndTime = requiredShiftStartTime + shifts.SHIFT_LENGTH * 3600 * 1000;
    const nextShiftOverlap = toTime - shiftEndTime;

    if (nextShiftOverlap > 0 && nextStartTime[shiftInfo.no])
    {
      toTime = moment.utc(nextStartTime[shiftInfo.no]).local(true).valueOf() + nextShiftOverlap;
    }
  ...
  function getNextStartTime({shiftData}, workingLine)
  {
    return shiftData.map((data, shiftNo) =>
    {
      if (shiftNo === 0)
      {
        return 0;
