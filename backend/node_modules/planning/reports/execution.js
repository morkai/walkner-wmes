...
exports.report = (app, module, options, done) =>
{
  ...
  function getLineOrderExecution(shiftOrders, lineOrder, planOrder, workingTimes, cache)
  {
    ...
    {
      fromTime = workingTimes.prevAt[shiftInfo.no]
        ? (moment.utc(workingTimes.prevAt[shiftInfo.no]).local(true).valueOf() + prevShiftOverlap)
        : 0;
    }

    if (nextShiftOverlap > 0)
    {
      toTime = workingTimes.nextAt[shiftInfo.no]
        ? (moment.utc(workingTimes.nextAt[shiftInfo.no]).local(true).valueOf() + nextShiftOverlap)
