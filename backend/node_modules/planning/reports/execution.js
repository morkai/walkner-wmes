...
exports.report = (app, module, options, done) =>
{
  ...
  function doNextDate(dateQueue, done)
  {
    ...
    step(
      ...
      function(err, plan, planSettings, dailyMrpCounts, nextPlans, prevPlans)
      {
        if (err)
        {
          return this.skip(err);
        ...
        plan.lines.forEach(planLine =>
        {
          ...
          this.planLines.set(planLine._id, planLine);
        ...
        for (const plan of prevPlans)
        {
          ...
          {
            const workingLine = getWorkingLine(this.workingLines, line._id);
      ...
      function(err, sapOrders, shiftOrders, lateOrders)
      {
        ...
        {
          const line = shiftOrder.prodLine;

          if (!this.shiftOrders[line])
          {
            this.shiftOrders[line] = [];
          }

          if (this.shiftOrders[line].length)
          {
            _.last(this.shiftOrders[line]).nextLinePso = shiftOrder;
          }

          this.shiftOrders[line].push(shiftOrder);
        });
      ...
      function()
      {
        ...
        {
          const workingTimes = getWorkingTimes(this.planLines.get(line), this.workingLines.get(line));
          ...
          lineOrders.forEach(lineOrder =>
          {
            ...
            const execution = getLineOrderExecution(this.shiftOrders[line] || [], lineOrder, planOrder, workingTimes);
  ...
  function getLineOrderExecution(shiftOrders, lineOrder, planOrder, workingTimes)
  {
    const execution = {
    ...
    const shiftInfo = shifts.getShiftInfo(localStartAt, false);
    const operationNo = planOrder.operation && planOrder.operation.no || null;
    const shiftStartTime = shiftInfo.startTime;
    const dayStartTime = shifts.getFirstShiftStartTime(shiftStartTime, false);
    const dayEndTime = dayStartTime + shifts.SHIFT_LENGTH * 3 * 3600 * 1000;
    ...
    if (prevShiftOverlap < 0 && workingTimes.prevAt[shiftInfo.no])
    {
      fromTime = moment.utc(workingTimes.prevAt[shiftInfo.no]).local(true).valueOf() + prevShiftOverlap;
    }

    if (nextShiftOverlap > 0 && workingTimes.nextAt[shiftInfo.no])
    {
      toTime = moment.utc(workingTimes.nextAt[shiftInfo.no]).local(true).valueOf() + nextShiftOverlap;
    }

    let plannedQuantityDoneSum = 0;

    getExecutionOrders(shiftOrders, orderNo, operationNo).forEach(o =>
    {
      ...
      if (shifts.getShiftInfo(o.startedAt, false).startTime === shiftStartTime)
      {
  ...
  function getExecutionOrders(linePsos, orderNo, operationNo)
  {
    const result = [];
    const orderPsos = linePsos.filter(pso => pso.orderId === orderNo);
    ...
    {
      if (pso.operationNo !== operationNo)
      {
        return;
      }
  ...
  function getWorkingTimes({shiftData}, workingLine)
  {
    return {
      prevAt: shiftData.map((data, shiftNo) =>
      {
        if (shiftNo === 0)
        {
          return 0;
        ...
        for (let s = shiftNo - 1; s >= 1; --s)
        {
          if (shiftData[s].finishAt)
          {
            return shiftData[s].finishAt.getTime();
          }
        }

        return workingLine ? workingLine.prevAt : 0;
      }),
      nextAt: shiftData.map((data, shiftNo) =>
      {
        if (shiftNo === 0)
        {
          return 0;
        ...
        for (let s = shiftNo + 1; s <= 3; ++s)
        {
          if (shiftData[s].startAt)
          {
            return shiftData[s].startAt.getTime();
          }
        }

        return workingLine ? workingLine.nextAt : 0;
      })
    };
  }
};
