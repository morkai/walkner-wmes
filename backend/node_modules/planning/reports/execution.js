...
exports.report = (app, module, options, done) =>
{
  ...
  function getLineOrderExecution(shiftOrders, lineOrder, planOrder)
  {
    ...
    let nextShiftOverlap = toTime - shiftEndTime;

    if (nextShiftOverlap > 0)
    {
      const nextShiftNo = shifts.getShiftInfo(localStartAt, false).no + 1;
      const nextShiftWorking = nextShiftNo !== 4
        && _.findLastIndex(shiftOrders, pso => pso.shift === nextShiftNo) !== -1;
    ...
    shiftOrders.forEach(function(pso)
    {
      ...
      else if (pso.shift === 1 && nextShiftOverlap > 0 && startedAt > toTime)
      {
        const fromTime = shifts.getShiftInfo(startedAt, false).startTime;
        const toTime = fromTime + nextShiftOverlap;
