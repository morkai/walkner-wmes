...
module.exports = (app, module, acceptDate) =>
{
  ...
  const UNFROZEN_PLANS = PROD ? [] : ['2021-03-19'];
  const LOG_LINES = PROD ? {} : {};
  const LOCK_LINES = true;
  const LOG = !PROD;
  const AUTO_GENERATE_NEXT = 0 && !DEV;
  const AUTO_GENERATE_START = 0 && !!1;
  ...
  function loadSettings(state, done)
  {
    step(
      function findExistingStep()
      {
        ...
        PlanOrderGroup.find({active: true, target: 'plan'}).lean().exec(this.parallel());
      },
  ...
  function createLineStates(state, done)
  {
    ...
    state.settings.lines.forEach(lineId =>
    {
      ...
      if (lineSettings.mrpPriority.length === 0)
      {
