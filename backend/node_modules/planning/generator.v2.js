...
module.exports = (app, module, acceptDate) =>
{
    ...
    step(
      ...
      function prepareStep(err, sapOrders)
      {
        ...
        sapOrders.forEach(sapOrder =>
        {
          ...
          (sapOrder.bom || []).forEach((component, i) =>
          {
            bom.set(
              component.nc12 || i.toString(),
              [
                `ITEM ${component.item || '0000'}`,
                `12NC ${component.nc12 || '000000000000'}`,
                `NAME ${component.name || '?'}`
              ].join(' ')
            );
          });
  ...
  function groupOrder(state, orderState)
  {
    ...
    state.grouping.orderGroups.forEach(orderGroup =>
    {
      const orderData = {
        ...
        bom: Array.from(sapOrder.bom.values())
      };
  ...
  function initLineState(state, lineState)
  {
    ...
    for (const [orderState, forward] of state.unfinishedOrders)
    {
      ...
      for (const plannedLineState of orderState.plannedOnLines)
      {
        ...
        if (!unfinishedOrder && _.last(plannedLineState.plannedOrders).orderNo === orderState.sapOrder._id)
        {
  ...
  function reverseLineState(state, lineState)
  {
    ...
    oldPlannedOrders.forEach(planOrder =>
    {
      const orderState = state.orderStates.get(planOrder.orderNo);

      orderState.quantityTodo += planOrder.quantity;

      if (!orderQuantities[planOrder.orderNo])
      {
        orderQuantities[planOrder.orderNo] = planOrder.quantity;

        reversedOrderStates.unshift({
          orderState,
          orderGroup: state.grouping.orderGroups.get(planOrder.group.toString())
        });
      }
      else
      {
  ...
  function handleOrderState(state, lineState, orderState, orderGroup)
  {
    ...
    else
    {
      ...
      if (orderState.lineGroup)
      {
        orderState.maxQuantityPerLine = resolveMaxQuantityPerLine(state, lineState, orderState);

        if (orderState.maxQuantityPerLine === 0)
        {
        ...
        else
        {
          logLine(lineState, 'Assigned order to line group.', {
            ...
            maxQuantityPerLine: orderState.maxQuantityPerLine
          });
        }
      }
    }
  ...
  function resolveMaxQuantityPerLine(state, lineState, orderState)
  {
    ...
    return Math.ceil(orderState.quantityTodo / availableLines.length);
  }
  ...
  function planOrderOnLine(state, lineState, orderState, orderGroup, quantityRemaining = 0)
  {
    ...
    {
      quantityRemaining = orderState.maxQuantityPerLine > 0
        ? Math.min(orderState.maxQuantityPerLine, orderState.quantityTodo)
    ...
    {
      logLine(lineState, `Order not planned on line.`, {
        line: lineState._id,
        orderNo
      });
  ...
  {
    if (orderState.pinnedLines.length === 0)
    {
      return false;
    }

    return orderState.pinnedLines[0] !== lineState._id;
  }
