...
const {mapTaktTimeCoeffs} = require('production/orderFinder');
  ...
  planSettingsSchema.statics.forGenerator = function(id, done)
  {
    ...
    step(
      function()
      {
        ...
        // TODO remove after 9.9.2020
        if (id >= Date.parse('2020-09-09T00:00:00.000Z'))
        {
          Setting
      ...
      function(err, planSettings, taktTimeCoeffs)
      {
        ...
        if (!planSettings)
        {
          return this.skip(app.createError('Settings not found.'));
        }

        taktTimeCoeffs = mapTaktTimeCoeffs(taktTimeCoeffs && taktTimeCoeffs.value || '');
        planSettings = planSettings.toGenerator(taktTimeCoeffs);
    ...
    const mrp = (mrpId) =>
    {
      if (!mrps.has(mrpId))
      {
        ...
        mrps.set(mrpId, {
          ...
          taktTimeCoeffs: taktTimeCoeffs[mrpId] || {'*': 1}
        });
      }
