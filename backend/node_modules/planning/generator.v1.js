...
module.exports = (app, module, acceptDate) =>
{
  ...
  const logger = module.logger.create({submodule: 'generator1'});
  const log = LOG && DEV
  ...
  function loadSettings(state, done)
  {
    step(
      ...
      function createNewStep(err, prevSettings)
      {
        ...
        newSettings.save(this.next(), {writeConcern: {w: 'majority'}});
      },
  ...
  function trySmallOrder(state, lineState, orderState, options)
  {
    ...
    let pceTime = getPceTime(order, workerCount, mrpSettings);
    ...
    while (quantityPlanned <= quantityTodo)
    {
      ...
      if (newFinishAt > activeTo || (shiftNo === 3 && h >= 6 && h < 22))
      {
        if (quantityPlanned > 0)
        {
          ...
          plannedOrders.push({
            ...
            pceTime,
            manHours: getManHours(order, quantityPlanned, mrpSettings),
      ...
      if ((shiftNo === 1 && h >= 14) || (shiftNo === 2 && (h >= 22 || h < 6)))
      {
        ...
        if (quantityPlanned === 0)
        {
          ...
          pceTime = getPceTime(order, workerCount, mrpSettings);
  ...
  function getLinesForBigOrder(state, order)
  {
    ...
    return state.settings.mrp(order.mrp).lines
      .filter(mrpLineSettings =>
      {
        ...
        const pceTime = getPceTime(order, workerCount, mrpSettings);
  ...
  function getPceTime(order, workerCount, mrpSettings)
  {
    if (!order.operation.laborTime)
    {
      return 0;
    }
    ...
    return Math.ceil(
      order.operation.laborTime / 100 / workerCount * 3600
        * schedulingRate
        * taktTimeCoeff(order)
    ) * 1000;
  }

  function getManHours(order, quantityTodo, mrpSettings)
  {
    if (!order.operation.laborTime)
    {
      return 0;
    ...
    return ((order.operation.laborTime / 100 * quantityTodo) + order.operation.laborSetupTime)
      * schedulingRate
      * taktTimeCoeff(order);
  }
  ...
  function getOrderStartOverhead(settings, lineState, orderState)
  {
    ...
    const laborSetupTime = orderState.order.operation.laborSetupTime
      * schedulingRate
      * taktTimeCoeff(orderState.order);
  ...
  {
    if (planLine.orders.length && planLine.orders[0].pceTimes.length === 0)
    {
      return;
    }
