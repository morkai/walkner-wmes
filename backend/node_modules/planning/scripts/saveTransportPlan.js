...
const host = process.argv[2] || 'wmes.pl';
const target = process.argv[3] || process.env.TEMP || process.env.TMP;
const headers = {
  'X-API-KEY': process.argv[4] || ''
};
...
step(
  function()
  {
    ...
    const req = {
      url: `https://${host}/reports/businessDays?from=${from.valueOf()}&to=${to.valueOf()}`,
      json: true,
      headers
    };
  ...
  function(err, res)
  {
    ...
    const from = res.body.days[1];
    const to = res.body.days.slice(0, 5).pop();

    const req = {
      url: `https://${host}/planning/reports/transport?from=${from}&to=${to}`,
      json: true,
      headers
    };
  ...
  function(err, res)
  {
    ...
    const req = {
      url: `https://${host}/xlsxExporter/${res.body}`,
      json: true,
      headers
    };
  ...
  function(err, res)
  {
    ...
    fs.copyFile(
      path.join(os.tmpdir(), `WMES_EXPORT__${res.body}.xlsx`),
      path.join(target, 'WMES_Plan_Transportu.xlsx'),
      this.next()
    );
  },
  function(err)
