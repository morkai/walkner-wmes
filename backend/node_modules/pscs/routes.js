...
module.exports = (app, module) =>
{
  ...
  const canView = user.auth('PSCS:VIEW');
  const canManage = user.auth('PSCS:MANAGE');
  ...
  function prepareForAdd(req, res, next)
  {
    ...
    step(
      ...
      function(err, userModel, passedResult, incompleteResult)
      {
        if (err)
        {
          return next(err);
        ...
        req.body = {
          ...
          user: user.createUserInfo(userModel, {}),
          answers: answers,
          validity: answers.map(() => false)
        };

        req.body.user.gender = userModel.gender;
