...
module.exports = (app, module) =>
{
  ...
  const canManageSettings = user.auth('PROD_DATA:MANAGE');
  const canViewMaster = user.auth('FTE:MASTER:VIEW');
  const canViewLeader = user.auth('FTE:LEADER:VIEW');
  const canViewWh = user.auth('FTE:WH:VIEW');
  const canExportWh = user.auth('FTE:WH:VIEW', 'REPORTS:VIEW', 'REPORTS:6:VIEW');

  // Settings
  express.get(
    '/fte/settings',
    canManageSettings,
    settings.browseRoute('fte')
  );
  ...
  express.get(
    ...
    express.crud.exportRoute.bind(null, app, {
      ...
      serializeStream: exportFteLeaderEntries.bind(null, app, subdivisions),
  ...
  function limitToDivision(limitToSubdivisions, req, res, next)
  {
    ...
    const limitedSubdivisions = [];

    if (divisionTerm)
    {
      _.pull(selector, divisionTerm);

      _.forEach(subdivisions.models, (subdivisionModel) =>
      {
        if (subdivisionModel.division === divisionTerm.args[1])
        {
          limitedSubdivisions.push(subdivisionModel._id.toString());
        }
      });
    }
    ...
    {
      limitToSubdivisions(limitedSubdivisions);
    }

    if (limitedSubdivisions.length)
    {
      selector.push({
        name: 'in',
        args: ['subdivision', limitedSubdivisions]
      });
    }
  ...
  function limitToOther(limitedSubdivisions)
  {
    subdivisions.models.forEach(s =>
    {
      if (s.division !== WH_DIVISION)
      {
        limitedSubdivisions.push(s._id.toString());
      }
    });
  }

  function limitToWarehouse(limitedSubdivisions)
  {
    subdivisions.models.forEach(s =>
    {
      if (s.division === WH_DIVISION)
  ...
  {
    FteEntry.findById(req.params.id).exec((err, fteEntry) =>
    {
      if (err)
      {
        return next(err);
  ...
  {
    module.getCachedEntry(type, req.params.id, (err, fteEntry) =>
    {
      if (err)
      {
        return next(err);
  ...
  function serializeFteMasterEntry(doc, req)
  {
    ...
    const subdivision = subdivisions.modelsById[doc.subdivision];
    ...
    doc.tasks.forEach(task =>
    {
      ...
      req.dictionaries.functionList.forEach(functionId =>
      {
        ...
        const functionCompanies = req.dictionaries.functionToCompanies[functionId];

        req.dictionaries.companyList.forEach(companyId =>
        {
          if (!functionCompanies[companyId])
          {
            return;
          }
