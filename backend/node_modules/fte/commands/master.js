...
module.exports = (app, module) =>
{
  ...
  return {
    findOrCreate: findOrCreate.bind(null, app, module, FteMasterEntry, 'MASTER'),
    updateCount: (socket, data, reply) =>
    {
      ...
      step(
        ...
        {
          this.releaseLock = module.acquireLock(data._id, this.next());
        },
        function getCachedEntryStep()
        {
          module.getCachedEntry('master', data._id, this.next());
        },
        ...
        function sendResultStep(err)
        {
          if (err)
          {
            module.cleanCachedEntry(data._id);
            reply(err);
          }
          else
          {
    ...
    addAbsentUser: (socket, data, reply) =>
    {
      if (!_.isFunction(reply))
      {
        reply = () => {};
      ...
      if (!_.isObject(data)
        ...
        || !_.isString(data.user.personnelId))
      {
        return reply(app.createError('INPUT', 400));
      }
      ...
      FteMasterEntry.addAbsentUser(data._id, data.user, updater, (err) =>
      {
        if (err)
        {
          return reply(err);
    ...
    removeAbsentUser: (socket, data, reply) =>
    {
      if (!_.isFunction(reply))
      {
        reply = () => {};
      ...
      FteMasterEntry.removeAbsentUser(data._id, data.userId, updater, (err) =>
      {
        if (err)
        {
          return reply(err);
