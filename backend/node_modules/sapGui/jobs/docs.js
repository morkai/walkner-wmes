...
module.exports = (app, sapGuiModule, job, done) =>
{
  ...
  const ordersUrl = new URL(job.baseUrl);
  const planUrl = new URL(job.baseUrl);
  ...
  function runNext(done)
  {
    ...
    step(
      ...
      function(err, res, plan)
      {
        ...
        const excludedMrps = job.excludeMrps || [];

        this.includedMrps = new Set();
        this.orders = new Set();

        plan.orders.forEach(o =>
        {
          if (excludedMrps.length && excludedMrps.includes(o.mrp))
          {
            return;
          }

          this.includedMrps.add(o.mrp);
          this.orders.add(o._id);
        });

        sapGuiModule.info(`[${job.id}] [${planDate}] Found ${this.orders.size} orders`);
      },
      function()
      {
        if (!Array.isArray(job.includeMrps) || job.includeMrps.length === 0)
        {
          return;
        }

        const extraMrps = job.includeMrps.filter(mrp => !this.includedMrps.has(mrp)).join(',');
        ...
        ordersUrl.search = `select(_id)&limit(0)&mrp=in=(${extraMrps})&scheduledStartDate=${planDate.valueOf()}`;
      ...
      function(err, res, orders)
      {
        if (err)
        {
          return this.skip(app.createError(`Failed to fetch extra orders: ${err.message}`));
        }

        if (orders && Array.isArray(orders.collection))
        {
          sapGuiModule.info(`[${job.id}] [${planDate}] Found ${orders.collection.length} extra orders`);

          orders.collection.forEach(o => this.orders.add(o._id));
        }
      },
      function()
      {
        ...
        fs.writeFile(this.inputPath, [...this.orders].join('\r\n'), this.next());
      },
      function(err)
      {
        if (err)
