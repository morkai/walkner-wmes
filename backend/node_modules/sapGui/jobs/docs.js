...
module.exports = (app, sapGui, job, done) =>
{
  ...
  function handleOrders()
  {
    ...
    {
      return setImmediate(done, app.createError('No orders specified.', 'INPUT', 400));
    }

    runNextBatch('0000-00-00', [job.orders], 0, job.orders.length, done);
  }
  ...
  function runNextBatch(date, batches, count, totalCount, done)
  {
    ...
    step(
      ...
      function(err, exitCode, output)
      {
        ...
        if (err)
        {
          if (err.message === 'MISSING_OUTPUT_FILE')
          {
            err = null;

            sapGui.warn('Batch NOK: missing output file.', {
              jobId,
              date,
              count,
              totalCount,
