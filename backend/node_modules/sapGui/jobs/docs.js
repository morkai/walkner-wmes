...
module.exports = (app, sapGuiModule, job, done) =>
{
  ...
  function runNext(done)
  {
    ...
    sapGuiModule.info(`Started...`, {jobId, date});
    ...
    step(
      ...
      function(err, response, result)
      {
        ...
        sapGuiModule.info(`Found orders.`, {jobId, date, totalCount});

        const batches = [];

        while (result.orders.length)
        {
          batches.push(result.orders.splice(0, 500));
        }
      ...
      function(err, exitCode, output)
      {
        if (err)
        {
          sapGuiModule.error(err, `Transaction failed.`, {jobId, date, exitCode, output});

          return done(err, exitCode, output);
        }

        sapGuiModule.info(`OK!`, {jobId, date});
  ...
  function runNextBatch(date, batches, count, totalCount, done)
  {
    ...
    step(
      function()
      {
        ...
        sapGuiModule.info(`Batch started...`, {jobId, date, count, totalCount});
      ...
      function(err, exitCode, output)
      {
        ...
        sapGuiModule.info(`Batch OK!`, {jobId, date, count, totalCount, duration});
