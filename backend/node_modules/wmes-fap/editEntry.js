...
{
  const {user, settings, FapEntry} = module;
  ...
  function edit(done)
  {
    step(
      function()
      {
        FapEntry.findById(entryId).exec(this.parallel());

        if (data.category || data.subCategory)
        {
          settings.findById('fap.categoryFunctions', this.parallel());
        }
      },
      function(err, entry, categoryFunctions)
      {
        if (err)
        {
        ...
        const canChangeCategory = categoryFunctions && _.includes(categoryFunctions.value, sessionUser.prodFunction);
        const isUser = user.isAllowedTo(sessionUser, [['USER']]);
        const isManager = user.isAllowedTo(sessionUser, [['FN:manager']]);
        const isProcessEngineer = user.isAllowedTo(sessionUser, [
          ['FN:process-engineer'],
          ['FN:process-engineer-NPI'],
        ...
        const isWhman = user.isAllowedTo(sessionUser, [['FN:whman'], ['FN:wh']]);
        ...
        if (canManage || canChangeCategory)
        {
          props.push(
            'category',
            'subCategory'
        ...
        if (canManage || isProcessEngineer || isMaster || isLeader || isWhman || isAnalyzer)
        {
        ...
        if (entry.level === 0 && input.level > 0)
        {
      ...
      async function(err, entry, notify)
      {
        done(err);
        ...
        if (change.data.orderNo)
        {
          try
          {
            await FapEntry.assignSapOrderData(entry);

            change.data.orderStatus = [null, entry.orderStatus];
          }
