  ...
  function edit(done)
  {
    step(
      ...
      function(err, entry)
      {
        ...
        const isQualityEngineer = user.isAllowedTo(sessionUser, 'FN:quality_engineer');
        ...
        if (canManage || isProcessEngineer || isQualityEngineer || isDesigner)
        {
          props.push(
            'category'
          );
        ...
        // TODO Remove after 17.07.2019 06:00:00
        if (Date.now() >= 1563336000000)
        {
        ...
        else if (entry.status === 'pending' && input.status === 'started')
        {
          resolveRemainingSubscribers(entry, input, this.parallel());
        }
      },
