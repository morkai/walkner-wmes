...
module.exports = (app, module) =>
{
  ...
  const templates = {
    ...
    creator: ejs.compile(fs.readFileSync(`${__dirname}/templates/notifier/creator.pl.ejs`, 'utf8'), {
      cache: true,
      compileDebug: false,
      rmWhitespace: true
    })
  ...
  const creatorNotifications = new Map();
  ...
  function onNotification({seen}, topic)
  {
    ...
    {
      seen.forEach(id => clearCreatorNotification(id, topic.split('.').pop()));
    }
  }
  ...
  function onEntryUpdated(message)
  {
    ...
    if (message.notify)
    {
      ...
      {
        scheduleCreatorNotification(message._id, message.creator);
      }
  ...
  function sendNewEmail(entry, recipients, done)
  {
    ...
    const email = {
      to: recipients,
      replyTo: recipients,
      subject: `[WMES] [FAP] Nowe zgłoszenie: ${entry.rid}`,
      html: templates.new({
  ...
  function sendCreatorEmail(entry, recipients, done)
  {
    const email = {
      to: recipients,
      replyTo: recipients,
      subject: `[WMES] [FAP] Wymagana akcja: ${entry.rid}`,
      html: templates.creator({
        urlPrefix: app.options.emailUrlPrefix,
        entry
      })
    };
  ...
  function scheduleCreatorNotification(entryId, userId)
  {
    ...
    if (!creatorNotifications.has(entryId))
    {
      creatorNotifications.set(entryId, {
        entryId,
        userId,
        timer: null
      });
    ...
    const pendingNotification = creatorNotifications.get(entryId);

    clearTimeout(pendingNotification.timer);
    pendingNotification.timer = setTimeout(notifyCreator, 60 * 1000, entryId, userId);
  }

  function clearCreatorNotification(entryId, userId)
  {
    if (typeof entryId !== 'string')
    {
      entryId = String(entryId);
    ...
    if (!pendingNotification || pendingNotification.userId !== userId)
    {
      return;
    }
    ...
    creatorNotifications.delete(entryId);
  }

  function notifyCreator(entryId, userId)
  {
    clearCreatorNotification(entryId);

    if (!app[module.config.mailSenderId])
    {
      return;
    }
    ...
    step(
      ...
      function(err, entry, user)
      {
        ...
        sendCreatorEmail(entry, [user.email], this.next());
      },
      function(err)
      {
        if (err)
