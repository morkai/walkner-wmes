...
exports.models = [
  ...
  require('wmes-fap/models/fapSubCategory'),
  require('user/models/user')
];

exports.report = (app, {FapCategory, FapSubCategory, FapEntry, User}, options, done) =>
{
  ...
  step(
    function findStep()
    {
      ...
      if (options.categories.length && options.subCategories.length)
      {
        conditions.$or = [
          {category: {$in: options.categories}},
          {subCategory: {$in: options.subCategories}}
        ];
      }
      ...
      else if (options.subCategories.length)
      {
        conditions.subCategory = {$in: options.subCategories};
      }

      if (options.levels >= 0)
      {
        conditions.level = options.levels;
      }
    ...
    function finalizeStep(err, categories, subCategories)
    {
      if (err)
      {
      ...
      {
        results.subCategories[subCategory._id] = subCategory.name;
      });
  ...
  function handleEntry(entry)
  {
    ...
    if (entry.divisions.length)
    {
      inc('division', entry.divisions);
    }
    ...
    inc('assessment');
    inc('level');
    inc('shift', shifts.getShiftNo(entry.createdAt, false));
    inc('hour', entry.createdAt.getHours());
    incUser('owner');
    incUser('solver');
