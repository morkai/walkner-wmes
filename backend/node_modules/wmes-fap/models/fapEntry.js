...
exports.setUp = (app, mongoose) =>
{
  ...
  fapEntrySchema.index({'changes.user.id': 1});
  fapEntrySchema.index({'creator.id': 1});
  ...
  fapEntrySchema.statics.assignSapOrderData = async function(entry)
  {
    entry.orderStatus = [];
    ...
    entry.orderStatus = sapOrder.statuses;
  ...
  fapEntrySchema.methods.updateAttachments = function(changes, attachments)
  {
    ...
    this.attachments.forEach(a => oldAttachments[a._id] = a);

    attachments.forEach(newAttachment =>
    {
      const oldAttachment = oldAttachments[newAttachment._id];
    ...
    else if (edited[0].length)
    {
      ...
      edited[1].forEach(a => Object.assign(oldAttachments[a._id], a));
    }
