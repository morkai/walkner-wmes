...
exports.setUp = (app, mongoose) =>
{
  ...
  fapEntrySchema.statics.markAsSeen = function(entryId, userId, done)
  {
    const FapEntry = this;

    FapEntry.findById(entryId, {_id: 1}).lean().exec((err, entry) =>
    {
      if (err)
      {
        return done(err);
      ...
      const conditions = {
        _id: entry._id,
        'observers.user.id': String(userId)
      };
      const update = {
        $set: {
      ...
      FapEntry.collection.updateOne(conditions, update, (err) =>
      {
        if (err)
        {
          return done(err);
    ...
    unsubscribersList)
  {
    ...
    {
      if (o.notify && changedPropertyMap.subscribers)
      {
        o.changes.subscribers = true;
      }
    });
    ...
    {
      if (!observer || newObserverMap[observer.user.id] || observer.user.id === guestUserId)
      {
        return;
      }
