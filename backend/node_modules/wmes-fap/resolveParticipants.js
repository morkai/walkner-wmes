...
module.exports = (app, {config, DelayReason}, {mrp, category, date, notifications}, done) =>
{
  ...
  step(
    function()
    {
      if (!notifications)
      {
        DelayReason.findById(category, {notifications: 1}).lean().exec(this.next());
      }
      else
      {
    ...
    function(err, delayReason)
    {
      ...
      if (!delayReason || !delayReason.notifications)
      {
        return this.skip(null, []);
      }

      const subdivisionToFunctions = mapSubdivisionToFunctions(delayReason.notifications);
      const users = new Map();
      const watchSections = [];
