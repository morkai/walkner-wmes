...
{
  const {express, FapEntry, FapCategory, FapSubCategory} = module;
  ...
  step(
    function()
    {
      FapCategory.find({}, {name: 1}).lean().exec(this.parallel());

      FapSubCategory.find({}, {name: 1}).lean().exec(this.parallel());
    },
    function(err, categories, subCategories)
    {
      if (err)
      {
      ...
      subCategories.forEach(c => subCategoryMap.set(c._id.toString(), c.name));
    },
    function(err)
    {
      if (err)
  ...
  function serializeRow(doc)
  {
    ...
    const subCategoryId = doc.subCategory ? doc.subCategory.toString() : null;
    const comments = [];
    ...
    return {
      ...
      status: doc.status,
      subdivisionType: doc.subdivisionType,
      orderNo: doc.orderNo,
      nc12: doc.nc12,
      mrp: doc.mrp,
      qtyTodo: doc.qtyTodo,
      ...
      component: doc.componentCode,
      componentName: doc.componentName,
      category: categoryMap.has(categoryId) ? categoryMap.get(categoryId) : categoryId,
      subCategory: subCategoryMap.has(subCategoryId) ? subCategoryMap.get(subCategoryId) : subCategoryId,
      problem: doc.problem,
