...
module.exports = (app, module, req, res, next) =>
{
  ...
  step(
    ...
    function(err, entry)
    {
      ...
      const props = ['comment', 'subscribers', 'attachments'];
  ...
  function resolveRemainingSubscribers(entry, input, done)
  {
    module.resolveParticipants(entry, (err, result) =>
    {
      ...
      input.subscribers.forEach(s => participants.add(s.id));

      entry.observers.forEach(o => participants.add(o.user.id));

      const subscribers = result.users.filter(u => !participants.has(u._id.toString())).map(u => ({
        id: u._id.toString(),
      ...
      {
        input.subscribers = input.subscribers.concat(prepareSubscribers(subscribers));
      }
