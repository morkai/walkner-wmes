...
define([
  ...
  'app/time',
  'app/core/View',
  'app/paintShop/templates/userPicker'
], function(
  _,
  t,
  time,
  ...
  return View.extend({
    ...
    dialogClassName: 'paintShop-userPicker-dialog',

    events: {
      'click .paintShop-userPicker-user': function(e)
      {
    ...
    loadUsers: function()
    {
      ...
      var req = view.ajax({
        url: '/users?select(login,firstName,lastName,searchName)'
          + '&privileges=in=(PAINT_SHOP%3APAINTER,PAINT_SHOP%3AMANAGE)'
      });
    ...
    renderUsers: function(users)
    {
      ...
      {
        return '<div class="paintShop-userPicker-user" data-id="' + user.id + '">' + _.escape(user.label) + '</div>';
      });
      ...
      if (this.model.user)
      {
        this.$('.paintShop-userPicker-user[data-id="' + this.model.user.id + '"]').click();
      }
    }
