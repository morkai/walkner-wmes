...
define([
  ...
  'app/time',
  'app/core/View',
  'app/wmes-wiring/templates/userPicker'
], function(
  _,
  t,
  time,
  ...
  return View.extend({
    ...
    dialogClassName: 'wiring-userPicker-dialog',

    events: {
      'click .wiring-userPicker-user': function(e)
      {
    ...
    loadUsers: function()
    {
      ...
      var req = view.ajax({
        url: '/users?select(login,firstName,lastName,searchName)'
          + '&privileges=in=(WIRING%3ADRILLER,WIRING%3AMANAGE)'
      });
    ...
    renderUsers: function(users)
    {
      ...
      {
        return '<div class="wiring-userPicker-user" data-id="' + user.id + '">' + _.escape(user.label) + '</div>';
      });
      ...
      if (this.model.user)
      {
        this.$('.wiring-userPicker-user[data-id="' + this.model.user.id + '"]').click();
      }
    }
